...
def build_pop_B(variant, min_rr_a=MIN_RR_NEW):
    ...
    pop_idx_sel ajoute plus bas (bug trouve en test de fumee sur le script
    de stress-test compagnon : n=519/630 au lieu de 460/571 attendus)."""
...
if __name__ == "__main__":
    ...
    for variant in ("naturel", "tout_indices"):
        pop_v = build_pop_B(variant)
        tickers = sorted(pop_v["ticker"].unique())
        excluded_map_v = precompute_correlation_pairs(tickers, corr_matrix, CORR_TH_NEW)
        populations[variant] = (pop_v, excluded_map_v)
        print(f"[verif] population B '{variant}' : {len(pop_v)} trades, {len(tickers)} tickers")
    ...
    for ceiling in ceilings_arg:
        ...
        for variant in ("naturel", "tout_indices"):
            pop_v, excluded_map_v = populations[variant]
            t0 = time.time()
            df = run_propagated(pop_v, market_data, excluded_map_v, ceiling, seq, config,
                                 bb_threshold=bb_th, **common_kwargs)
            row = summarize(df, f"B_{variant}", ceiling, bb_th, True)
            rows.append(row)
            print(f"[B_{variant:13s} plafond={ceiling:.0f}$] profit_moy={row['profit_moyen']:+,.0f}$ "
                  f"solde_neg={row['solde_negatif_annee4']:.2f}% hit_ceiling={row['hit_ceiling_pct']:.2f}% "
                  f"annee1<0={row['annee1_neg']:.2f}% ({time.time()-t0:.0f}s)")
        pd.DataFrame(rows).to_csv(f"chantier_strategie_b_isolation_indices_n{n_sims}_2026-08-18.csv", index=False)
