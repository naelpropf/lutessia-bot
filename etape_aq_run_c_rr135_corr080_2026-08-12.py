"""
Etape AQ (08/12) : REGENERATION de Run C sous la nouvelle base RR>=1,35 +
...
etape_ai_payout_cadence_calibration_2026-08-11.py, SEULS 2 parametres
changes : min_rr 1.25->1.35 (build_population_with_trailing) et le seuil de
correlation CORR_TH(0.6)->0.80 (precompute_correlation_pairs) -- aucune
autre logique touchee. Le script etape_ai original reste intact et figure
(convention du projet, "un rapport deja ecrit reste fige") -- ses chiffres
(5 491 410$/1000$, 5 542 103$/3000$) restent l'ANCIENNE reference pour
...
if __name__ == "__main__":
    ...
    for ceiling in ceilings_arg:
        ...
        row = summarize(df, "RUN_C_rr135_corr080_2026-08-12", ceiling)
        rows.append(row)
        ...
        pd.DataFrame(rows).to_csv(f"etape_aq_run_c_rr135_corr080_n{n_sims}.csv", index=False)
