"""
Etape AL (08/11) -- RUN B de la decomposition delai/forfeiture :
FORFEITURE REELLE SEULE, sans delai. Copie de etape_ai_payout_cadence_
calibration_2026-08-11.py (cadence par firm/occurrence conservee comme
horloge THEORIQUE uniquement, voir plus bas) -- mecanisme change :

- Le profit NET credite IMMEDIATEMENT dans total_funded_pnl/state["reserve"]
  a chaque trade gagnant (comme le moteur original, avant toute correction
  payout) -- PAS de retard d'acces au cash.
- Pour le suivi de la fenetre de forfeiture, chaque compte accumule quand
  meme acc["theoretical_pending"] (bookkeeping pur, ne bloque RIEN) =
  profit gagne depuis le dernier flush THEORIQUE (meme cadence par firm/
  occurrence que Run A/C). A chaque flush theorique, remis a 0 (le cash
  est deja disponible depuis longtemps, seul le compteur se reinitialise).
- Sur une VRAIE casse (firm a forfeiture, Blueberry/GFT/Fivers) :
  acc["theoretical_pending"] (ce qui SERAIT encore en attente sous le
  modele a delai) est retroactivement RETIRE de total_funded_pnl et de
  state["reserve"] (proportion reserve_share) -- le cash avait deja ete
  disponible/depense potentiellement, ce retrait peut rendre reserve
  negatif ponctuellement (approximation acceptee, coherente avec le
  traitement existant de shortfall/reserve dans handle_cost_hybrid).

Objectif : avec Run A (delai seul, etape_ak) et Run C (deja fait, les
deux combines = ~32,67% annee1<0), decomposer les ~8pt d'ecart entre Run
C (32,67%) et Run E (24,67%, aucune contrainte cash) en part attribuable
au SEUL retard d'acces au cash (Run A) vs a la SEULE forfeiture reelle
(Run B, ce script).
    ...
    for slot_idx, trade_idx in enumerate(order):
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                # Run B (etape AL) : credit IMMEDIAT, PAS de reversal --
                # process_trade_mf a deja credite net_pnl dans
                # total_funded_pnl/reserve, on laisse tel quel. On
                # accumule seulement acc["pending_payout"] comme compteur
                # THEORIQUE (bookkeeping pur, ne bloque/ne retire RIEN au
                # moment du gain) pour savoir combien serait "encore en
                # attente" sous le modele a delai si une casse survenait.
                ...
                # Run B : "flush" THEORIQUE seulement -- le cash est deja
                # disponible depuis le gain (credit immediat ci-dessus),
                # ce point ne fait que remettre a 0 le compteur de fenetre
                # de forfeiture (meme cadence par firm/occurrence que Run
                # A/C), aucun mouvement de total_funded_pnl/reserve ici.
                ...
                if use_goat_guard:
                    ...
                    # Run B : forfeiture RETROACTIVE -- le cash avait deja
                    # ete credite immediatement au gain, on le retire
                    # maintenant de total_funded_pnl/reserve (peut rendre
                    # reserve ponctuellement negatif, approximation
                    # acceptee, coherente avec le traitement existant de
                    # shortfall dans handle_cost_hybrid).
                    if use_payout_cycle and acc["pending_payout"] != 0.0:
                        ...
                        if forfeited > 0:
                            ...
                            events[gname] += 1
                            acc["total_funded_pnl"] -= forfeited
                            state["reserve"] -= forfeited * reserve_share
                        acc["pending_payout"] = 0.0
...
if __name__ == "__main__":
    ...
    for ceiling in ceilings_arg:
        ...
        row = summarize(df, "RUN_B_forfeiture_seule_2026-08-11", ceiling)
        rows.append(row)
        ...
        pd.DataFrame(rows).to_csv(f"etape_al_run_b_forfeiture_seule_n{n_sims}.csv", index=False)
