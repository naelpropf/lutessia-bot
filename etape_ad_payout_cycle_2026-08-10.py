"""
Etape AD (08/10 nuit, suite 13) : modelise un cycle de payout realiste
pour Blueberry/GFT/Fivers (confirmes par contact support direct : profit
NON preserve sur casse) -- FTMO et FundedNext restent NON concernes
(profit preserve, confirme). Copie de etape_q_v2_plus_ftmo_gft_2026-08-10.py,
mecanisme ajoute :

- Chaque compte Blueberry/GFT/Fivers finance accumule le profit NET (net_pnl,
  meme formule split_flat que l'existant) d'un trade gagnant dans
  acc["pending_payout"] au lieu de le crediter immediatement dans
  total_funded_pnl/reserve. Les PERTES continuent de frapper total_funded_pnl
  immediatement (une perte reelle n'est jamais "en attente de payout").
- Tous les PAYOUT_CYCLE_DAYS=14 jours (par compte, depuis ouverture ou
  dernier payout), pending_payout est "verse" -- transfere dans
  total_funded_pnl et (si positif) dans state["reserve"] -- puis remis a 0.
- Sur une VRAIE casse (pas un soft-breach GFT Goat Guard, qui n'est PAS un
  reset), pending_payout est simplement abandonne (perdu) -- pas transfere.
  Instrumentation : montant total perdu par firm, pre/post-deblocage.
- FTMO/FundedNext : comportement INCHANGE (credit immediat, comme avant),
  profit preserve confirme par contact support (FTMO Clause 12.3,
  FundedNext confirme).

Toggle payout_cycle=False reproduit EXACTEMENT etape_q (comportement
actuel, non corrige) -- permet des ablations controlees (avec/sans V2,
avec/sans Goat Guard, avec/sans le cycle de payout) depuis le meme script.
    ...
    for slot_idx, trade_idx in enumerate(order):
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                # Cycle de payout : verse pending_payout tous les 14j (par
                # compte). Verifie AVANT la detection de casse -- si le
                # versement tombe le meme trade qu'une casse, l'argent est
                # deja "sorti" avant que la casse ne puisse le reprendre.
                if use_payout_cycle and acc["active"] and acc["last_payout_time"] is not None \
                        and now - acc["last_payout_time"] >= PAYOUT_CYCLE_DAYS * 86400:
...
def summarize(df, label, ceiling):
    ...
    row = dict(config=label, ceiling=ceiling, n=len(df), profit=net.mean(), ruine=(net < 0).mean() * 100,
...
if __name__ == "__main__":
    print("Ce script est une bibliotheque -- utiliser un driver dedie (voir etape_ae_*).")
