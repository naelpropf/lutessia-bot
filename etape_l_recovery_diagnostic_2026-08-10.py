...
if __name__ == "__main__":
    ...
    configs = [("REF_pur", None, None, False), ("REF_V2", 0.20, 0.5, True)]
    ...
    for label, entry, reduction, pre_only in configs:
        ...
        df.to_csv(f"etape_l_recovery_diagnostic_{label}_n{n_sims}_c{int(ceiling)}.csv", index=False)
        print(f"[{label}] termine en {time.time()-t0:.0f}s -> etape_l_recovery_diagnostic_{label}_n{n_sims}_c{int(ceiling)}.csv")
