"""
CHANTIER instant funding -- bascule conditionnelle Blueberry, FLOTTE COMPLETE
(08/16). Contrairement aux chantiers precedents (compte isole, tresorerie
infinie), ce script integre le mecanisme dans le vrai moteur de flotte
(copie de chantier_position_cap_2026-08-15.py / etape_aq, convention du
projet "copie figee, seuls les points marques <<< CHANTIER changes") avec
le vrai plafond personnel et la vraie trajectoire de reserve dans le temps.

Mecanisme de bascule : tant que state["reserve"] < bb_threshold, tout
compte Blueberry (nouveau OU reouvert apres casse) utilise le format
classique deja retenu (Blueberry_Prime2Step). Des que reserve >=
bb_threshold, tout NOUVEAU compte Blueberry (reouverture ou extra-compte)
bascule sur Blueberry_InstantElite. Un compte deja ouvert ne change PAS de
format en cours de route (la decision se prend uniquement a l'ouverture/
reouverture, pas retroactivement).

bb_threshold=float('inf') = jamais d'instant (reference 100% classique).
bb_threshold=0.0 = instant des le premier accès a la decision (mais le
starter jour 0 est TOUJOURS classique car ouvert AVANT que reserve puisse
etre >=0 de facon significative -- reserve=0.0 au tout debut, donc
bb_threshold=0.0 bascule bien des le premier evenement qui suit, cf. note
plus bas).

Le "reset Blueberry" (2x prix challenge, skip-to-funded) est un mecanisme
documente SPECIFIQUEMENT pour le produit Prime2Step -- aucune source ne
documente d'equivalent pour Instant Elite dans ce projet (cf. chantier
instant funding Phase 1). Modelise ici : le reset a prix reduit reste
disponible UNIQUEMENT si le compte est actuellement en format classique
au moment de la casse ; en format instant, chaque casse paie le plein
tarif instant (coherent avec les chantiers precedents "compte isole").
"""
import random
import sys
import time
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
            ...
            bb_threshold=float("inf")):
    fmt_by_firm = {g: FORMATS[k] for g, k in format_by_firm.items()}

    def bb_choose_fmt_key():
        # <<< CHANTIER : decision de format Blueberry, prise UNIQUEMENT a
        # l'ouverture/reouverture (jamais retroactive sur un compte deja actif).
    ...
    accounts_by_group = {}
    active0_cost = 0.0
    # <<< CHANTIER : state doit exister AVANT la creation des comptes (bb_choose_fmt_key
    # lit state["reserve"]) -- initialise ici en 2 temps (state minimal puis complet).
    state = {"reserve": 0.0}
    for gname in FIRMS:
        ...
        if gname == "Blueberry":
            ...
            cost = ei.price_for(fmt_key, palier)
        else:
    ...
    def reopen_account(acc, cost, fmt, skip_to_funded=False):
        ...
        acc["cost"] = cost  # <<< CHANTIER : conserve le VRAI cout de cette reouverture
        # (peut differer de base_cost si le format vient de changer)
        ...
        if downgrade_active() and acc.get("_gname") == ei.STARTER and acc.get("_fmt_key") == BB_CLASSIC_KEY:
            # <<< CHANTIER : le downgrade pre-unlock (palier ramene a base) ne s'applique
            # qu'en format classique -- Instant Elite n'a pas de mecanisme de palier
            # variable dans ce moteur (toujours 25k, cf. FORMATS), rien a downgrader.
    ...
    for slot_idx, trade_idx in enumerate(order):
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                if not acc["active"]:
                    continue
                fmt = FORMATS[acc["_fmt_key"]]  # <<< CHANTIER : format PAR COMPTE, pas par groupe
                ...
                if use_goat_guard:
                    ...
                    # <<< CHANTIER : reset a prix reduit (2x base_cost) reserve au format
                    # CLASSIQUE uniquement (documente pour Prime2Step, pas pour Instant
                    # Elite -- cf. docstring de module et chantier Phase 1).
                    ...
                    if use_bb_reset:
                        ...
                        if gname == "Blueberry":
                            # <<< CHANTIER : re-decide le format A CHAQUE reouverture,
                            # selon la reserve au moment precis de la casse.
                            ...
                            cost = ei.price_for(new_fmt_key, palier_for_cost)
...
def summarize(df, label, ceiling, bb_threshold):
    ...
    return dict(config=label, ceiling=ceiling, bb_threshold=bb_threshold, n=len(df),
...
if __name__ == "__main__":
    ...
    ceilings_arg = [float(x) for x in sys.argv[2].split(",")] if len(sys.argv) > 2 else [960.0, 5000.0]
    ...
    for ceiling in ceilings_arg:
        # reference 100% classique (bb_threshold=inf) EN PREMIER pour chaque plafond
        for bb_threshold in [float("inf")] + thresholds_arg:
            ...
            df = run_propagated(pop, market_data, excluded_map, ceiling, seq, config, ei.DEFAULT_EMERGENCY,
                                 ...
                                 bb_threshold=bb_threshold)
            label = "REF_100pct_classique" if bb_threshold == float("inf") else f"bb_threshold_{bb_threshold:.0f}"
            row = summarize(df, label, ceiling, bb_threshold)
            rows.append(row)
            print(f"[plafond={ceiling:.0f}$ {label:24s}] profit_moyen={row['profit_moyen']:+,.0f}$ "
                  ...
                  f"bb_instant_opens={row['bb_instant_opens_moy']:.2f} "
                  f"bb_classic_opens={row['bb_classic_opens_moy']:.2f} ({time.time()-t0:.0f}s)")
            pd.DataFrame(rows).to_csv(f"chantier_blueberry_switch_n{n_sims}.csv", index=False)
