"""
Regeneration 2026-08-19 -- copie de chantier_S1_8_officiel_n600_risque_
corrige_2026-08-17.py, AUCUNE logique modifiee. Seul le nom de fichier CSV
de sortie change (suffixe date + ceiling pour permettre l'execution
parallele par plafond sans collision).

Raison : `rr_threshold_test.build_extended_population()` contenait encore
le bug forex-only (321 trades indices exclus) au moment ou le CSV original
(chantier_S1_8_officiel_n600_risque_corrige_2026-08-17.csv, mtime 08/18
01:18) a ete genere -- le correctif forex-only n'a ete commit que le 08/18
14:38:33 (acdfb62). Le chiffre COMBINE_corrige=6 693 474$/15,67% du
registre S2.45 est donc SEULEMENT corrige du cap risque Instant, PAS de la
population elargie (742 trades vs 631). Ce script, execute maintenant,
herite automatiquement des deux correctifs (le fix forex-only est dans le
code partage `rr_threshold_test.py`, appele via build_population_with_
trailing -> build_realistic_payoff_population -> build_extended_population)
-- premiere regeneration reellement doublement corrigee.
...
def build_market_data_with_indices():
    """<<< CORRECTIF 2026-08-19 : la population elargie (742 trades, fix
    forex-only) fait planter feasible_risk_pct() (KeyError) car forex_market_
    data.json ne couvre que le forex -- aucune spec broker reelle pour les
    indices (tick_size/tick_value/margin_per_lot DAX40/S&P500/NASDAQ100 CFD
    chez Blueberry, jamais recherchee). Reprise A L'IDENTIQUE du precedent deja
    etabli et documente dans chantier_strategie_b_isolation_indices_2026-08-18.py:
    802-819 (et chantier_b6_montecarlo_2026-08-19.py:755-763) : contrainte de
    capacite desactivee (valeurs qui ne contraignent jamais feasible_risk_pct)
    pour les 5 labels index reellement presents dans la population (verifie,
    'MINI DJ30 FULL0625' absent car deja exclu par le filtre de mappabilite).
    L'economie du trade (R realise) vient des donnees historiques reelles,
    INCHANGEE par cette simplification. A remplacer par de vraies specs
    broker si ce candidat devient serieux."""
...
if __name__ == "__main__":
    ...
    for ceiling in ceilings_arg:
        t0 = time.time()
        df_ref = run_propagated(pop, market_data, excluded_map, ceiling, seq, config,
                                 bb_threshold=float("inf"), use_any_rr=False, **common_kwargs)
        row = summarize(df_ref, "REF (rr135corr080 seul)", ceiling, None, False)
        rows.append(row)
        ...
        t0 = time.time()
        df_combined = run_propagated(pop, market_data, excluded_map, ceiling, seq, config,
                                      bb_threshold=bb_th, use_any_rr=True, apply_instant_risk_cap=False,
                                      **common_kwargs)
        row = summarize(df_combined, f"COMBINE_original (bb_th={bb_th:.0f} + any-RR, risque NON corrige)", ceiling, bb_th, True)
        rows.append(row)
        print(f"[COMBINE_original bb={bb_th:.0f} plafond={ceiling:.0f}$] profit_moy={row['profit_moyen']:+,.0f}$ "
              f"solde_neg={row['solde_negatif_annee4']:.2f}% hit_ceiling={row['hit_ceiling_pct']:.2f}% "
              f"annee1<0={row['annee1_neg']:.2f}% instant_trades={row['instant_trades_total']:.0f} "
              f"instant_capped={row['instant_risk_capped_pct']:.1f}% ({time.time()-t0:.0f}s)")

        t0 = time.time()
        df_corrected = run_propagated(pop, market_data, excluded_map, ceiling, seq, config,
                                       bb_threshold=bb_th, use_any_rr=True, apply_instant_risk_cap=True,
                                       **common_kwargs)
        row = summarize(df_corrected, f"COMBINE_corrige (bb_th={bb_th:.0f} + any-RR, cap 1,5% Instant)", ceiling, bb_th, True)
        rows.append(row)
        print(f"[COMBINE_corrige bb={bb_th:.0f} plafond={ceiling:.0f}$] profit_moy={row['profit_moyen']:+,.0f}$ "
              f"solde_neg={row['solde_negatif_annee4']:.2f}% hit_ceiling={row['hit_ceiling_pct']:.2f}% "
              f"annee1<0={row['annee1_neg']:.2f}% instant_trades={row['instant_trades_total']:.0f} "
              f"instant_capped={row['instant_risk_capped_pct']:.1f}% ({time.time()-t0:.0f}s)")

        pd.DataFrame(rows).to_csv(f"chantier_S1_8_regen_population_n{n_sims}_{out_suffix}_2026-08-19.csv", index=False)
