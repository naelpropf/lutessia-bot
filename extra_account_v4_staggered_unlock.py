"""
Etape 3b de la demande (08/08, suite) : etalement leger du deblocage groupe
-- au lieu d'ouvrir simultanement les ~8-9 comptes restants (FTMO/Fivers/
GFT/FundedNext) des que la reserve atteint 30k$, les etaler d'un delai
calendaire fixe entre chaque firm (1/2/4 semaines testees), sans condition
de reserve supplementaire par etape. Cible la categorie A (flotte debloquee
mais plusieurs comptes finances simultanement, aucun compounding accumule
au moment ou la variance frappe).

Le declenchement (group_funded_count>=1 ET reserve>=30k) reste inchange et
n'arme qu'UNE fois une sequence d'ouvertures programmees a now, now+delai,
now+2*delai, now+3*delai (ordre FTMO->Fivers->GFT->FundedNext). Chaque
ouverture paye son propre cout (handle_cost_hybrid, comme avant). fleet_unlocked
(downgrade-on-reopen Blueberry + eligibilite mecanisme "compte
supplementaire") ne passe True qu'a la DERNIERE ouverture programmee
(FundedNext) -- comportement inchange pour tout le reste du moteur par
rapport a un deblocage simultane une fois la sequence terminee.
"""
import random
import time
...
STAGGER_ORDER = ("FTMO", "Fivers", "GFT", "FundedNext")
SEQ_GROUPED_STATIC = [((STARTER,), "day0"), (STAGGER_ORDER, ("after_count", 1))]
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, min_reserve_for_unlock,
            ...
            reserve_share, extra_threshold_mult, stagger_seconds):
    accounts_by_group = {}
    active0_cost = 0.0
    for group_names, trigger in SEQ_GROUPED_STATIC:
    ...
    pending_group_trigger = [(names, trig) for names, trig in SEQ_GROUPED_STATIC if trig != "day0"]
    stagger_armed = False
    pending_reopen = []
    pending_group_open = []
    scheduled_opens = []  # (time, gname, is_last)
    ...
    for slot_idx, trade_idx in enumerate(order):
            ...
            for group_names, trig in pending_group_trigger:
                ...
                if state["group_funded_count"] >= n_req and state["reserve"] >= min_reserve_for_unlock:
                    for i, gname in enumerate(group_names):
                        scheduled_opens.append((now + i * stagger_seconds, gname, i == len(group_names) - 1))
                    stagger_armed = True
                else:
        ...
        still_scheduled = []
        for t_open, gname, is_last in scheduled_opens:
            if now >= t_open:
                cost0 = sum(a["cost"] for a in accounts_by_group[gname])
                handle_cost_hybrid(cost0, pending_group_open, gname, lambda g=gname, f=is_last: open_group(g, f))
            else:
                still_scheduled.append((t_open, gname, is_last))
        scheduled_opens = still_scheduled
...
def run_propagated(pop, market_data, excluded_map, ceiling, min_reserve, emergency, target_risk_ov, eval_risk_ov,
                    gft_eval_risk_ov, reserve_share, extra_threshold_mult, stagger_seconds, n_sims, seed):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        weeks = [float(x) for x in sys.argv[2].split(",")]
    else:
        weeks = [1, 2, 4]
    configs = [("baseline (deblocage simultane)", 0.0)] + \
              [(f"etale {w} semaine(s) entre firms", w * WEEK_SECONDS) for w in weeks]

    rows = []
    for ceiling in (1000.0, 3000.0):
        print(f"\n{'='*100}\nPlafond {ceiling:.0f}$ (n={n_sims})\n{'='*100}")
        for label, stagger_seconds in configs:
            t0 = time.time()
            df = run_propagated(pop, market_data, excluded_map, ceiling, DEFAULT_RESERVE, DEFAULT_EMERGENCY,
                                 ...
                                 EXTRA_THRESHOLD_MULT, stagger_seconds, n_sims, seed=4000)
        ...
        pd.DataFrame(rows).to_csv("extra_account_v4_staggered_unlock.csv", index=False)
