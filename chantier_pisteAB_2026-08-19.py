...
# <<< CORRECTIF RISQUE INSTANT (regen 2026-08-19, meme raison/ordre que
# chantier_rrtp2_sizing_2026-08-19.py) : cap reel 1,5%/trade sur Blueberry
# Instant Elite/Lite, applique APRES tout autre multiplicateur (size_func
# rr_tp2 inclus) -- toujours actif.
...
if __name__ == "__main__":
    ...
    out_suffix = sys.argv[3] if len(sys.argv) > 3 else "all"
    ...
    CONFIGS = {
        "REF_travail (S1.8+S2.35)": None,
        "PisteA_BBx2": "Blueberry",
        "PisteB_BBplusGFT_day0": "GFT",
    }

    rows = []
    for ceiling in ceilings_arg:
        bb_th = BB_THRESHOLD_BY_CEILING[ceiling]
        for name, extra_starter in CONFIGS.items():
            t0 = time.time()
            ...
            pd.DataFrame(rows).to_csv(f"chantier_pisteAB_n{n_sims}_{out_suffix}_2026-08-19.csv", index=False)
