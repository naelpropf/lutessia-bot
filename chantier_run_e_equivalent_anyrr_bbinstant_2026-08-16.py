...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
    ...
    state = {"reserve": 0.0}  # <<< CHANTIER : existe AVANT la creation des comptes (bb_choose_fmt_key le lit)
    ...
    for gname in FIRMS:
        ...
        if gname == "Blueberry":
            # <<< CHANTIER : format Blueberry decide a la creation (jamais
            # reconsidere ensuite -- Run E n'a aucun reopen).
        ...
        for a in accs:
            a["_gname"] = gname
            a["_fmt_key"] = fmt_key if gname == "Blueberry" else format_by_firm[gname]
    ...
    for slot_idx, trade_idx in enumerate(order):
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                if not acc["active"]:
                    continue
                fmt = FORMATS[acc["_fmt_key"]]  # <<< CHANTIER : format PAR COMPTE (Blueberry peut varier)
...
def run_propagated(pop, market_data, excluded_map, ceiling, seq_grouped, format_by_firm, emergency,
    ...
    for _ in range(n_sims):
        ...
        res = run_one(raw_trades, raw_slots, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
                      ...
                      use_any_rr=use_any_rr, bb_threshold=bb_threshold)
        rows.append(res)
...
if __name__ == "__main__":
    ...
    for ceiling in ceilings_arg:
        t0 = time.time()
        df = run_propagated(pop, market_data, excluded_map, ceiling, seq, config, use_any_rr=True,
                             bb_threshold=0.0, **common_kwargs)
        row = summarize(df, "RUN_E_equiv_anyRR_bbInstant0_2026-08-16", ceiling)
        rows.append(row)
        ...
        pd.DataFrame(rows).to_csv(f"chantier_run_e_equivalent_anyrr_bbinstant_n{n_sims}_2026-08-16.csv", index=False)
