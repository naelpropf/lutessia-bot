"""
Partie 1 - Etape 4 (2026-08-17) : chiffrage flotte du seul candidat ayant
passe le garde-fou out-of-sample (`chantier_segmentation_variables_2026-
08-17.py`) -- distance_SL% = (entree-SL)/entree*100, segment > 0,235%
(quintiles 4-5, ~40% de la population). SOUS la moyenne locale dans 6/6
sous-periodes (H1/H2 + 4 blocs k-fold), n=252, Pearson quasi nul (-0,06,
donc pas une relation lineaire simple mais un effet de seuil stable).
Quasi independant de rr_tp2 (Pearson=-0,15, chevauchement 16/252 avec le
segment deja boosted rr_tp2>=8). Winrate segment 36,5% vs 41,4% reste.

Boost rr_tp2>=8 -> x1,6 (deja adopte S2.35) MAINTENU, compose
multiplicativement avec le downsizing distance_SL (x0,5 / x0,8) sur son
segment -- les deux mecanismes se chevauchent tres peu (16/631 trades).
size_func recoit maintenant le dict `trade` complet (pas juste rr_tp2)
pour lire les deux variables.
"""
import random
import sys
import time
...
def make_size_func_tail(mult, threshold=8.0):
    """REF S2.35 : boost seul, sur trade dict (lit rr_tp2)."""
    def f(trade):
        return mult if trade["rr_tp2"] >= threshold else 1.0
    return f


DIST_SL_THRESHOLD = 0.235  # quintiles 4-5 combines, sous IC95% bas 6/6 sous-periodes


def make_size_func_distsl_downsize(downsize_mult, tail_mult=1.6, tail_th=8.0, dist_th=DIST_SL_THRESHOLD):
    """Boost rr_tp2>=tail_th (S2.35, inchange) COMPOSE multiplicativement avec
    downsizing sur distance_sl_pct>dist_th (chevauchement 16/631 trades)."""
    def f(trade):
        mult = tail_mult if trade["rr_tp2"] >= tail_th else 1.0
        if trade["distance_sl_pct"] > dist_th:
            mult *= downsize_mult
        return mult
    return f
...
def build_flexible_population_with_rr(pop, target_winrate, rr_stress_factor, use_slippage, rng):
    ...
    dist_sl = (sub["prix_entree"] - sub["stop_loss_init"]).abs() / sub["prix_entree"] * 100
    for t, rr1, rr2, dsl in zip(trades, sub["rr_tp1"], sub["rr_tp2"], dist_sl):
        t["rr_tp1"] = float(rr1)
        t["rr_tp2"] = float(rr2)
        t["distance_sl_pct"] = float(dsl)
...
if __name__ == "__main__":
    ...
    CANDIDATES = {
        ...
        "DistSL_downsize_x0.5": make_size_func_distsl_downsize(0.5),
        "DistSL_downsize_x0.8": make_size_func_distsl_downsize(0.8),
    }
    for name, f in CANDIDATES.items():
        probe_trades = [dict(rr_tp2=rr, distance_sl_pct=dsl)
                         for rr in [0, 8, 8.82, 30] for dsl in [0.1, 0.235, 0.5, 1.77]]
        maxm = max(f(t) for t in probe_trades)
        minm = min(f(t) for t in probe_trades)
        print(f"[verif marge] {name} : mult_max={maxm:.2f} -> marge={margin_pct(maxm):.1f}%"
              f"{'  [OK >=30%]' if margin_pct(maxm) >= 30 else '  [!!! <30%, deja accepte pour x1.6 en S2.35]'}"
              f"  mult_min={minm:.2f} (downsizing/compose, ne peut pas depasser le max deja accepte)")
    ...
    for ceiling in ceilings_arg:
        ...
        for name, size_func in CANDIDATES.items():
            ...
            pd.DataFrame(rows).to_csv(f"chantier_segmentation_fleet_test_n{n_sims}_2026-08-17.csv", index=False)
