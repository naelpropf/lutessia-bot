"""
Chantier 2, suite (08/09) : sous-balayage de la fenetre "eval synthetique"
(variante a) trouvee la plus efficace pour Blueberry-adaptatif (ruine
14,00%->5,67% au plafond 1000$ avec fenetre 20 trades/4 semaines, cf.
etape_e_bb_instant_protection_2026-08-09.py) mais qui ne fermait pas
completement l'ecart avec REF pur (ruine encore >REF pur aux deux
plafonds). Teste des fenetres plus longues : 30/40 trades x 6/8 semaines
(grille 2x2), au seuil de bascule 10k$ (10k et 20k$ donnaient des
resultats identiques dans le criblage precedent -- le seuil ne mord pas
en dessous de ~10k, aucune raison de re-tester les deux valeurs).

Copie de etape_e_bb_instant_protection_2026-08-09.py, avec la fenetre
parametree au lieu de constantes de module fixes. N'importe pas ce script
directement.
"""
import random
import sys
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped,
            ...
            blueberry_adaptive, switch_threshold, protection_variant, n_trades_protect, w_weeks_protect):
...
if __name__ == "__main__":
    ...
    THRESH = 10000.0  # 10k et 20k$ identiques au criblage precedent, un seul teste ici
    WINDOWS = [(30, 6), (30, 8), (40, 6), (40, 8)]
    # REF pur + reset Blueberry (cible de domination stricte)
    REF_PUR = {1000.0: dict(profit=4827736.0, ruine=1.67, annee1=20.83),
               3000.0: dict(profit=4892588.0, ruine=0.50, annee1=20.33)}

    rows = []
    for n_tr, w_wk in WINDOWS:
        label = f"fenetre={n_tr}tr/{w_wk}sem"
        for ceiling in (1000.0, 3000.0):
            ...
            df = run_propagated(pop, market_data, excluded_map, ceiling, seq, ei.DEFAULT_EMERGENCY,
                                 ...
                                 ei.EXTRA_THRESHOLD_MULT, n_sims=n_sims, seed=14000,
                                 blueberry_adaptive=True, switch_threshold=THRESH, protection_variant="a",
                                 n_trades_protect=n_tr, w_weeks_protect=w_wk)
            row = summarize(df, label)
            row["ceiling"] = ceiling
            row["n_trades_protect"] = n_tr
            row["w_weeks_protect"] = w_wk
            ref = REF_PUR[ceiling]
            row["domine_ref_pur"] = (row["ruine"] <= ref["ruine"] and row["profit"] >= ref["profit"]
                                      and row["annee1_neg"] <= ref["annee1"])
            rows.append(row)
            print(f"[{label} plafond={ceiling:.0f}$] profit={row['profit']:+,.0f}$ ruine={row['ruine']:.2f}% "
                  ...
                  f"domine_ref_pur={row['domine_ref_pur']} ({time.time()-t0:.0f}s)")
            pd.DataFrame(rows).to_csv("etape_e_bb_window_subsweep_screen.csv", index=False)
