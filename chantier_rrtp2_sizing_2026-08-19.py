"""
Regeneration 2026-08-19 -- copie de chantier_rrtp2_sizing_2026-08-16.py
(§2.35 du registre), sous la pile de parametres actuelle. Le script original
etait construit sur DEUX premisses depuis corrigees, ni l'une ni l'autre
appliquee ici a l'origine :
1. Cap de risque Instant (registre_strategie_trading.md S2.45, 08/17-18) :
   Blueberry Instant Elite/Lite plafonnes a 1,5%/trade reel, pas 1,90%.
   AJOUTE ICI (bloc <<< CORRECTIF RISQUE INSTANT), identique a
   chantier_S1_8_officiel_n600_risque_corrige_2026-08-17.py:379-481 --
   applique EN DERNIER (apres size_func rr_tp2, qui peut lui-meme
   multiplier x1,6), car c'est une contrainte broker reelle qu'aucun
   levier de sizing ne peut depasser.
2. Population elargie 631->742 trades (bug forex-only, registre_parametres_
   projet.md S1.8bis, 08/18) : deja heritee automatiquement via
   build_population_with_trailing -> ... -> rr_threshold_test.
   build_extended_population (correctif dans le code partage). Consequence
   directe non geree par le script original : feasible_risk_pct() plante
   (KeyError) sur les tickers indices, forex_market_data.json ne couvrant
   que le forex -- corrige par build_market_data_with_indices(), reprise a
   l'identique du precedent deja etabli dans chantier_strategie_b_isolation_
   indices_2026-08-18.py:802-819.

Section D 2026-08-16 (texte original) : premier levier FACTORIEL -- sizing
module par le RR PLANIFIE du signal (rr_tp1, connu au moment du trade),
par-dessus la cascade groupee deja integree (bascule Blueberry Instant +
any-RR, S1.8).
...
# <<< CORRECTIF RISQUE INSTANT (regen 2026-08-19, identique a chantier_S1_8_
# officiel_n600_risque_corrige_2026-08-17.py:69-73) : cap reel 1,5%/trade sur
# Blueberry Instant Elite/Lite, applique APRES tout autre multiplicateur (DD-
# distance V2 ET size_func rr_tp2 inclus) -- toujours actif, plus un flag
# optionnel (c'est desormais juste le comportement correct).
    ...
    for slot_idx, trade_idx in enumerate(order):
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                # EN DERNIER (apres size_func inclus, cf. docstring de tete).
...
def summarize(df, label, ceiling, bb_threshold, use_any_rr):
    ...
    return dict(config=label, ceiling=ceiling, bb_threshold=bb_threshold, use_any_rr=use_any_rr, n=len(df),
                ...
                instant_trades_total=int(df["instant_trades_total"].sum()),
                instant_risk_capped_pct=(df["instant_risk_capped_trades"].sum() / df["instant_trades_total"].sum() * 100
                                          if df["instant_trades_total"].sum() > 0 else float("nan")))


def build_market_data_with_indices():
    """<<< CORRECTIF 2026-08-19 : identique a chantier_strategie_b_isolation_
    indices_2026-08-18.py:802-819 / chantier_S1_8_regen_population_2026-08-19.py
    -- feasible_risk_pct() plante sur les 5 labels index de la population
    elargie (742 trades), aucune spec broker reelle disponible. Contrainte de
    capacite desactivee pour ces labels (l'economie du trade vient des
    donnees historiques reelles, inchangee)."""
...
if __name__ == "__main__":
    # <<< Regen 2026-08-19 : par defaut, variante B x1.6 (le candidat deja
    # etabli/adopte en S2.35, PAS un nouveau balayage de mecanisme -- seul le
    # pipeline (cap Instant + population 742) change). ceilings_arg accepte 1
    # seul plafond pour permettre l'execution parallele par plafond.
    ...
    variant_letter = sys.argv[3] if len(sys.argv) > 3 else "B"
    mults_arg = [float(x) for x in sys.argv[4].split(",")] if len(sys.argv) > 4 else [1.6]
    out_suffix = sys.argv[5] if len(sys.argv) > 5 else "all"
    ...
    for ceiling in ceilings_arg:
        ...
        for mult in mults_arg:
            ...
            pd.DataFrame(rows).to_csv(f"chantier_rrtp2_sizing_{variant_letter}_n{n_sims}_{out_suffix}_2026-08-19.csv", index=False)
