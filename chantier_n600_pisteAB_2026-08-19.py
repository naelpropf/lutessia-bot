        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                if gname == "Blueberry" and acc["_fmt_key"] in BB_INSTANT_FORMAT_KEYS:
                    state["instant_trades_total"] = state.get("instant_trades_total", 0) + 1
                    if r > BB_INSTANT_RISK_CAP:
                        r = BB_INSTANT_RISK_CAP
                        state["instant_risk_capped_trades"] = state.get("instant_risk_capped_trades", 0) + 1
...
if __name__ == "__main__":
    # <<< Regen 2026-08-19 : confirmation n=600+cascade sous le cap Instant
    # 1,5% + population 742 trades (les deux herites via load_common/run_one
    # ci-dessus), APRES stress-test H1/H2+4blocs (chantier_stresstest_
    # pisteAB_2026-08-19.py). Seed=9999 comme toute la session.
    ...
    RUNS = [
        ("REF@3000", None, 3000.0),
        ("REF@5000", None, 5000.0),
    ...
    rows = []
    for label, extra_starter, ceiling in RUNS:
        ...
        rows.append(row)
        print(f"[{label}] profit_moy={row['profit_moyen']:+,.0f}$ profit_med={row['profit_median']:+,.0f}$ "
              f"solde_neg={row['solde_negatif_annee4']:.2f}% hit_ceiling={row['hit_ceiling_pct']:.2f}% "
              f"annee1<0={row['annee1_neg']:.2f}% ({time.time()-t0:.0f}s)")
        pd.DataFrame(rows).to_csv(f"chantier_n600_pisteAB_{out_suffix}_2026-08-19.csv", index=False)
