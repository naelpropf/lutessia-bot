...
# <<< TACHE D (2026-08-23) : process_trade_corr_swap_rr/build_flexible_
# population_with_rr de s18 n'attachent/ne routent QUE rr_tp1 (pas de
# routing_field) -- S2.35 (adopte) a besoin de rr_tp2 + routing_field.
# Remplace par les versions de chantier_rrtp2_sizing_2026-08-19.py (rr2),
# sur-ensemble strict (meme signature + rr_tp2/routing_field en plus).
_spec_rr2 = importlib.util.spec_from_file_location("rr2", "chantier_rrtp2_sizing_2026-08-19.py")
rr2 = importlib.util.module_from_spec(_spec_rr2)
_spec_rr2.loader.exec_module(rr2)
process_trade_corr_swap_rr = rr2.process_trade_corr_swap_rr
build_flexible_population_with_rr = rr2.build_flexible_population_with_rr
...
# <<< TACHE D (2026-08-23) : risque optimal DIFFERENT par cote, trouve en
# Tache C2 (n=600 verdict, chantier_taskC2_risque_sweep_AB_2026-08-23.py) --
# A garde la reference 1,25%/1,90% (non battue), B passe a 1,50%/1,50%
# uniforme (+4,2-4,5% profit vs reference, risque identique). GFT_EVAL_RISK
# inchange (non teste separement en C2), commun aux deux cotes.
RISK_BY_TID = {"A": (1.25, 1.90), "B": (1.50, 1.50)}
GFT_EVAL_RISK = 1.75
...
# <<< TACHE D (2026-08-23) : prior remplace par celui de B_tradable_pgp
# courante (n=1248, cf. point_d_bloc1_bloc2_2026-08-22.py::ALPHA_POST_B_PGP/
# BETA_POST_B_PGP=625,625) -- l'ancien 762/745 etait calibre sur B_config0
# (n=1051, sans Palladium/Platinum, pre-fix), plus la population utilisee ici.
ALPHA_POST_B_METAUX, BETA_POST_B_METAUX = 625, 625
...
def run_dual_ab(trades_A, slots_A, trades_B, slots_B, market_data, excluded_map,
    ...
    def process_one_account(tid, gname, acc, fmt, trade, now):
        ...
        eval_risk_t, fleet_risk_t = RISK_BY_TID[tid]
        base_risk = GFT_EVAL_RISK if gname == "GFT" else eval_risk_t
        base_r = fleet_risk_t if acc["phase"] == "funded" else base_risk
        pdef = _current_phase(fmt, acc)
        r = effective_risk(acc, pdef, base_r, s["fleet_unlocked"])
        # <<< TACHE D : S2.35 (adopte, cf. Tache C/C2) -- sizing multiplie par
        # size_func(rr_tp2), EN DERNIER avant le cap Blueberry Instant, meme
        # ordre que chantier_rrtp2_sizing_2026-08-19.py:530-538 (run_one).
        ...
        just_funded = process_trade_corr_swap_rr(acc, trade, now, fmt, proxy, r, market_data, excluded_map,
                                                   ...
                                                   cost_override=0.0, routing_field="rr_tp2")
...
def build_pop_B():
    # <<< TACHE D (2026-08-23) : population B_tradable_pgp courante (n=1248,
    # metaux+forex/indices corriges), remplace l'ancienne Config0 (n=1051,
    # sans Palladium/Platinum, pre-fix r_trailing).
...
def build_market_data_and_excluded_map(pop_A, pop_B):
    ...
    # <<< TACHE D : PALLADIUM/PLATINUM (B_tradable_pgp courante) absents de
    # GOLD_SILVER_LABELS et de build_market_data_with_indices() -- meme
    # traitement "unconstrained" que point_d_bloc1_bloc2_2026-08-22.py::
    # load_scenario_pgp(). Les 3 labels index (DAX40/NASDAQ100/S&P500) sont
    # deja couverts par build_market_data_with_indices() (load_common_A).
...
def run_n_sims(pop_A, pop_B, ceiling, n_sims, seed, include_B, market_data, excluded_map, metal_set,
    ...
    meme mecanisme que chantier_coldstart_svb_isrhamas_2026-08-23.py -- force
    le TOUT PREMIER bloc de B a etre les trades reels de la fenetre choc,
    A garde son tirage articuli normal sur ce meme premier cran de curseur)
    -- PRESERVE le couplage calendaire joint A/B (meme idx tire pour
    blocks_A[idx]/blocks_B[idx] a chaque pas, cf. corr_PnL_A_B) partout
    ailleurs que sur ce premier cran force."""
    ...
    for _ in range(n_sims):
        ...
        if include_B:
            wr_B = winrate_override_B if winrate_override_B is not None else rng_wr_B.betavariate(ALPHA_POST_B_METAUX, BETA_POST_B_METAUX)
            trades_B, slots_B = build_flexible_population_with_rr(pop_B, wr_B, 1.0, False, random.Random(rng_boot_B.random()))
            if ev_scale_B is not None:
                for t in trades_B:
                    if t["outcome_r"] > 0:
                        t["outcome_r"] = t["outcome_r"] * ev_scale_B
            slots_B_abs = [s + offset_B for s in slots_B]
            forced_B_idxs = None
            if forced_window_B is not None:
                sub_B_sorted = pop_B.sort_values("date_creation").reset_index(drop=True)
                start_ts, end_ts = forced_window_B
                mask = (sub_B_sorted["date_creation"] >= start_ts) & (sub_B_sorted["date_creation"] < end_ts)
                forced_B_idxs = [i for i, keep in enumerate(mask.to_numpy()) if keep]
                assert len(forced_B_idxs) > 0, f"fenetre forcee vide : {forced_window_B}"
        else:
        ...
        while cursor < target_duration:
            ...
            if first_iter and forced_B_idxs is not None:
                # <<< TACHE D : 1er cran de curseur seulement -- B force sur
                # la fenetre choc reelle (trades intacts, offsets re-ancres a
                # cursor=0), A garde son tirage normal blocks_A[idx] ci-dessus.
                base_slot = min(slots_B_abs[i] for i in forced_B_idxs) - offset_B
                for i in forced_B_idxs:
                    raw_B_t.append(trades_B[i]); raw_B_s.append(cursor + (slots_B_abs[i] - offset_B) - base_slot)
            else:
