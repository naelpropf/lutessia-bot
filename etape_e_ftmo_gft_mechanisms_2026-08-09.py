"""
Chantier 2, tache 2 (08/09) : integration de deux mecanismes reels trouves
a l'Etape A (recherche officielle) mais jamais codes dans aucun script :

  (a) FTMO -10% SUR RACHAT <24H APRES CASSE (eval ou finance) -- confiance
      elevee, source multiple convergente. Verifie ici (comme pour la
      fenetre 7 jours du reset Blueberry) : le moteur ne modelise AUCUN
      delai entre casse et rachat -- handle_cost_hybrid() rouvre des que
      la tresorerie le permet, jamais avec un delai calendaire. Le rabais
      -10% s'applique donc SYSTEMATIQUEMENT a CHAQUE restart FTMO, sans
      condition de delai (puisque le modele n'a jamais de delai > 0 de
      toute facon). Implementation directe : cout de reouverture FTMO
      x0.90, aucun autre changement de comportement.

  (b) GFT "GOAT GUARD" -- coupe-circuit qui ferme les positions a -2% de
      perte flottante sur un compte GFT FINANCE, transformant une 1ere
      casse potentielle en "soft breach" (split reduit a 50%, compte
      reste ouvert) au lieu d'une casse dure ; la 2e occurrence devient
      une vraie casse.

      LIMITE DE MODELISATION A SIGNALER EXPLICITEMENT (pas une simple
      "confiance moyenne", une vraie limite de donnees) : ce moteur ne
      modelise PAS de courbe d'equite flottante intra-trade -- chaque
      trade se resout directement a son outcome_r final a la cloture,
      sans etape intermediaire observable. Impossible de detecter
      litteralement "le flottant a touche -2%" avant la cloture d'un
      trade. Approximation retenue : le trade qui ferait normalement
      franchir le DD max sert de PROXY pour "le flottant a touche -2%"
      (le vrai declencheur, -2%, est bien plus serre que le DD max du
      format GFT 2-Step finance, 6-10% selon le palier -- en toute
      rigueur Goat Guard se declencherait BEAUCOUP plus tot et plus
      souvent dans la realite). Cette approximation SOUS-ESTIME donc la
      frequence/le benefice reel du mecanisme -- biais connu, documente,
      dans le sens conservateur (ne surestime pas le gain).
      Duree de la penalite de split (50%) APRES declenchement : NON
      documentee dans la source (qui confirme seulement la consequence
      "1ere casse -> split 50%, 2e casse -> casse reelle", pas combien de
      temps le split reduit dure). Duree choisie ici, non sourcee :
      30 jours (~1 mois), un ordre de grandeur raisonnable compte tenu
      des fenetres deja utilisees ailleurs dans le projet (rampes,
      protections), a ajuster si une source precise est trouvee plus
      tard.
      Le compteur de declenchement (_gg_triggered_count) est reinitialise
      a chaque REOUVERTURE du compte (nouvelle vie financee = nouveau
      "essai gratuit") -- choix de modelisation raisonnable mais non
      confirme par la source, qui ne precise pas si le compteur est
      a vie ou par cycle de financement.

Copie de etape_e_final_lock_bbreset_2026-08-09.run_one, augmentee des 2
mecanismes (parametres ftmo_discount / gft_goat_guard, combinables).
N'importe pas ce script directement.
"""
import random
import sys
import time
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
            ...
            ftmo_discount=False, gft_goat_guard=False):
...
def run_propagated(pop, market_data, excluded_map, ceiling, seq_grouped, format_by_firm, emergency,
    ...
    for _ in range(n_sims):
        ...
        res = run_one(raw_trades, raw_slots, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
                      ...
                      ftmo_discount=ftmo_discount, gft_goat_guard=gft_goat_guard)
        rows.append(res)
...
def summarize(df, label):
    ...
    return dict(config=label, n=len(df), profit=net.mean(), ruine=(net < 0).mean() * 100,
                ...
                mean_gft_soft_breaches=df["gft_soft_breaches"].mean(),
...
if __name__ == "__main__":
    ...
    configs = [
        ("baseline (REF pur + reset BB)", False, False),
        ("(a) FTMO -10% seul", True, False),
        ("(b) GFT Goat Guard seul", False, True),
        ("(c) combine (a)+(b)", True, True),
    ]

    rows = []
    for label, ftmo_disc, gg in configs:
        for ceiling in (1000.0, 3000.0):
            ...
            df = run_propagated(pop, market_data, excluded_map, ceiling, seq, config, ei.DEFAULT_EMERGENCY,
                                 ...
                                 ftmo_discount=ftmo_disc, gft_goat_guard=gg)
            ...
            print(f"[{label:28s} plafond={ceiling:.0f}$] profit={row['profit']:+,.0f}$ ruine={row['ruine']:.2f}% "
                  ...
                  f"bb_resets={row['mean_bb_resets']:.2f} gft_soft={row['mean_gft_soft_breaches']:.2f} "
                  f"casse<=30j={row['break_rate_30d_pct']:.2f}% quasi_gele={row['quasi_frozen_pct']:.1f}% "
                  f"({time.time()-t0:.0f}s)")
            pd.DataFrame(rows).to_csv(f"etape_e_ftmo_gft_mechanisms_n{n_sims}.csv", index=False)
