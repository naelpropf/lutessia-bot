"""
Partie 2, PISTE C (2026-08-17) : re-test du "fonds d'urgence reactif"
(ancien S2.39 `registre_parametres_projet.md`) sous la pile actuelle.

Mecanisme original (reconstruit a l'identique) : une fraction
`p1_emergency_pct` de CHAQUE credit positif net vers `state["reserve"]`
(profit funded reverse a la reserve, ET liberation de payout_cycle) est
deviee vers un bucket verrouille `state["reserve_emergency"]`, deverse
d'un coup dans la reserve normale des que `p1_break_trigger` casses
surviennent dans une fenetre glissante `p1_window_days`. Sous l'ANCIENNE
pile (08/11) : REJETE a 1000$ (20% fait exploser hit_ceiling x2,2-x2,9),
**candidat n=300 SEULEMENT a 3000$** avec la meilleure config 10%/7j/N2
(profit quasi gratuit +0,03%, solde_neg et hit_ceiling divises par 2-2,5)
-- jamais confirme n=600, jamais adopte. Config testee ici : 10%/7j/N2
(la meilleure identifiee a l'epoque).
"""
import random
import sys
import time
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
            ...
            p1_emergency_pct=0.0, p1_break_trigger=2, p1_window_days=7.0):
    ...
    state.update({"ever_funded": False, "real_cash_paid": active0_cost, "total_breaks": 0,
             ...
             "reserve_emergency": 0.0, "recent_break_times": []})
    ...
    for slot_idx, trade_idx in enumerate(order):
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                reserve_before_trade = state["reserve"]
                if use_any_rr:
                ...
                # <<< PISTE C (fonds d'urgence, 2026-08-17) : devie une
                # fraction de CHAQUE credit positif net a state["reserve"]
                # (sur cette passe trade/compte) vers un bucket verrouille.
                if p1_emergency_pct > 0:
                    net_delta = state["reserve"] - reserve_before_trade
                    if net_delta > 0:
                        divert = net_delta * p1_emergency_pct
                        state["reserve"] -= divert
                        state["reserve_emergency"] += divert
                ...
                if use_goat_guard:
                    ...
                    if p1_emergency_pct > 0:
                        ...
                        window_s = p1_window_days * 86400
                        state["recent_break_times"] = [t for t in state["recent_break_times"] if now - t <= window_s]
                        if len(state["recent_break_times"]) >= p1_break_trigger:
                            state["reserve"] += state["reserve_emergency"]
                            state["reserve_emergency"] = 0.0
                            state["recent_break_times"] = []
...
def run_propagated(pop, market_data, excluded_map, ceiling, seq_grouped, format_by_firm, emergency,
    ...
    for _ in range(n_sims):
        ...
        res = run_one(raw_trades, raw_slots, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
                      ...
                      p1_emergency_pct=p1_emergency_pct, p1_break_trigger=p1_break_trigger, p1_window_days=p1_window_days)
        rows.append(res)
...
if __name__ == "__main__":
    ...
    CONFIGS = {
        "REF_travail (S1.8+S2.35)": dict(p1_emergency_pct=0.0),
        "PisteC_10pct_7j_N2": dict(p1_emergency_pct=0.10, p1_break_trigger=2, p1_window_days=7.0),
    }
    ...
    for ceiling in ceilings_arg:
        ...
        for name, kw in CONFIGS.items():
            ...
            pd.DataFrame(rows).to_csv(f"chantier_pisteC_n{n_sims}_2026-08-17.csv", index=False)
