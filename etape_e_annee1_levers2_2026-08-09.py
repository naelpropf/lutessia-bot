"""
Etape E, annee1<0, round 2 (08/09) -- suite de etape_e_annee1_levers_2026-08-09.py
(4 leviers deja epuises : seuil FTMO, etalement calendaire, rampe classique/
ciblee -- aucun n'a donne de gain net). Cette session cherche 2 NOUVEAUX
degres de liberte jamais testes, plus une verification a cout nul sur des
resultats deja produits :

  POINT 1 (verification, pas de nouvelle simu) : p10/p25 du profit par run
  pour les points de risque du dernier resweep complet -- verifie si les
  CSV existants contiennent les runs individuels ou seulement des agregats.

  POINT 2 (comptes SUPPLEMENTAIRES, redemarrage asymetrique) : actuellement
  un compte supplementaire (Blueberry/FTMO/GFT, cf. GROWTH_FIRMS_EXTRA) qui
  casse une fois finance se rouvre a SA PROPRE taille (EXTRA_UNIT_PALIER =
  BASE_PALIER x2, fixe a la creation, aucune croissance individuelle par
  design -- cf. etape_e_fleet_integration.py note de conception #1/#2).
  Nouveau levier : a la casse d'un compte supplementaire DEJA FINANCE, le
  rouvrir au plus petit palier viable de la firm (BASE_PALIER[gname], la
  moitie du palier extra standard) plutot qu'a sa taille d'origine -- cout
  de reouverture reduit, laisse le mecanisme d'ouverture de nouveaux
  comptes supplementaires (deja existant, inchange) regonfler le capital
  total plus tard plutot que de reconstituer CE compte precis a sa taille
  d'origine immediatement.

  POINT 3 (risque degressif en fin de phase d'evaluation) : reduit le
  risque par trade quand un compte en phase "challenge" approche sa cible
  de palier (derniers 10%/20% du chemin restant vers la cible), pour
  reduire le risque de casser tres pres du but -- s'applique a TOUTE
  entree en phase challenge, y compris les redemarrages apres casse d'un
  compte deja finance (le "cout du restart" identifie comme piste ouverte
  par le rapport precedent).
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
            ...
            downsize_extra_restart=False, degressive_window_frac=None, degressive_factor=0.5):
    ...
    def reopen_account(acc, cost, fmt, downsize_palier=None):
        acc["active"] = True
        ...
        elif downsize_palier is not None:
            acc["palier"] = downsize_palier
            acc["cost"] = cost
    ...
    def effective_risk_challenge(acc, fmt, base_risk):
        if degressive_window_frac is None or not fmt["phases"]:
            return base_risk
        pdef = fmt["phases"][acc["phase_index"]]
        if pdef["target_pct"] is None:
            return base_risk
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                if acc["phase"] == "funded":
                    ...
                    r = effective_risk_challenge(acc, fmt, base_risk)
                ...
                if broke:
                    ...
                    downsize_this = (downsize_extra_restart and acc.get("_is_extra") and was_funded
                                      and acc["_ever_funded_before"])
                    if downsize_this:
                        new_palier = BASE_PALIER[gname]
                        cost = ei.price_for(format_by_firm[gname], new_palier)
                    else:
                        new_palier = None
                    ...
                    handle_cost_hybrid(cost, pending_reopen, id(acc),
                                        lambda a=acc, c=cost, f=fmt, dp=new_palier: reopen_account(a, c, f, dp))
                else:
                    if acc["phase"] == "funded":
...
def run_propagated(pop, market_data, excluded_map, ceiling, seq_grouped, format_by_firm, emergency,
    ...
    for _ in range(n_sims):
        ...
        res = run_one(raw_trades, raw_slots, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
                      ...
                      downsize_extra_restart=downsize_extra_restart, degressive_window_frac=degressive_window_frac,
                      degressive_factor=degressive_factor)
        rows.append(res)
...
def summarize(df, label):
    ...
    return dict(config=label, n=len(df), profit=net.mean(), p25_profit=net.quantile(0.25),
                p10_profit=net.quantile(0.10), ruine=(net < 0).mean() * 100,
...
if __name__ == "__main__":
    ...
    if part in ("2", "all"):
        print(f"\n{'='*100}\nPOINT 2 -- REDEMARRAGE ASYMETRIQUE COMPTES SUPPLEMENTAIRES (n={n_sims})\n{'='*100}")
        rows = []
        for label, downsize in [("baseline (reouverture taille identique)", False),
                                 ("downsize (reouverture au palier de base)", True)]:
            t0 = time.time()
            df = run_propagated(pop, market_data, excluded_map, CEILING, seq, config, ei.DEFAULT_EMERGENCY,
                                 ...
                                 ei.EXTRA_THRESHOLD_MULT, n_sims=n_sims, seed=6000, downsize_extra_restart=downsize)
            ...
            print(f"[{label:42s}] profit={row['profit']:+,.0f}$ ruine={row['ruine']:.2f}% "
                  ...
                  f"post={row['annee1_neg_post_pct']:.2f}%) post_unlock_breaks={row['mean_post_unlock_breaks']:.1f} "
                  f"casse<=30j={row['break_rate_30d_pct']:.2f}% casse<=60j={row['break_rate_60d_pct']:.2f}% "
                  f"({time.time()-t0:.0f}s)")
            pd.DataFrame(rows).to_csv("etape_e_annee1_levers2_point2.csv", index=False)

    if part in ("3", "all"):
        print(f"\n{'='*100}\nPOINT 3 -- RISQUE DEGRESSIF FIN DE PHASE (n={n_sims})\n{'='*100}")
        rows = []
        variants = [("baseline (pas de degressif)", None, None)]
        for window in (0.10, 0.20):
            variants.append((f"window={window*100:.0f}% factor=0.5", window, 0.5))
        variants.append(("window=20% factor=0.7", 0.20, 0.7))
        for label, window, factor in variants:
            t0 = time.time()
            df = run_propagated(pop, market_data, excluded_map, CEILING, seq, config, ei.DEFAULT_EMERGENCY,
                                 EVAL_RISK, FLEET_RISK, GFT_EVAL_RISK, ei.FINAL_RESERVE_SHARE,
                                 ei.EXTRA_THRESHOLD_MULT, n_sims=n_sims, seed=6000,
                                 degressive_window_frac=window, degressive_factor=(factor if factor else 0.5))
            row = summarize(df, label)
            rows.append(row)
            print(f"[{label:28s}] profit={row['profit']:+,.0f}$ ruine={row['ruine']:.2f}% "
                  ...
                  f"({time.time()-t0:.0f}s)")
            pd.DataFrame(rows).to_csv("etape_e_annee1_levers2_point3.csv", index=False)
