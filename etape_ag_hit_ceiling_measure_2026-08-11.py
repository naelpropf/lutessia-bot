"""
Etape AG (08/11) : copie exacte de etape_q_v2_plus_ftmo_gft_2026-08-10.py
(moteur officiel REF+V2+FTMO-10%/GFT Goat Guard) -- SEUL ajout :
state["hit_ceiling"] est remonte dans le dict de resultat (il existait
deja dans le state mais n'etait jamais rapporte). Objectif : comparer le
VRAI taux d'epuisement de plafond (hit_ceiling==True).mean() au taux
actuellement affiche comme "ruine" (net<0 a l'annee 4) -- ce sont deux
concepts differents, cf. investigation demandee 08/11.
...
def summarize(df, label, ceiling):
    ...
    return dict(config=label, ceiling=ceiling, n=len(df), profit=net.mean(), ruine=(net < 0).mean() * 100,
                hit_ceiling_rate=df["hit_ceiling"].mean() * 100,
...
if __name__ == "__main__":
    ...
    label, ftmo_disc, gg = "REF_V2 + (a)+(b) combine", True, True
    ...
    for ceiling in ceilings_arg:
        ...
        both = (ruine_mask & hc_mask).sum()
        hc_not_ruine = (hc_mask & ~ruine_mask).sum()
        ruine_not_hc = (ruine_mask & ~hc_mask).sum()
        print(f"[{label:26s} plafond={ceiling:.0f}$] profit={row['profit']:+,.0f}$ "
              f"ruine(net<0 an4)={row['ruine']:.2f}% hit_ceiling={row['hit_ceiling_rate']:.2f}% "
              f"annee1<0={row['annee1_neg']:.2f}% ({time.time()-t0:.0f}s)")
        print(f"    crosstab (n={len(df)}) : hit_ceiling ET ruine={both} | hit_ceiling SANS ruine={hc_not_ruine} "
              f"| ruine SANS hit_ceiling={ruine_not_hc}")
        pd.DataFrame(rows).to_csv(f"etape_ag_hit_ceiling_measure_n{n_sims}.csv", index=False)
