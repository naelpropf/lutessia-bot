"""
Session 08/08 (suite, jour 2) -- decomposition categorie A/B (meme logique
que extra_account_v4_year1_diagnosis.py, cf session precedente) SOUS le
mecanisme de deblocage echelonne 5-paliers gagnant (FTMO/Fivers/GFT/
FundedNext a seuils independants, extra_account_v4_multi_stagger.py) --
pour verifier si l'echelonnement reduit specifiquement la categorie A
(retard de compounding) comme observe avec le levier FTMO seul.
"""
import random
import time
...
if __name__ == "__main__":
    ...
    # combo unique passable en CLI : "5-15-25-30" (FTMO-Fivers-GFT-FundedNext, en k$)
    if len(sys.argv) > 2:
        t_f, t_v, t_g, t_n = [float(x) * 1000 for x in sys.argv[2].split("-")]
        combo = (t_f, t_v, t_g, t_n)
    else:
        combo = (5000., 15000., 25000., 30000.)
    configs = [("baseline v4 (gate unique 30k)", (30000., 30000., 30000., 30000.)),
               (f"echelonne {combo[0]/1000:.1f}/{combo[1]/1000:.1f}/{combo[2]/1000:.1f}/{combo[3]/1000:.1f}k", combo)]
    ...
    for ceiling in (1000.0, 3000.0):
        ...
        for label, (t_f, t_v, t_g, t_n) in configs:
            ...
            print(f"    Categorie A (flotte debloquee a 12 mois) : {len(cat_a)}/{n_neg} ({len(cat_a)/n_neg*100:.1f}%)")
            print(f"    Categorie B (flotte PAS debloquee a 12 mois) : {len(cat_b)}/{n_neg} ({len(cat_b)/n_neg*100:.1f}%)")
            if len(cat_a) > 0:
                print(f"    Cat.A deficit moyen={cat_a['year1_net_split'].mean():,.0f}$ | "
                      f"prefund_fees={cat_a['year1_starter_prefund_fees'].mean():,.0f}$ | "
                      f"unlock_cost={cat_a['year1_fleet_unlock_cost'].mean():,.0f}$ | "
                      f"extra_open_cost={cat_a['year1_extra_account_open_cost'].mean():,.0f}$ | "
                      f"postfund_break_cost={cat_a['year1_postfund_break_net_cost'].mean():,.0f}$")
