"""
Section C (2026-08-17) : double starter -- repartition du capital jour 0
entre Blueberry et une 2e firm, en add-on isole par-dessus la reference de
travail complete (cascade groupee BB Instant+any-RR S1.8 + sizing rr_tp2
Variante B x1.6 S2.35).

Choix de la 2e firm -- justifie par les contraintes DEJA MODELISEES dans le
moteur (`etape_e_fleet_integration.seq_grouped_multi`), pas par supposition :
la sequence actuelle `seq_grouped_multi(t_ftmo=1000, t_fivers=15000,
t_gft=25000, t_fundednext=25000)` encode deja un ordre de priorite par
seuil de reserve necessaire pour deverrouiller chaque firm apres Blueberry
-- FTMO a de LOIN le seuil le plus bas (1000$, vs 15 000$/25 000$/25 000$
pour Fivers/GFT/FundedNext), donc FTMO est deja structurellement la firm
la plus proche d'un demarrage immediat : c'est celle pour laquelle
transformer une attente sequentielle en parallelisme jour 0 comble le plus
petit ecart de capital. Cout day0 confirme par citation (`ei.CONFIG_REF`,
`FORMATS['FTMO_2Step_Swing']['price']`) : 1 compte FTMO a 50 000$ = 345$,
comparable au cout Blueberry jour0 (165$), tandis que Fivers day0 (4
comptes x 545$=2 180$) est ~13x plus cher -- FTMO est aussi la seule
alternative dont le cout jour0 reste du meme ordre de grandeur que
Blueberry.

Mecanisme : le PREMIER compte FTMO (parmi les N_ACCOUNTS_DAY0['FTMO']=2
deja pre-alloues) devient actif des t=0 en parallele de Blueberry, au lieu
d'attendre le trigger sequentiel existant (`after_count`, reserve>=1000$).
Le 2e compte FTMO reste gate par le trigger normal (inchange). La
"repartition du capital" est operationnalisee comme un partage du RISQUE
par trade (pas de fractionnement de palier -- FTMO n'a pas de palier
25000$ au catalogue) : le compte FTMO jour0 trade a
`eval_risk * ftmo_day0_risk_frac` tant qu'il reste ce slot initial.
  - REF (starter unique)         : ftmo_day0_risk_frac=0.0 (FTMO day0
                                    desactive, comportement actuel inchange)
  - 50/50 (parite complete)      : ftmo_day0_risk_frac=1.0 (FTMO day0 trade
                                    au meme risque que Blueberry)
  - Asymetrique favorable BB     : ftmo_day0_risk_frac=0.5 (FTMO day0 a
                                    moitie risque, Blueberry reste dominant)

Correctif de mecanisme necessaire pour cette section (documente, pas
cache) : `open_group()` (generique, utilise aussi par Fivers/GFT/
FundedNext) re-chargeait le cout ET remettait a zero les compteurs de
TOUS les comptes du groupe -- y compris un compte deja actif depuis le
jour 0 -- au moment ou le trigger sequentiel normal se declenche. Sans
correctif, le compte FTMO day0 aurait ete facture 2x et ses compteurs
internes remis a zero au moment du trigger classique. Corrige en sautant
les comptes deja actifs (`if a["active"]: continue`).

Copie figee de chantier_A_rr_sizing_2026-08-17.py (lui-meme copie figee de
chantier_rrtp2_sizing_2026-08-16.py / chantier_cascade_combined_bb_switch_
any_rr_2026-08-16.py). Sizing rr_tp2 fixe au candidat deja adopte
(x1.6, seuil 8) -- pas les candidats non-verifies de la Section A.
"""
import random
import sys
import time
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
    ...
    for gname in FIRMS:
        ...
        # <<< CHANTIER C : le 1er compte FTMO (parmi les 2 pre-alloues)
        # devient actif des t=0 si ftmo_day0_risk_frac>0 -- le 2e reste
        # gate par le trigger sequentiel normal, inchange.
        ftmo_day0_slot = (gname == "FTMO" and ftmo_day0_risk_frac > 0.0)
        accs = []
        for slot_i in range(ei.N_ACCOUNTS_DAY0[gname]):
            this_is_day0 = is_day0 or (ftmo_day0_slot and slot_i == 0)
            ...
            a["_ftmo_day0_scaled"] = (ftmo_day0_slot and slot_i == 0)
            accs.append(a)
    ...
    state.update({"ever_funded": False, "real_cash_paid": active0_cost, "total_breaks": 0,
             ...
             "first_break_gname": None, "first_break_time_days": None})
    ...
    def open_group(gname, is_final):
        # <<< CHANTIER C : sauter les comptes deja actifs (le slot FTMO
        # day0) pour eviter un double-facturation/reset au trigger normal.
    ...
    for slot_idx, trade_idx in enumerate(order):
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                if acc.get("_ftmo_day0_scaled", False):
                    base_r = base_r * ftmo_day0_risk_frac
                ...
                if use_goat_guard:
                    ...
                    if state["first_break_gname"] is None:
                        state["first_break_gname"] = gname
                        state["first_break_time_days"] = now / 86400.0
    ...
    result = {"final_net_split": combined_net(), "is_paid_cum": state["is_paid_cum"],
              ...
              "first_break_gname": state["first_break_gname"],
              "first_break_time_days": state["first_break_time_days"]}
...
def run_propagated(pop, market_data, excluded_map, ceiling, seq_grouped, format_by_firm, emergency,
    ...
    for _ in range(n_sims):
        ...
        res = run_one(raw_trades, raw_slots, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
                      ...
                      ftmo_day0_risk_frac=ftmo_day0_risk_frac)
        rows.append(res)
...
def summarize(df, label, ceiling):
    ...
    fb_counts = df["first_break_gname"].value_counts(normalize=True) * 100
    fb_time_mean = df["first_break_time_days"].mean()
    fb_time_median = df["first_break_time_days"].median()
    return dict(config=label, ceiling=ceiling, n=len(df),
                ...
                first_break_bb_pct=fb_counts.get("Blueberry", 0.0),
                first_break_ftmo_pct=fb_counts.get("FTMO", 0.0),
                first_break_other_pct=100.0 - fb_counts.get("Blueberry", 0.0) - fb_counts.get("FTMO", 0.0),
                first_break_time_moy_j=fb_time_mean, first_break_time_med_j=fb_time_median)
...
if __name__ == "__main__":
    ...
    for ceiling in ceilings_arg:
        ...
        for name, frac in CONFIGS.items():
            t0 = time.time()
            df = run_propagated(pop, market_data, excluded_map, ceiling, seq, config,
                                 bb_threshold=bb_th, ftmo_day0_risk_frac=frac, **common_kwargs)
            ...
            print(f"[{name} plafond={ceiling:.0f}$] profit_moy={row['profit_moyen']:+,.0f}$ "
                  ...
                  f"annee1<0={row['annee1_neg']:.2f}% 1erecasse_BB={row['first_break_bb_pct']:.1f}% "
                  f"1erecasse_FTMO={row['first_break_ftmo_pct']:.1f}% 1erecasse_autre={row['first_break_other_pct']:.1f}% "
                  f"tempsmoy_1erecasse={row['first_break_time_moy_j']:.1f}j ({time.time()-t0:.0f}s)")
            pd.DataFrame(rows).to_csv(f"chantier_C_double_starter_n{n_sims}_2026-08-17.csv", index=False)
