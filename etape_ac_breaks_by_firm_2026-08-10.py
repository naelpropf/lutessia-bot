"""
Etape AC (08/10 nuit, suite 12) : nombre moyen de casses par run sur
l'horizon complet (4 ans), separe pre/post-deblocage et par firm, sous
la reference officielle actuelle (REF+V2 + cap Blueberry corrige +
FTMO-10%/GFT Goat Guard). Contexte pour evaluer combien de fois le
risque "profit non retire perdu a la casse" (deja verifie NON avere,
Etape Z Q2) pourrait se materialiser si un mecanisme de retrait etait
un jour ajoute.

Copie de etape_q_v2_plus_ftmo_gft_2026-08-10.py, instrumentation ajoutee :
compteur de casses par firm, split pre/post `fleet_unlocked`.
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
    ...
    state = {"reserve": 0.0, "ever_funded": False, "real_cash_paid": active0_cost, "total_breaks": 0,
             ...
             "breaks_pre": {g: 0 for g in FIRMS}, "breaks_post": {g: 0 for g in FIRMS}}
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                if use_goat_guard:
                    ...
                    if fleet_unlocked:
                        state["breaks_post"][gname] += 1
                    else:
                        state["breaks_pre"][gname] += 1
    ...
    for g in FIRMS:
        result[f"breaks_pre_{g}"] = state["breaks_pre"][g]
        result[f"breaks_post_{g}"] = state["breaks_post"][g]
    return result
...
if __name__ == "__main__":
    ...
    for ceiling in ceilings_arg:
        ...
        for g in FIRMS:
            row[f"mean_breaks_pre_{g}"] = df[f"breaks_pre_{g}"].mean()
            row[f"mean_breaks_post_{g}"] = df[f"breaks_post_{g}"].mean()
        row["mean_breaks_pre_total"] = sum(row[f"mean_breaks_pre_{g}"] for g in FIRMS)
        row["mean_breaks_post_total"] = sum(row[f"mean_breaks_post_{g}"] for g in FIRMS)
        rows.append(row)
        pre_str = " ".join(f"{g}={row[f'mean_breaks_pre_{g}']:.2f}" for g in FIRMS)
        post_str = " ".join(f"{g}={row[f'mean_breaks_post_{g}']:.2f}" for g in FIRMS)
        print(f"[plafond={ceiling:.0f}$] PRE-deblocage total={row['mean_breaks_pre_total']:.2f} [{pre_str}] | "
              f"POST-deblocage total={row['mean_breaks_post_total']:.2f} [{post_str}] ({time.time()-t0:.0f}s)")
        pd.DataFrame(rows).to_csv(f"etape_ac_breaks_by_firm_n{n_sims}.csv", index=False)
