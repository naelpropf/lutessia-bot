...
if __name__ == "__main__":
    ...
    CANDIDATES = {
        ...
        "A4_worstdecile_x0.5": make_size_func_downsize(WORST_DECILE_RANGE, 0.5),
        "A4_worstdecile_x0.8": make_size_func_downsize(WORST_DECILE_RANGE, 0.8),
        "A5_belowmean_x0.5": make_size_func_downsize(BELOW_MEAN_RANGES, 0.5),
        "A5_belowmean_x0.8": make_size_func_downsize(BELOW_MEAN_RANGES, 0.8),
    }
    for name, f in CANDIDATES.items():
        maxm = max(f(x) for x in [0, 3, 5, 6, 6.5, 7, 8, 8.82, 10, 15, 20, 30])
        minm = min(f(x) for x in [0, 1.8, 2.5, 3.2, 4.2, 4.8, 5.9, 6.2, 8.82, 10, 15, 20, 30])
        print(f"[verif marge] {name} : mult_max={maxm:.2f} -> marge={margin_pct(maxm):.1f}%"
              ...
              f"  mult_min={minm:.2f} (downsizing, contrainte marge triviale -- reduit le risque, ne peut pas la violer)")
    ...
    for ceiling in ceilings_arg:
        ...
        for name, size_func in CANDIDATES.items():
            ...
            pd.DataFrame(rows).to_csv(f"chantier_A_downsizing_n{n_sims}_2026-08-17.csv", index=False)
