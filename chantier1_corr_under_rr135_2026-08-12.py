"""
Chantier 1 (confirmation combinee), Section A (08/12) : le premier balayage
de seuil de correlation (chantier1_corr_threshold_sweep_2026-08-12.py) a ete
fait SOUS RR>=1.25 -- mais le chantier a depuis trouve RR>=1.35 meilleur
(registre_strategie_trading.md §2.8bis, candidat n=300). Choisir le seuil de
correlation sur la base de RR=1.25 pourrait etre biaise si le meilleur seuil
depend du seuil RR (plus de trades admis a RR=1.35 relache change la
distribution des conflits de correlation).

Redensifie {0.60, 0.65, 0.70, 0.75, 0.80, 0.85} sur le moteur flotte
officiel (Option A/Prime, config Run C), n=300, deux plafonds, SOUS
RR>=1.35 (population construite avec min_rr=1.35, pas 1.25). Regle JPY-JPY
inchangee (active).
...
if __name__ == "__main__":
    ...
    print(f"[verif] population construite (RR>=1.35) : {len(pop)} trades (attendu 631)")
    ...
    for thr in THRESHOLDS:
        excluded_map = precompute_correlation_pairs_variant(tickers, corr_matrix, thr, apply_jpy_rule=True)
        label = f"corr_{thr:.2f}" + ("_baseline0.6" if thr == 0.6 else "")
        for ceiling in ceilings_arg:
            ...
            pd.DataFrame(rows).to_csv(f"chantier1_corr_under_rr135_n{n_sims}.csv", index=False)
