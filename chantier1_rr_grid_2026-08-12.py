"""
Chantier 1, Section 3 (08/12) : le seuil rr_tp1>=1.25 vient d'une grille grossiere
{1.0/1.1/1.25/1.35/1.5/1.75/2.0/2.5} (registre_strategie_trading.md §2.8). Affine
autour de l'optimum trouve : {1.05,1.10,1.15,1.20,1.25(ref),1.30,1.35,1.40,1.45},
sur la population 721 trades AVEC LE PAYOFF ACTUELLEMENT EN PRODUCTION (trailing
0.15xSL, pas le payoff TP1-plat utilise par l'ancien rr_threshold_test.py -- cf.
etape_ai_payout_cadence_calibration_2026-08-11.py: build_population_with_trailing
("fixed", 0.15, min_rr=1.25)).

Etape 1 (SOLO, pas flotte) : pour chaque seuil, EV et n des trades PRIS apres
plafond 3 positions + correlation 0.6/JPY (meme filtre chronologique deterministe
que rr_threshold_test.taken_trades, adapte au payoff trailing).
Etape 2 (FLOTTE, n=300, 2 plafonds) : confirme les 2-3 seuils les plus prometteurs
identifies a l'etape 1 (+ 1.25 comme reference), moteur officiel Option A/Prime
(config Run C).
...
from scaling_simulation import is_jpy, MAX_POSITIONS
...
GRID = [1.05, 1.10, 1.15, 1.20, 1.25, 1.30, 1.35, 1.40, 1.45]
...
def correlated(a, b, corr_matrix, threshold):
    if is_jpy(a) and is_jpy(b):
        return True
    return abs(corr_matrix.loc[a, b]) > threshold


def taken_trades_trailing(sub, corr_matrix, threshold=CORR_TH):
    """Meme logique que rr_threshold_test.taken_trades (plafond 3 positions +
    correlation 0.6/JPY, marche chronologique reelle), adaptee pour ne pas
    dependre de resolution_time_est deja calculee differemment ici (sub porte
    deja resolution_time_est depuis build_population_with_trailing)."""
    ...
    for idx, row in sub.iterrows():
        ...
        if len(open_positions) >= MAX_POSITIONS:
            continue
        if any(correlated(row["ticker"], t, corr_matrix, threshold) for (t, _) in open_positions):
            continue
...
def ev_stats(sub, col="r_trailing"):
    n = len(sub)
    if n == 0:
        return dict(n=0, winrate_pct=float("nan"), ev=float("nan"), sum_r=float("nan"))
    wins = sub[sub["statut_final"] == "OBJECTIF ATTEINT"]
    winrate = len(wins) / n
    ev = sub[col].mean()
    sum_r = sub[col].sum()
    return dict(n=n, winrate_pct=winrate * 100, ev=ev, sum_r=sum_r)
...
if __name__ == "__main__":
    ...
    n_confirm = int(sys.argv[3]) if len(sys.argv) > 3 else 3
    t_start = time.time()

    print("=" * 100)
    print("ETAPE 1 -- SOLO : EV/n par seuil rr_tp1, payoff trailing 0.15xSL (superset min_rr=1.0)")
    print("=" * 100)
    pop_all = build_population_with_trailing("fixed", 0.15, min_rr=1.0, verbose=False)
    print(f"[verif] population superset (min_rr=1.0) : {len(pop_all)} trades")
    corr_matrix = pd.read_csv("correlation_matrix.csv", index_col=0)

    solo_rows = []
    for thr in GRID:
        sub = pop_all[pop_all["rr_tp1"] >= thr].copy()
        gs_raw = ev_stats(sub)
        taken = taken_trades_trailing(sub, corr_matrix)
        gs_taken = ev_stats(taken)
        solo_rows.append(dict(
            threshold=thr, n_brut=gs_raw["n"], winrate_brut_pct=gs_raw["winrate_pct"], ev_brut=gs_raw["ev"],
            n_pris=gs_taken["n"], winrate_pris_pct=gs_taken["winrate_pct"], ev_pris=gs_taken["ev"],
            sum_r_pris=gs_taken["sum_r"],
        ))
        print(f"  seuil={thr:.2f} | brut n={gs_raw['n']:>3} winrate={gs_raw['winrate_pct']:.1f}% EV={gs_raw['ev']:+.4f}R "
              f"| PRIS n={gs_taken['n']:>3} winrate={gs_taken['winrate_pct']:.1f}% EV={gs_taken['ev']:+.4f}R "
              f"sommeR={gs_taken['sum_r']:+.1f}")

    solo_df = pd.DataFrame(solo_rows)
    solo_df.to_csv("chantier1_rr_grid_solo.csv", index=False)

    ranked = solo_df.sort_values("ev_pris", ascending=False)
    top_candidates = [t for t in ranked["threshold"].tolist() if t != 1.25][:n_confirm]
    print(f"\n[ETAPE 1] Top {n_confirm} candidats par EV_pris (hors reference 1.25) : {top_candidates}")

    print("\n" + "=" * 100)
    print(f"ETAPE 2 -- FLOTTE n={n_sims}, confirmation des {len(top_candidates)} candidats + reference 1.25")
    print("=" * 100)
    ...
    flotte_rows = []
    for thr in sorted(set(top_candidates + [1.25])):
        pop_thr = build_population_with_trailing("fixed", 0.15, min_rr=thr, verbose=False)
        tickers = sorted(pop_thr["ticker"].unique())
        excluded_map = precompute_correlation_pairs(tickers, corr_matrix, CORR_TH)
        label = f"rr_{thr:.2f}" + ("_ref" if thr == 1.25 else "")
        for ceiling in ceilings_arg:
            t0 = time.time()
            df = run_propagated(pop_thr, market_data, excluded_map, ceiling, seq, config, ei.DEFAULT_EMERGENCY,
            ...
            flotte_rows.append(row)
            print(f"[{label} plafond={ceiling:.0f}$] n_trades={len(pop_thr)} "
                  f"profit_moyen={row['profit_moyen']:+,.0f}$ profit_median={row['profit_median']:+,.0f}$ "
            ...
            pd.DataFrame(flotte_rows).to_csv(f"chantier1_rr_grid_flotte_n{n_sims}.csv", index=False)
