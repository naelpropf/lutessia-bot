"""
Etape 3a de la demande (08/08, suite) : deblocage partiel anticipe -- debloquer
FTMO seul a un seuil de reserve reduit (10k/15k/20k$), AVANT le deblocage
complet du reste de la flotte (Fivers/GFT/FundedNext) a 30k$ comme
actuellement. Cible la categorie B (flotte pas encore debloquee a 12 mois,
Blueberry seul trop lent) sans toucher au reste de la config finale v4
(mecanisme "compte supplementaire" deplafonne, flotte=2.75%, eval=2.25%/
GFT=1.75%).

SEQ_GROUPED passe de 2 etapes (Blueberry day0 -> reste groupe a 30k) a 3 :
Blueberry day0 -> FTMO seul au seuil reduit -> reste (Fivers/GFT/FundedNext)
a 30k. fleet_unlocked (qui gouverne le downgrade-on-reopen Blueberry ET
l'eligibilite du mecanisme "compte supplementaire") ne passe True qu'au
DERNIER palier (reste de la flotte a 30k), pas au deblocage FTMO partiel --
comportement inchange pour tout le reste du moteur.
"""
import random
import time
...
def seq_grouped_partial(partial_threshold):
    # (group_names, trigger, reserve_threshold, sets_fleet_unlocked)
    return [
        ((STARTER,), "day0", None, False),
        (("FTMO",), ("after_count", 1), partial_threshold, False),
        (("Fivers", "GFT", "FundedNext"), ("after_count", 1), DEFAULT_RESERVE_FINAL, True),
    ]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        thresholds = [float(x) * 1000 for x in sys.argv[2].split(",")]
    else:
        thresholds = [10000.0, 15000.0, 20000.0]
    configs = [("baseline (pas de deblocage partiel)", None)] + \
              [(f"FTMO partiel a {t/1000:.0f}k$", t) for t in thresholds]

    rows = []
    for ceiling in (1000.0, 3000.0):
        print(f"\n{'='*100}\nPlafond {ceiling:.0f}$ (n={n_sims})\n{'='*100}")
        for label, thresh in configs:
            seq = ([((STARTER,), "day0", None, False),
                    (("FTMO", "Fivers", "GFT", "FundedNext"), ("after_count", 1), DEFAULT_RESERVE_FINAL, True)]
                   if thresh is None else seq_grouped_partial(thresh))
            t0 = time.time()
        ...
        pd.DataFrame(rows).to_csv("extra_account_v4_partial_unlock.csv", index=False)
