"""
Section B, strategie ADAPTEE (2026-08-17) -- suite de la reouverture
ciblee : GFT Instant en comptes extra paralleles (S2.37, registre_
strategie_trading.md, REJETE) rejouait la strategie CLASSIQUE a
l'identique sur des comptes Instant. Ce chantier teste des candidats
INFORMES par le diagnostic du mode de casse
(`chantier_B_gft_instant_failure_diagnostic_2026-08-17.py`, n=100 comptes
isoles, 7790 casses observees) :

RESULTAT CLE DU DIAGNOSTIC (cause directe, pas une intuition generique) :
- 74,9%+11,1%=86,0% des casses impliquent le DD JOURNALIER (3,0%), pas
  le DD max trailing (6,0%) seul (14,0%).
- Le trade DECLENCHEUR (celui qui fait franchir le seuil) est rr_tp2>=8
  (deja sizee x1,6 par le sizing S2.35 adopte) dans **49,0% des cas, contre
  15,2% dans la population globale -- surrepresentation x3,22**.
- **100% des casses "en un seul trade" (n=801/7790) ont un trade
  declencheur rr_tp2>=8.** Mecanisme arithmetique direct : risque funded
  1,90% x1,6 (sizing S2.35) = 3,04% > seuil DD journalier 3,0% -- UN SEUL
  trade sizee suffit a lui seul a casser le compte, la ou un trade non
  sizee (1,90%) ne le peut jamais seul. Meme SANS sizing, 2 pertes le meme
  jour a 1,90% chacune (3,80%) depassent deja le seuil 3,0% (explique la
  part max_trailing/both restante).
- Par contraste, aucun segment rr_tp2 n'est surrepresente parmi les
  PERTES en general (ratios 0,89-1,08 vs population) -- ce n'est pas "les
  gros RR planifies perdent plus souvent", c'est "le sizing qu'on leur
  applique les rend disproportionnellement dangereux pour CE DD-ci
  specifiquement".

2 candidats, tous deux DISTINCTS de la strategie classique rejouee
(S2.37) et de ce qui a deja ete teste en S6.2/S6.3 (RR filtre, cap
position, exclusion paires -- aucun ne touchait au sizing rr_tp2, qui
n'existait pas encore le 08/15/16) :
  B1_no_sizing       : comptes GFT Instant EXEMPTES du sizing rr_tp2 (size_
                       func force a 1,0x pour ces comptes uniquement, le
                       reste de la flotte garde le sizing normal).
  B2_no_sizing_capped : B1 + risque de base funded plafonne a 1,40% pour
                       GFT Instant specifiquement (2x1,40%=2,80%<3,0% --
                       protege aussi contre 2 pertes NON sizees le meme
                       jour, seuil derive du calcul ci-dessus, pas un
                       "reduire le risque" generique).

Copie figee de chantier_B_gft_instant_parallel_2026-08-17.py.
"""
import random
import sys
import time
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
            ...
            gft_extra_instant=False, exempt_gft_instant_sizing=False, gft_instant_risk_cap=None):
    ...
    state.update({"ever_funded": False, "real_cash_paid": active0_cost, "total_breaks": 0,
             ...
             "gft_instant_extra_opens": 0, "gft_classic_extra_opens": 0,
             "gft_breaks": 0, "gft_instant_breaks": 0})
    ...
    def process_extra_account(now):
        ...
        for gname in ei.GROWTH_FIRMS_EXTRA:
            ...
            elif gname == "GFT" and gft_extra_instant:
                extra_fmt_key = "GFT_InstantGOAT"
                extra_fmt = FORMATS[extra_fmt_key]
            ...
            if state["reserve"] >= extra_threshold_mult * extra_cost:
                ...
                if gname == "Blueberry":
                    ...
                    if extra_fmt_key == "GFT_InstantGOAT":
                        state["gft_instant_extra_opens"] += 1
                    else:
                        state["gft_classic_extra_opens"] += 1
    ...
    for slot_idx, trade_idx in enumerate(order):
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                is_gft_instant = (gname == "GFT" and acc["_fmt_key"] == "GFT_InstantGOAT")
                if is_gft_instant and gft_instant_risk_cap is not None:
                    base_r = min(base_r, gft_instant_risk_cap)
                pdef = _current_phase(fmt, acc)
                r = effective_risk(acc, pdef, base_r)
                if size_func is not None and not (is_gft_instant and exempt_gft_instant_sizing):
                ...
                if use_goat_guard:
                    ...
                    if gname == "GFT":
                        state["gft_breaks"] += 1
                        if acc["_fmt_key"] == "GFT_InstantGOAT":
                            state["gft_instant_breaks"] += 1
                    ...
                    if use_bb_reset:
                        ...
                        if gname == "Blueberry":
                            ...
                            new_fmt = fmt
                            cost = acc["base_cost"]
                        elif gname == "GFT" and acc.get("_is_extra", False) and gft_extra_instant:
                            new_fmt_key = "GFT_InstantGOAT"
                            acc["_fmt_key"] = new_fmt_key
                            new_fmt = FORMATS[new_fmt_key]
                            cost = ei.price_for(new_fmt_key, acc["palier"])
                        else:
                            new_fmt = fmt
    ...
    result = {"final_net_split": combined_net(), "is_paid_cum": state["is_paid_cum"],
              ...
              "gft_instant_extra_opens": state["gft_instant_extra_opens"],
              "gft_classic_extra_opens": state["gft_classic_extra_opens"],
              "gft_breaks": state["gft_breaks"], "gft_instant_breaks": state["gft_instant_breaks"]}
...
def run_propagated(pop, market_data, excluded_map, ceiling, seq_grouped, format_by_firm, emergency,
    ...
    for _ in range(n_sims):
        ...
        res = run_one(raw_trades, raw_slots, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
                      ...
                      gft_extra_instant=gft_extra_instant, exempt_gft_instant_sizing=exempt_gft_instant_sizing,
                      gft_instant_risk_cap=gft_instant_risk_cap)
        rows.append(res)
...
def summarize(df, label, ceiling):
    ...
    return dict(config=label, ceiling=ceiling, n=len(df),
                ...
                gft_instant_extra_opens_moy=df["gft_instant_extra_opens"].mean(),
                gft_classic_extra_opens_moy=df["gft_classic_extra_opens"].mean(),
                gft_breaks_moy=df["gft_breaks"].mean(),
                gft_instant_breaks_moy=df["gft_instant_breaks"].mean())
...
if __name__ == "__main__":
    ...
    CONFIGS = {
        "REF_travail (S1.8+S2.35)": dict(gft_extra_instant=False, exempt_gft_instant_sizing=False, gft_instant_risk_cap=None),
        "B0_original_REJETE (S2.37)": dict(gft_extra_instant=True, exempt_gft_instant_sizing=False, gft_instant_risk_cap=None),
        "B1_no_sizing": dict(gft_extra_instant=True, exempt_gft_instant_sizing=True, gft_instant_risk_cap=None),
        "B2_no_sizing_capped1.4": dict(gft_extra_instant=True, exempt_gft_instant_sizing=True, gft_instant_risk_cap=1.40),
    }
    ...
    for ceiling in ceilings_arg:
        ...
        for name, kw in CONFIGS.items():
            ...
            df = run_propagated(pop, market_data, excluded_map, ceiling, seq, config,
                                 bb_threshold=bb_th, **kw, **common_kwargs)
            ...
            print(f"[{name} plafond={ceiling:.0f}$] profit_moy={row['profit_moyen']:+,.0f}$ "
                  ...
                  f"annee1<0={row['annee1_neg']:.2f}% gft_instant_extra_opens_moy={row['gft_instant_extra_opens_moy']:.2f} "
                  f"gft_breaks_moy={row['gft_breaks_moy']:.2f} gft_instant_breaks_moy={row['gft_instant_breaks_moy']:.2f} "
                  f"({time.time()-t0:.0f}s)")
            pd.DataFrame(rows).to_csv(f"chantier_B_gft_instant_adapted_n{n_sims}_2026-08-17.csv", index=False)
