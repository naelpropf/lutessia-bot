"""
Piste "amplification" (08/11) : ETAPE 2b/3 -- mecanisme d'amplification
d'exposition sur bonne serie CONFIRMEE, teste au niveau flotte. Copie de
etape_ao_run_f_cout_reel_2026-08-11.py (config officielle par plafond
deja adoptee -- Run C/BB14j a 1000$, Run F/BB7j a 3000$, decision #16
registre_parametres_projet.md), UN ajout : detection + 3 variantes
d'action, symetriques du coupe-circuit deja teste/rejete
(edge_circuit_breaker_v2_2026-08-11.py) mais avec un seuil INVERSE
(winrate glissant > moyenne+2sigma au lieu de < moyenne-2sigma) et un
EFFET OPPOSE (amplifier au lieu de couper).

Etape 1 (edge_amplification_episodes_2026-08-11.py) a deja mesure un
signal d'alerte fort AVANT ce test : la persistance winrate-glissant-avant
-> issue-du-trade-suivant est quasi nulle sur le cote positif (corr
+0.03 a +0.11 selon N), et les "episodes chauds" trouves avec le meme
seuil 2sigma que Piste 7 sont beaucoup plus rares/courts (0-10 episodes
de qq jours a qq semaines) que les 6 episodes froids substantiels de
Piste 7 (semaines a mois). L'hypothese d'asymetrie du cout d'erreur
(prompt utilisateur) reste testee ici jusqu'au bout malgre ce signal
defavorable -- mesure, pas suppose.

Detection : compute_boost_mask(), meme convention que compute_pause_mask
(edge_circuit_breaker_v2, §2.16 registre_parametres_projet.md) -- un seul
flux de trades brut, fenetre glissante N sans lookahead, entree si
winrate glissant > X_entry_upper, sortie M_recover trades apres l'entree
(delai fixe). Seuil par defaut : formule 2sigma identique a Etape 1,
N=20 (X_entry_upper=0.623), M_recover=20 -- config la plus proche du
point3 par defaut du coupe-circuit (20, 0.18, 20) pour comparaison
apples-to-apples directe.

3 variantes d'action testees separement (jamais combinees) :
1. "unlock"  -- seuil de reserve pour le declenchement de groupe/palier
   temporairement abaisse (x BOOST_UNLOCK_MULT) pendant un episode boost.
2. "sizing"  -- risque relache (x BOOST_SIZE_MULT) pendant un episode
   boost, symetrique inverse du sizing DD-distance deja adopte (V2,
   §2.9x registre_parametres_projet.md) qui REDUIT le risque en approche
   de DD -- ici on l'AUGMENTE en confirmation de bonne serie.
3. "reopen"  -- cout de reouverture/rachat apres casse reduit
   (x BOOST_REOPEN_DISCOUNT) pendant un episode boost, meme mecanique que
   le discount FTMO deja code (ftmo_discount), applique ici a TOUTES les
   firms pendant un boost au lieu d'etre une remise permanente
   FTMO-only.
...
# Decision #16 (08/11, tranchee, registre_parametres_projet.md §1.8/§2.35bis) :
# Blueberry 7j+surcout UNIQUEMENT a 3000$ (Run F), 14j sans surcout a
# 1000$ (Run C) -- meme convention que etape_ao_run_f_cout_reel corrige.
...
# Amplification -- parametres par defaut des 3 variantes (jamais combinees)
BOOST_UNLOCK_MULT = 0.5      # variante "unlock" : seuil de reserve x0.5 pendant un boost
BOOST_SIZE_MULT = 1.3        # variante "sizing" : risque x1.3 pendant un boost
BOOST_REOPEN_DISCOUNT = 0.85  # variante "reopen" : cout de rachat x0.85 pendant un boost
...
def compute_boost_mask(raw_trades, N, X_entry_upper, M_recover):
    """Detection de bonne serie confirmee -- meme convention que
    compute_pause_mask (edge_circuit_breaker_v2), seuil et sens inverses.
    Fenetre glissante des N derniers outcome_r (sans lookahead). Entree :
    winrate glissant > X_entry_upper. Sortie : M_recover trades ecoules
    depuis l'entree (delai fixe)."""
    n = len(raw_trades)
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
            ...
            boost_mask=None, boost_variant=None):
    ...
    def effective_risk(acc, pdef, base_r, boosting):
        r = base_r
        if b_entry_frac is not None and not (pre_unlock_only and fleet_unlocked):
            ...
            if dd_max is not None:
                ...
                r = r * mult
        if boosting and boost_variant == "sizing":
            r = r * BOOST_SIZE_MULT
        return r
    ...
    for slot_idx, trade_idx in enumerate(order):
        ...
        state["_now"] = now
        boosting = bool(boost_mask[slot_idx]) if boost_mask is not None else False
        if boosting:
            state["boosted_trades"] += 1
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                r = effective_risk(acc, pdef, base_r, boosting)
                ...
                if use_goat_guard:
                    ...
                    reopen_discount = BOOST_REOPEN_DISCOUNT if (boosting and boost_variant == "reopen") else 1.0
                    if use_bb_reset:
                        cost = 2.0 * acc["base_cost"] * reopen_discount
                        acc["active"] = False
        ...
        unlock_mult = BOOST_UNLOCK_MULT if (boosting and boost_variant == "unlock") else 1.0
        still_pending = []
        for group_names, trig, thresh, is_final in pending_group_trigger:
            _, n_req = trig
            if state["group_funded_count"] >= n_req and state["reserve"] >= thresh * unlock_mult:
    ...
    result = {"final_net_split": combined_net(), "is_paid_cum": state["is_paid_cum"],
              ...
              "boosted_trades": state["boosted_trades"]}
...
# Grille de detection (Etape 1 : formule 2sigma, mirroir exact du coupe-circuit
# deja teste -- edge_circuit_breaker_v2 CB_GRID, seuils bas devenus hauts).
GOOD_WINDOWS = [  # Etape 1, edge_amplification_episodes_2026-08-11.csv, N=20 (config par defaut ci-dessous)
    ("2022-04-11", "2022-05-13"), ("2023-07-11", "2023-08-01"), ("2023-12-20", "2023-12-20"),
    ("2024-07-04", "2024-07-07"), ("2024-12-03", "2024-12-10"), ("2026-07-23", "2026-07-24"),
]
GOOD_WINDOWS = [(pd.Timestamp(s), pd.Timestamp(e)) for s, e in GOOD_WINDOWS]
DETECT_N, DETECT_X, DETECT_M = 20, 0.623, 20  # defaut : mirroir exact de (20, 0.18, 20), meilleure config point3 CB
        ...
        for label, variant in configs:
            ...
            for run_idx in range(n_sims):
                ...
                if variant is not None:
                    mask, episodes = compute_boost_mask(raw_trades, DETECT_N, DETECT_X, DETECT_M)
                    for (s, e) in episodes:
                        dates = [raw_trades[j]["date"] for j in range(s, e + 1)]
                        overlaps = any(gs <= d <= ge for d in dates for (gs, ge) in GOOD_WINDOWS)
                        n_episodes_total += 1
                        if overlaps:
                            n_tp_total += 1
                        else:
                ...
                res = run_one(raw_trades, raw_slots, market_data, excluded_map, order, ceiling, seq, config,
                              ...
                              boost_mask=mask, boost_variant=variant)
                recs.append(res)
            df = pd.DataFrame(recs)
            net = df["final_net_split"] - df["is_paid_cum"]
            year1_neg = df["year1_net_split"] < 0
            row = dict(ceiling=ceiling, variant=label, n=len(df),
                       ...
                       boosted_trades_moy=df["boosted_trades"].mean(),
                       n_episodes_total=n_episodes_total, n_tp_episodes=n_tp_total, n_fp_episodes=n_fp_total,
                       fp_rate_pct=(n_fp_total / n_episodes_total * 100) if n_episodes_total else None)
            rows.append(row)
            print(f"[ceiling={ceiling:.0f}$ variant={label:10s}] profit_moyen={row['profit_moyen']:+,.0f}$ "
                  ...
                  f"episodes={n_episodes_total}(TP={n_tp_total}/FP={n_fp_total}) ({time.time()-t0:.0f}s)")
            pd.DataFrame(rows).to_csv(f"edge_amplification_fleet_n{n_sims}.csv", index=False)
...
if __name__ == "__main__":
    ...
    variants_arg = sys.argv[3].split(",") if len(sys.argv) > 3 else ["unlock", "sizing", "reopen"]
    ...
    print(f"[verif] detection : N={DETECT_N} X_entry_upper={DETECT_X} M_recover={DETECT_M}")

    run_sweep(n_sims, ceilings_arg, variants_arg)
