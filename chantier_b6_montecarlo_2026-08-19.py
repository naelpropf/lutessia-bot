"""
Chantier B6-2 (2026-08-19) -- Monte Carlo fleet des leviers B mûrs :
trailing 0,10x, filtre ADX>32,27, segmentation rr_tp1<=1,25 (sizing reduit).
Piste "marge RR minimale sur swap" EXCLUE (jugee pas mure, Partie 1.1).

Copie figee de chantier_strategie_b_isolation_indices_2026-08-18.py (meme
moteur run_one/run_propagated/summarize/process_trade_corr_swap_rr,
INCHANGES), SEULES modifications :
  1. size_func recoit desormais le trade ENTIER (pas juste rr_tp2) pour
     permettre un sizing conditionne sur rr_tp1 EN PLUS du boost rr_tp2
     existant (S2.35) -- ligne <<< MODIFIE ci-dessous.
  2. build_pop_B_variant() : meme construction que build_pop_B("tout_indices")
     mais avec facteur de trailing configurable (0,10 au lieu de 0,15 en dur)
     et filtre optionnel d'exclusion de trades (utilise pour ADX).
...
# <<< CORRECTIF 2026-08-19 : ei.ALPHA_POST/ei.BETA_POST=260,388 sont calibres sur la
# population Stratégie A (646 trades figes au 27/07, winrate 40,09%) -- errones pour
# la Strategie B (winrate reel ~48%). Recalcules ICI sur la population EXACTE que
# construit build_pop_B_variant() (forex rr_tp1 in [0.75,1.35) non-index + tout_indices,
# historique_lutessia_15k_force.csv) : n=571 (401 fx + 170 idx), wins=275, losses=296,
# winrate=48,16%. Meme methodologie que ei.ALPHA_POST (prior uniforme Beta(1,1) +
# wins/losses observes) : alpha=275+1, beta=296+1. Derivation complete dans
# registre_parametres_projet.md §9.6. Ne couvre PAS un eventuel perimetre B avec
# metaux/nouveau routage (session 08-19 soir, pas encore adopte) -- a refaire si ce
# perimetre change.
ALPHA_POST_B, BETA_POST_B = 276, 297
...
def make_size_func_combined(rrtp2_mult=1.6, rrtp2_threshold=8.0, rrtp1_mult=1.0, rrtp1_threshold=None):
    """<<< NOUVEAU (B6-2) : compose le boost rr_tp2 existant (S2.35) avec un
    downsize optionnel sur rr_tp1<=seuil (piste 1.2, segmentation interne B).
    rrtp1_mult=1.0/rrtp1_threshold=None -> desactive (comportement identique
    a make_size_func_tail seul)."""
    def f(trade):
        m = rrtp2_mult if trade["rr_tp2"] >= rrtp2_threshold else 1.0
        if rrtp1_threshold is not None and trade["rr_tp1"] <= rrtp1_threshold:
            m *= rrtp1_mult
        return m
    return f
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                if size_func is not None:
                    r = r * size_func(trade)  # <<< MODIFIE (B6-2) : trade entier, pas juste rr_tp2
...
def run_propagated(pop, market_data, excluded_map, ceiling, seq_grouped, format_by_firm, emergency,
    ...
    for _ in range(n_sims):
        wr_draw = rng_wr.betavariate(ALPHA_POST_B, BETA_POST_B)
...
def load_index_population_with_payoff(trailing_factor=0.15):
    ...
    for _, row in pop.iterrows():
        ...
        if candles is None:
            ...
            r_realiste.append(val); r_trailing.append(val); tp1_times.append(None); sl_times.append(None)
            continue
        res = tpseq.analyze_trade(row, candles)
        case = res.get("case", "pas_de_donnees")
        tp1_times.append(res.get("tp1_time")); sl_times.append(res.get("sl_time"))
        ...
        if row["statut_final"] == "OBJECTIF ATTEINT" and case in CONTINUATION_CONFIRMED_CASES:
            sim = simulate_trailing(row, candles, _stop_fn_fixed(trailing_factor), f"fixed_{trailing_factor}")
...
def build_pop_B_variant(trailing_factor=0.15, exclude_keys=None, min_rr_a=MIN_RR_NEW):
    """<<< NOUVEAU (B6-2) : meme construction que build_pop_B('tout_indices')
    (chantier_strategie_b_isolation_indices_2026-08-18.py:772-799, deja
    valide/deduplique), avec facteur de trailing configurable au lieu de
    0,15 en dur, et filtre optionnel d'exclusion de trades par cle
    (ticker, date_creation) -- utilise pour le filtre ADX>32,27."""
    pop_B_fx_all = build_population_with_trailing("fixed", trailing_factor, min_rr=0.75, verbose=False)
    ...
    pop_idx = load_index_population_with_payoff(trailing_factor)

    keep_cols = ["date_creation", "ticker", "rr_tp1", "rr_tp2", "statut_final", "r_trailing",
                 "resolution_time_est", "prix_entree", "stop_loss_init"]
    combined = pd.concat([pop_B_fx[keep_cols], pop_idx[keep_cols]], ignore_index=True)
    combined = combined.sort_values("date_creation").reset_index(drop=True)

    if exclude_keys:
        mask = ~combined.apply(lambda r: (r["ticker"], r["date_creation"]) in exclude_keys, axis=1)
        n_before = len(combined)
        combined = combined[mask].reset_index(drop=True)
        print(f"  [filtre exclusion] {n_before} -> {len(combined)} ({n_before-len(combined)} exclus)")
...
if __name__ == "__main__":
    ...
    adx_spec = importlib.util.spec_from_file_location("adx_b62", "edge_adx_atr_filters_2026-08-11.py")
    adx_mod = importlib.util.module_from_spec(adx_spec)
    adx_spec.loader.exec_module(adx_mod)
    ...
    levers_arg = sys.argv[3].split(",") if len(sys.argv) > 3 else ["baseline", "trailing010", "adx", "rrtp1sizing"]
    ...
    print("=" * 78)
    print(f"Leviers testes : {levers_arg}")
    print("=" * 78)

    populations = {}

    if "baseline" in levers_arg:
        pop_base = build_pop_B_variant(trailing_factor=0.15)
        print(f"[verif] baseline (0,15x, sans filtre) : n={len(pop_base)}")
        populations["baseline"] = pop_base

    if "trailing010" in levers_arg:
        pop_t010 = build_pop_B_variant(trailing_factor=0.10)
        print(f"[verif] trailing010 (0,10x) : n={len(pop_t010)}")
        populations["trailing010"] = pop_t010

    if "adx" in levers_arg:
        # Calcule ADX sur la population de reference (0,15x, tickers/dates identiques
        # a baseline), determine les cles a exclure, puis reconstruit avec le filtre.
        pop_ref_for_adx = populations["baseline"] if "baseline" in populations else build_pop_B_variant(trailing_factor=0.15)
        pop_ref_for_adx = pop_ref_for_adx.copy()
        pop_ref_for_adx["yahoo_symbol"] = pop_ref_for_adx["ticker"].apply(tpseq.ticker_to_yahoo_symbol)
        unique_symbols = sorted(pop_ref_for_adx["yahoo_symbol"].dropna().unique())
        indic_by_symbol = {}
        for symbol in unique_symbols:
            candles = tpseq.fetch_h1_history(symbol, pop_ref_for_adx["date_creation"].min().to_pydatetime(),
        ...
        exclude_keys = set()
        ADX_TH = 32.268
        for _, row in pop_ref_for_adx.iterrows():
            candles = indic_by_symbol.get(row["yahoo_symbol"])
            if candles is None:
                continue  # <<< pas de donnee ADX -> PAS exclu (choix explicite)
            prior = candles[candles["datetime"] <= row["date_creation"]]
            if prior.empty or "adx" not in prior:
                continue
            adx_val = prior["adx"].iloc[-1]
            if adx_val > ADX_TH:
                exclude_keys.add((row["ticker"], row["date_creation"]))
        print(f"[verif] filtre ADX>{ADX_TH:.2f} : {len(exclude_keys)} trades identifies a exclure "
              f"(sur {pop_ref_for_adx['yahoo_symbol'].notna().sum()} couverts par ADX)")
        pop_adx = build_pop_B_variant(trailing_factor=0.15, exclude_keys=exclude_keys)
        populations["adx"] = pop_adx

    if "rrtp1sizing" in levers_arg:
        pop_rrtp1 = populations["baseline"] if "baseline" in populations else build_pop_B_variant(trailing_factor=0.15)
        populations["rrtp1sizing"] = pop_rrtp1  # meme population, sizing gere via size_func

    seq = ei.seq_grouped_multi(1000, 15000, 25000, 25000)
    config = ei.CONFIG_REF
    size_func_baseline = make_size_func_combined(rrtp2_mult=1.6, rrtp2_threshold=8.0)
    size_func_rrtp1 = make_size_func_combined(rrtp2_mult=1.6, rrtp2_threshold=8.0,
                                                rrtp1_mult=0.7, rrtp1_threshold=1.25)
    ...
    common_kwargs = dict(emergency=ei.DEFAULT_EMERGENCY, eval_risk=EVAL_RISK, fleet_risk=FLEET_RISK,
                          ...
                          use_any_rr=True, apply_instant_risk_cap=True, routing_field="rr_tp2")
    ...
    for ceiling in ceilings_arg:
        ...
        for lever in levers_arg:
            pop_v = populations[lever]
            tickers = sorted(pop_v["ticker"].unique())
            excluded_map_v = precompute_correlation_pairs(tickers, corr_matrix, CORR_TH_NEW)
            sf = size_func_rrtp1 if lever == "rrtp1sizing" else size_func_baseline
            t0 = time.time()
            df = run_propagated(pop_v, market_data, excluded_map_v, ceiling, seq, config,
                                 bb_threshold=bb_th, size_func=sf, **common_kwargs)
            row = summarize(df, lever, ceiling, bb_th, True)
            rows.append(row)
            print(f"[{lever:14s} plafond={ceiling:.0f}$] profit_moy={row['profit_moyen']:+,.0f}$ "
                  f"solde_neg={row['solde_negatif_annee4']:.2f}% hit_ceiling={row['hit_ceiling_pct']:.2f}% "
                  f"annee1<0={row['annee1_neg']:.2f}% ({time.time()-t0:.0f}s)")
            pd.DataFrame(rows).to_csv(f"chantier_b6_montecarlo_n{n_sims}_2026-08-19.csv", index=False)
