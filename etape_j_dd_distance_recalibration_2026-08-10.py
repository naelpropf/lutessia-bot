...
if __name__ == "__main__":
    ...
    volet = sys.argv[3] if len(sys.argv) > 3 else "both"  # 1 | 2 | both
    ...
    configs = []
    if volet in ("1", "both"):
        for entry in (0.10, 0.15):
            for reduction in (0.30, 0.40):
                configs.append((f"V1_entry{int(entry*100)}_red{int(reduction*100)}", entry, reduction, False))
    if volet in ("2", "both"):
        configs.append(("V2_entry20_red50_preunlock_only", 0.20, 0.5, True))

    rows = []
    for label, entry, reduction, pre_only in configs:
        for ceiling in ceilings_arg:
            ...
            df = run_propagated(pop, market_data, excluded_map, ceiling, seq, config, ei.DEFAULT_EMERGENCY,
                                 ...
                                 b_entry_frac=entry, b_reduction=reduction, pre_unlock_only=pre_only)
            ...
            print(f"[{label:32s} plafond={ceiling:.0f}$] profit={row['profit']:+,.0f}$ ruine={row['ruine']:.2f}% "
                  ...
                  f"({time.time()-t0:.0f}s)")
            pd.DataFrame(rows).to_csv(f"etape_j_dd_distance_recalibration_n{n_sims}.csv", index=False)
