"""
Etape Z (08/10 nuit, suite 10) : verifie si la trajectoire de capital
Fivers (The5%ers) depasse jamais le seuil de 500 000$ auquel le support
Fivers interdit le copytrade -- "total capital managed" = somme des
paliers + equity/profit cumule NON RETIRE sur les comptes Fivers actifs
(le modele ne simule pas d'evenements de retrait discrets : le split du
trader est credite en continu, trade par trade, dans state["reserve"]
(cf. engine_multiformat.py:337-341) SANS reduire acc["cumulative_since_
reset"], qui reste donc la meilleure proxy du solde reel de chaque
compte Fivers sur la plateforme -- palier de depart + P&L net cumule
depuis le dernier reset).

Copie EXACTE de etape_q_v2_plus_ftmo_gft_2026-08-10.py (config officielle
REF+V2 + FTMO-10% + GFT Goat Guard, growth_firms=("Blueberry","FTMO","GFT")
inchange -- Fivers reste a 4 comptes fixes, PAS le mecanisme extra-compte
deja teste et rejete en Etape Y). Instrumentation ajoutee : a chaque pas
de temps (apres traitement du trade courant sur tous les comptes actifs),
calcule fivers_total = somme(palier + cumulative_since_reset) sur les
comptes Fivers ACTIFS, et garde le MAX observe sur toute la duree du run
(aucune tolerance de duree -- un seul pic, meme d'un trade, compte).
...
FIVERS_COPYTRADE_LIMIT = 500000.0
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
    ...
    fivers_total_max = 0.0
    fivers_total_max_time = None
    ...
    for slot_idx, trade_idx in enumerate(order):
        ...
        # --- instrumentation Etape Z : capital Fivers total (palier + P&L
        # non retire) sur les comptes actifs, apres traitement du trade
        # courant sur tous les comptes.
        fivers_total_now = sum(a["palier"] + a["cumulative_since_reset"]
                                for a in accounts_by_group["Fivers"] if a["active"])
        if fivers_total_now > fivers_total_max:
            fivers_total_max = fivers_total_now
            fivers_total_max_time = now
    ...
    return {"final_net_split": combined_net(), "is_paid_cum": state["is_paid_cum"],
            ...
            "fivers_total_max": fivers_total_max,
            "fivers_total_max_month": (fivers_total_max_time / MONTH_SECONDS) if fivers_total_max_time else float("nan"),
            "breach_500k": fivers_total_max > FIVERS_COPYTRADE_LIMIT}
...
if __name__ == "__main__":
    ...
    for ceiling in ceilings_arg:
        ...
        pct_breach = df["breach_500k"].mean() * 100
        row = dict(ceiling=ceiling, n=len(df), pct_breach_500k=pct_breach,
                   mean_fivers_max=df["fivers_total_max"].mean(),
                   worst_fivers_max=df["fivers_total_max"].max(),
                   median_month_of_breach=df.loc[df["breach_500k"], "fivers_total_max_month"].median())
        rows.append(row)
        print(f"[plafond={ceiling:.0f}$] %runs_depassant_500k={pct_breach:.2f}% "
              f"fivers_total_max_moy={row['mean_fivers_max']:,.0f}$ "
              f"fivers_total_max_pire_cas={row['worst_fivers_max']:,.0f}$ "
              f"mois_median_depassement={row['median_month_of_breach']:.1f} "
              f"({time.time()-t0:.0f}s)")
        pd.DataFrame(rows).to_csv(f"etape_z_fivers_500k_n{n_sims}.csv", index=False)
