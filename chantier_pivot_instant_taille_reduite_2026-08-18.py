"""
Pivot Instant a taille reduite (2026-08-18) -- suite du correctif risque
Instant (1,5%/trade sur taille initiale, confirme et quantifie le 08/17,
n=300 : dominance S1.8 survit, ~33-37% du gain de profit s'evapore).

QUESTION : le compte pivot (1er compte Blueberry, jour0) ouvre a 25k$ dans
le moteur actuel (cout reel 800$ en Instant Elite) -- ecart significatif
avec le budget utilisateur prevu (165$, prix Prime 2-step). Un pivot en
Instant Elite a taille REDUITE (5k$/10k$, cout 200$/400$) beneficie-t-il
encore de l'avantage structurel d'Instant (finance des jour0) a moindre
cout d'entree, et cela suffit-il a faire basculer le verdict "Blueberry-
adaptatif" REJETE en 08/09 (`registre_parametres_projet.md` ligne 681 --
rejet motive par "trade au risque flotte des jour0, pas de phase
protectrice", sous l'ANCIENNE pile pre-08/12 et SANS le correctif de
risque 1,5% -- les deux conditions du rejet ont change depuis) ?

4 configs statiques pour le pivot (format+palier FIXES pour toute la vie
du compte, y compris apres casse -- pas de bascule dynamique bb_threshold
sur le pivot specifiquement, seulement sur les comptes extra/croissance
qui gardent le mecanisme S1.8 standard) :
  - Prime25k  (165$, cout d'ouverture reel)
  - InstantElite25k (800$)
  - InstantElite10k (400$)
  - InstantElite5k  (200$)
Risque 1,5%/trade sur taille initiale INTEGRE DES LE DEPART pour tout
compte Instant (toujours applique, pas un flag optionnel -- correction
deja validee, pas la peine de la revalider ici).

Prix reels fournis par l'utilisateur (PAS dans FORMATS["Blueberry_
InstantElite"]["price"], qui n'a que {25000: 800} -- ei.price_for()
retomberait sur une extrapolation lineaire fausse si on l'utilisait pour
5k/10k) : table locale PIVOT_PRICE ci-dessous, utilisee a la place de
ei.price_for() pour le pivot uniquement.

Copie de chantier_bb_instant_risk_cap_correction_2026-08-17.py (lui-meme
copie figee de chantier_cascade_combined_bb_switch_any_rr_2026-08-16.py,
S1.8), n=300 screening, 4 plafonds (960$/1000$/3000$/5000$), colonne
"cout_ouverture_reel" ajoutee -- c'est le critere prioritaire pour la
decision de lancement de la semaine, pas seulement le profit relatif.
"""
import random
import sys
import time
...
# <<< CORRECTIF RISQUE INSTANT : cap reel, TOUJOURS applique dans ce script
# (deja valide le 08/17, pas un flag optionnel ici).
BB_INSTANT_RISK_CAP = 1.5
BB_INSTANT_FORMAT_KEYS = {"Blueberry_InstantElite", "Blueberry_InstantLite"}

# <<< PIVOT TAILLE REDUITE : prix reels fournis par l'utilisateur, PAS dans
# FORMATS[...]["price"] (qui n'a que {25000: 800}/{25000: 165}) -- table
# locale utilisee pour le pivot uniquement, ei.price_for() reste inchange
# pour tous les autres comptes (extras/croissance).
PIVOT_PRICE = {
...
def pivot_price(fmt_key, palier):
    return PIVOT_PRICE[(fmt_key, palier)]


PIVOT_CONFIGS = {
    "Prime25k": ("Blueberry_Prime2Step", 25000.0),
    "InstantElite25k": ("Blueberry_InstantElite", 25000.0),
    "InstantElite10k": ("Blueberry_InstantElite", 10000.0),
    "InstantElite5k": ("Blueberry_InstantElite", 5000.0),
}
...
def process_trade_corr_swap_rr(acc, trade, now, fmt, state, risk_pct, market_data, excluded_map,
    ...
    de chantier_correlation_swap_2026-08-16.py -- 'rr_hybrid' omis ici (deja
    demontre strictement identique a 'rr' pur, S2.33, egalites exactes de RR
    quasi inexistantes)."""
    if not acc["active"]:
        return False
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
            ...
            pivot_fmt_key=BB_CLASSIC_KEY, pivot_palier=25000.0):
    ...
    for gname in FIRMS:
        ...
        # <<< PIVOT TAILLE REDUITE : format+palier FIXES pour le pivot,
        # PAS la bascule dynamique bb_choose_fmt_key() (celle-ci reste
        # utilisee pour les comptes extra/croissance, cf. process_extra_account).
        if gname == "Blueberry":
            fmt_key = pivot_fmt_key
            fmt = FORMATS[fmt_key]
            palier = pivot_palier
            cost = pivot_price(fmt_key, palier)
        else:
    ...
    for slot_idx, trade_idx in enumerate(order):
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                # APRES tout multiplicateur (DD-distance V2 inclus). Palier ne
                # scale jamais dans ce moteur -> "taille initiale fixe" deja
                # satisfait par construction, capper r suffit.
                ...
                if use_goat_guard:
                    ...
                    if use_bb_reset:
                        ...
                        if gname == "Blueberry":
                            if acc.get("_is_pivot"):
                                # <<< PIVOT TAILLE REDUITE : reste dans le
                                # meme format+palier pour toute sa vie, pas
                                # de bascule dynamique bb_choose_fmt_key().
                                new_fmt_key = acc["_fmt_key"]
                                new_fmt = FORMATS[new_fmt_key]
                                cost = pivot_price(new_fmt_key, acc["base_palier"])
                            else:
...
def summarize(df, label, ceiling, bb_threshold, use_any_rr, cout_ouverture_reel):
    ...
    return dict(config=label, ceiling=ceiling, bb_threshold=bb_threshold, use_any_rr=use_any_rr, n=len(df),
                cout_ouverture_reel=cout_ouverture_reel,
...
if __name__ == "__main__":
    ...
    # (label, fmt_key, palier, cout_ouverture_reel) -- risque 1,5% Instant
    # toujours actif (apply_instant_risk_cap=True partout, correction deja
    # validee le 08/17, pas un flag optionnel ici)
    PIVOT_TESTS = [
    ...
    for ceiling in ceilings_arg:
        ...
        for label, fmt_key, palier, cout in PIVOT_TESTS:
            ...
            row = summarize(df, label, ceiling, bb_th, True, cout)
            rows.append(row)
            ...
            pd.DataFrame(rows).to_csv(f"chantier_pivot_instant_taille_reduite_n{n_sims}_2026-08-18.csv", index=False)
