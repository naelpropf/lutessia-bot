"""
Etape AJ (08/11, priorite 2 -- RUN E) : teste si le simple RYTHME de
versement par paquets (vs flux continu) cree un probleme de liquidite EN
LUI-MEME, independamment de toute casse/forfeiture. Copie de
etape_ai_payout_cadence_calibration_2026-08-11.py (cadence par firm/
occurrence deja corrigee, INCHANGEE ici), UN seul changement : la casse
est desactivee au niveau CASH -- plus de cout de reouverture, plus
d'indisponibilite, plus de forfeit de pending_payout sur breach. Le
compte reste actif en continu (le tracking DD interne de
process_trade_mf, code partage, continue de se reinitialiser normalement
au moment d'un breach -- non touche). state["total_breaks"] reste
incremente pour instrumentation (compter les breach qui SERAIENT
survenus) mais n'a plus aucune consequence cash/temps.

Objectif : si hit_ceiling/solde_negatif_annee4 apparaissent encore dans
ce run, la cause ne peut PAS etre une casse ou une forfeiture -- ce
serait alors uniquement le rythme "par paquets" (creux de tresorerie
entre deux flush du cycle 14j/3j/1,5j) qui cree le probleme.
    ...
    for slot_idx, trade_idx in enumerate(order):
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                # Run E (08/11, priorite 2) : casse desactivee au niveau
                # CASH uniquement -- pas de cout de reouverture, pas
                # d'indisponibilite, pas de forfeit de pending_payout.
                # Le compte reste actif (process_trade_mf a deja reinitialise
                # son tracking DD interne normalement au moment du breach,
                # code partage inchange). Objectif : isoler si le seul
                # rythme de versement par paquets cree hit_ceiling/solde_
                # negatif_annee4 en l'absence de TOUT cout lie a une casse.
                if broke:
...
if __name__ == "__main__":
    ...
    for ceiling in ceilings_arg:
        ...
        row = summarize(df, "RUN_E_sans_casse_2026-08-11", ceiling)
        rows.append(row)
        print(f"[plafond={ceiling:.0f}$] profit_moyen={row['profit_moyen']:+,.0f}$ "
              ...
              f"annee1<0={row['annee1_neg']:.2f}% breaks_totaux(instrumentation)={df['total_breaks'].mean():.1f} "
              f"({time.time()-t0:.0f}s)")
        pd.DataFrame(rows).to_csv(f"etape_aj_run_e_no_casse_n{n_sims}.csv", index=False)
