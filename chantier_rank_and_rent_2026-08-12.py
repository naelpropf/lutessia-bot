"""
Chantier rank-and-rent, Section 3 (08/12) : compare 3 scenarios de tresorerie
pour la decision "40$ vers rank-and-rent, viser 5-10k$ de tresorerie de
securite" -- (A) attendre que le rank-and-rent ait produit le plafond cible
avant de demarrer, (B) demarrer maintenant a 960$ (960 = 1000-40) et y
rester indefiniment, (C) demarrer maintenant a 960$ puis basculer au
plafond cible des que le rank-and-rent rapporte. Sensibilite sur le delai
(3/6/9/12 mois). Base = chantier_ceiling_sweep_2026-08-12.py (meme moteur,
meme reference RR1,35/corr0,80/BB7j generalise), etendu avec 2 capacites
nouvelles :

1. `ceiling_schedule` (liste de (t_debut_secondes, valeur), triee) remplace
   le parametre scalaire `ceiling` -- handle_cost_hybrid lit desormais
   state["ceiling"], recalcule une fois par slot via _ceiling_at(now).
   Modelise le scenario C (bascule de plafond a une date donnee, sans
   apport de capital personnel supplementaire au-dela du saut lui-meme --
   la mecanique de plafond existante gere deja "argent disponible jusqu'a
   ce montant", donc augmenter le plafond a la date T EQUIVAUT exactement
   a injecter la difference (cible-960) dans le systeme a la date T, sans
   modification de logique supplementaire necessaire).
2. `start_delay_seconds` : si fourni, ne conserve QUE les trades dont le
   timestamp original est >= ce delai, et retranche ce delai a tous les
   timestamps conserves -- modelise le scenario A (aucune activite avant
   la date de depart, horizon actif reduit d'autant, plutot qu'un horizon
   de 4 ans complet dont on gelerait artificiellement le debut).

Scenario B est un cas particulier deja couvert par chantier_ceiling_sweep
(ligne ceiling=960$) -- PAS reexecute ici, reutilise directement.
...
def _ceiling_at(schedule, now):
    """schedule : liste de (t_debut, valeur) TRIEE par t_debut croissant.
    Retourne la valeur active a l'instant `now` (dernier palier dont
    t_debut <= now)."""
    val = schedule[0][1]
    for t, v in schedule:
        if t <= now:
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
            ...
            ftmo_discount=False, gft_goat_guard=False, payout_cycle=False, ceiling_schedule=None):
    fmt_by_firm = {g: FORMATS[k] for g, k in format_by_firm.items()}
    ceiling_schedule = ceiling_schedule if ceiling_schedule is not None else [(0.0, ceiling)]
    ...
    def base_palier_cost(gname):
        ...
        return palier, price_for_bb(gname, format_by_firm[gname], palier, ceiling_schedule[0][1])
    ...
    def handle_cost_hybrid(cost, pending_list, pending_key, on_success):
        ...
        state["reserve"] = 0.0
        room = max(0.0, state["ceiling"] - state["real_cash_paid"])
        if shortfall <= room:
    ...
    def process_extra_account(now):
        ...
        for gname in ei.GROWTH_FIRMS_EXTRA:
            ...
            if current_capital + unit_palier > ei.FIRM_CAPITAL_CAP[gname]:
                continue
            extra_cost = price_for_bb(gname, format_by_firm[gname], unit_palier, state["ceiling"])
    ...
    for slot_idx, trade_idx in enumerate(order):
        ...
        state["_now"] = now
        state["ceiling"] = _ceiling_at(ceiling_schedule, now)
        ...
        while i < len(tax_events):
            ...
            if t_ev > now:
                ...
                continue
            tax_events.pop(i)
            handle_tax_payment(amt, state, state["ceiling"], now, pending_reopen, pending_group_open)
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                if use_payout_cycle and acc["active"] and acc["last_payout_time"] is not None \
                        and now - acc["last_payout_time"] >= payout_cycle_days(gname, acc["_first_payout_done"], state["ceiling"]) * 86400:
                ...
                if use_goat_guard:
                    ...
                    if use_bb_reset:
                        ...
                        if downgrade_active() and gname == ei.STARTER:
                            ...
                            cost = price_for_bb(gname, format_by_firm[gname], acc["palier"], state["ceiling"])
...
def run_propagated(pop, market_data, excluded_map, ceiling, seq_grouped, format_by_firm, emergency,
                    ...
                    ceiling_schedule=None, start_delay_seconds=None):
    ...
    for _ in range(n_sims):
        ...
        if start_delay_seconds:
            # Scenario A (attendre) : rien ne se passe avant le delai --
            # ne conserve que les trades posterieurs, horloge remise a zero
            # (t=0 du run = date reelle de demarrage), horizon actif reduit
            # d'autant plutot qu'un horizon de 4 ans gele artificiellement.
            kept = [(t, s - start_delay_seconds) for t, s in zip(raw_trades, raw_slots) if s >= start_delay_seconds]
            raw_trades = [t for t, s in kept]
            raw_slots = [s for t, s in kept]
        ...
        res = run_one(raw_trades, raw_slots, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
                      ...
                      ceiling_schedule=ceiling_schedule)
        rows.append(res)
...
DAYS_PER_MONTH_APPROX = 30.4375  # 365.25/12, coherent avec DAYS_PER_MONTH importe ailleurs

if __name__ == "__main__":
    n_sims = int(sys.argv[1]) if len(sys.argv) > 1 else 300
    target_ceiling = float(sys.argv[2]) if len(sys.argv) > 2 else 5000.0
    delays_months = [float(x) for x in sys.argv[3].split(",")] if len(sys.argv) > 3 else [3.0, 6.0, 9.0, 12.0]
    START_CEILING = 960.0

    t_start = time.time()
    import rr_threshold_test as rrt
    print(f"[verif] HIST_PATH = {rrt.HIST_PATH}, cible={target_ceiling:.0f}$, depart={START_CEILING:.0f}$")
    ...
    rows = []
    for delay_m in delays_months:
        delay_seconds = delay_m * DAYS_PER_MONTH_APPROX * DAY_SECONDS

        t0 = time.time()
        df_a = run_propagated(pop, market_data, excluded_map, target_ceiling, seq, config, ei.DEFAULT_EMERGENCY,
                               EVAL_RISK, FLEET_RISK, GFT_EVAL_RISK, ei.FINAL_RESERVE_SHARE,
                               ei.EXTRA_THRESHOLD_MULT, n_sims=n_sims, seed=9999,
                               start_delay_seconds=delay_seconds, **COMMON_KW)
        row_a = summarize(df_a, f"A_attendre_{delay_m:.0f}mois", target_ceiling)
        rows.append(row_a)
        print(f"[A attendre {delay_m:.0f}mois -> cible {target_ceiling:.0f}$] "
              f"profit_moyen={row_a['profit_moyen']:+,.0f}$ profit_median={row_a['profit_median']:+,.0f}$ "
              f"solde_negatif_annee4={row_a['solde_negatif_annee4']:.2f}% hit_ceiling_pct={row_a['hit_ceiling_pct']:.2f}% "
              f"annee1<0={row_a['annee1_neg']:.2f}% ({time.time()-t0:.0f}s)")

        t0 = time.time()
        schedule_c = [(0.0, START_CEILING), (delay_seconds, target_ceiling)]
        df_c = run_propagated(pop, market_data, excluded_map, START_CEILING, seq, config, ei.DEFAULT_EMERGENCY,
                               EVAL_RISK, FLEET_RISK, GFT_EVAL_RISK, ei.FINAL_RESERVE_SHARE,
                               ei.EXTRA_THRESHOLD_MULT, n_sims=n_sims, seed=9999,
                               ceiling_schedule=schedule_c, **COMMON_KW)
        row_c = summarize(df_c, f"C_demarre960_bascule_{delay_m:.0f}mois", target_ceiling)
        rows.append(row_c)
        print(f"[C demarre {START_CEILING:.0f}$ -> bascule {delay_m:.0f}mois -> {target_ceiling:.0f}$] "
              f"profit_moyen={row_c['profit_moyen']:+,.0f}$ profit_median={row_c['profit_median']:+,.0f}$ "
              f"solde_negatif_annee4={row_c['solde_negatif_annee4']:.2f}% hit_ceiling_pct={row_c['hit_ceiling_pct']:.2f}% "
              f"annee1<0={row_c['annee1_neg']:.2f}% ({time.time()-t0:.0f}s)")

        pd.DataFrame(rows).to_csv(f"chantier_rank_and_rent_n{n_sims}.csv", index=False)
