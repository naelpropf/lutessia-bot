"""
Cascade groupee 2026-08-16 : integration simultanee de deux mecanismes
valides isolement --
  (1) bascule Blueberry Instant (chantier_blueberry_switch_2026-08-15.py,
      registre_parametres_projet.md S7.1) : format Blueberry par compte,
      decide a l'ouverture/reouverture selon state["reserve"] vs bb_threshold.
  (2) any-RR (chantier_correlation_swap_2026-08-16.py, registre_strategie_
      trading.md S2.33) : au blocage-correlation avec exactement une position
      ouverte conflictuelle, admet le signal bloque si son RR planifie
      (rr_tp1) est strictement superieur a celui de la position occupante.

Copie figee de chantier_blueberry_switch_2026-08-15.py (deja lui-meme copie
figee de chantier_position_cap_2026-08-15.py), points <<< CHANTIER GROUPE
ajoutes pour integrer any-RR par-dessus la bascule Blueberry deja presente.
Objectif : detecter si les deux mecanismes se renforcent, sont neutres, ou
se cannibalisent (any-RR change quels trades entrent en flotte -> peut
changer le rythme d'accumulation de reserve -> peut changer le point de
bascule optimal de Blueberry Instant).
"""
import random
import sys
import time
...
if __name__ == "__main__":
    ...
    for ceiling in ceilings_arg:
        ...
        df_combined = run_propagated(pop, market_data, excluded_map, ceiling, seq, config,
                                      bb_threshold=bb_th, use_any_rr=True, **common_kwargs)
        row = summarize(df_combined, f"COMBINE (bb_th={bb_th:.0f} + any-RR)", ceiling, bb_th, True)
        rows.append(row)
        print(f"[COMBINE bb={bb_th:.0f} plafond={ceiling:.0f}$] profit_moy={row['profit_moyen']:+,.0f}$ "
              f"solde_neg={row['solde_negatif_annee4']:.2f}% hit_ceiling={row['hit_ceiling_pct']:.2f}% "
              f"annee1<0={row['annee1_neg']:.2f}% bb_instant={row['bb_instant_opens_moy']:.2f} "
              f"bb_classic={row['bb_classic_opens_moy']:.2f} corr_admits={row['corr_swap_admits_moy']:.2f} "
              f"({time.time()-t0:.0f}s)")

        pd.DataFrame(rows).to_csv(f"chantier_cascade_combined_n{n_sims}_2026-08-16.csv", index=False)
