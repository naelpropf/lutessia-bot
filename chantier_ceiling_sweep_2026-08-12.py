"""
Chantier rank-and-rent -> plafond personnel (08/12) : balayage du plafond
personnel {960, 1000, 2000, 3000, 5000, 7500, 10000} sous la nouvelle
reference RR>=1,35 + correlation 0,80 (registre_parametres_projet.md
§2.62-2.63, post-cascade). Base = etape_ar_run_f_rr135_corr080_2026-08-12
.py (lui-meme copie de etape_ao_run_f_cout_reel_2026-08-11.py), avec le
seuil BB7j GENERALISE en ceiling>=BB_7J_CEILING_THRESHOLD (au lieu du set
exact {3000.0}) -- meme convention et meme justification que Section D de
structure_pistes_2026-08-11.py (decision #16, extrapolation raisonnable
pour les plafonds intermediaires/superieurs jamais couverts par la decision
originale, qui ne tranchait que 1000$/3000$). Ancien balayage Section D
(§2.48) datait de RR1,25/corr0,6 -- potentiellement obsolete, refait ici a
neuf sous la base actuelle.

--- Docstring original (etape_ar) conserve ci-dessous pour tracabilite ---
...
if __name__ == "__main__":
    ...
    for ceiling in ceilings_arg:
        ...
        bb_label = "RUN_F_BB7j" if ceiling >= BB_7J_CEILING_THRESHOLD else "RUN_C_BB14j_defaut"
        ...
        pd.DataFrame(rows).to_csv(f"chantier_ceiling_sweep_n{n_sims}.csv", index=False)
