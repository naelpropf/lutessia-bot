"""
Session 08/08 (suite, jour 2) -- Etape 3 : verifie l'ABSENCE d'effet de
cascade cree par le deblocage echelonne a seuils bas (Etape 2). Le
screening n=300 de l'Etape 2 a deja montre empiriquement qu'empiler
plusieurs seuils trop bas simultanement (2,5/5/10/15k et 2,5/10/15/20k)
fait bondir la ruine de ~0-1% (baseline) a 2,7-4,3% -- un vrai mode de
ruine nouveau, pas du bruit. Ce script instrumente PRECISEMMENT ce
mecanisme sur le(s) meilleur(s) combo(s) retenu(s) (qui eux N'ONT PAS
montre ce symptome en profit/ruine agrege) pour confirmer qu'aucun effet
pervers residuel ne se cache dans les moyennes :
  1. Taux de casse a J+30 / J+60 apres CHAQUE ouverture de compte (initiale
     ou reouverture), compare a l'ancien systeme (gate unique 30k).
  2. Reserve minimale atteinte durant les 6 premiers mois, sur CHAQUE run
     (pas seulement en moyenne) -- cherche des cas de gel quasi-total
     (reserve proche de 0 pendant une longue duree) qui n'existeraient pas
     dans l'ancien systeme.
Reprend extra_account_v4_multi_stagger.py a l'identique (meme mecanisme de
trigger a seuils par firm), instrumente avec last_open_time par compte et
un suivi de la reserve min sur les 6 premiers mois.
"""
import random
import time
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped,
    ...
    for slot_idx, trade_idx in enumerate(order):
        ...
        if now <= SIX_MONTHS_SECONDS:
            ...
            if state["total_opens"] > 1:  # au moins un compte ouvert au-dela de Blueberry day0
                reserve_min_after_first_unlock = min(reserve_min_after_first_unlock, state["reserve"])
        if reserve_at_6mo is None and now >= SIX_MONTHS_SECONDS:
            reserve_at_6mo = state["reserve"]
    ...
    return {
        ...
        "breaks_within_30d": state["breaks_within_30d"],
        ...
        "reserve_at_6mo": reserve_at_6mo if reserve_at_6mo is not None else state["reserve"],
    }
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        ...
        combos = [(1000., 15000., 25000., 25000.)]  # config finale verrouillee (session 08/08 5-6)
    ...
    for ceiling in (1000.0, 3000.0):
        ...
        for label, (t_f, t_v, t_g, t_n) in configs:
            ...
            rmau = df["reserve_min_after_first_unlock"].dropna()
            row = dict(ceiling=ceiling, config=label, profit=net.mean(), ruine=(net < 0).sum()/len(df)*100,
                       ...
                       reserve_at_6mo_mean=df["reserve_at_6mo"].mean(),
                       reserve_min_after_unlock_mean=rmau.mean() if len(rmau) else float("nan"),
                       reserve_min_after_unlock_worst=rmau.min() if len(rmau) else float("nan"),
                       pct_runs_frozen_post_unlock=(rmau < 100.0).mean() * 100 if len(rmau) else float("nan"))
            rows.append(row)
            print(f"[{label}] profit={row['profit']:+,.0f}$ | ruine={row['ruine']:.2f}% | "
                  f"P(annee1<0)={row['annee1_neg']:.2f}% | casse<=30j={break_rate_30d:.2f}% | "
                  f"casse<=60j={break_rate_60d:.2f}% | reserve@6mo moy={row['reserve_at_6mo_mean']:,.0f}$ | "
                  f"reserve_min_post_unlock moy={row['reserve_min_after_unlock_mean']:,.0f}$ "
                  f"pire={row['reserve_min_after_unlock_worst']:,.0f}$ | %runs geles apres deblocage(<100$)="
                  f"{row['pct_runs_frozen_post_unlock']:.1f}% ({time.time()-t0:.0f}s)")
        pd.DataFrame(rows).to_csv("extra_account_v4_cascade_check.csv", index=False)
