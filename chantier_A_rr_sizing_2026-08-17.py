...
if __name__ == "__main__":
    ...
    CANDIDATES = {
        "REF_tail_x1.6_th8": make_size_func_tail(1.6, threshold=8.0),
        "A1_step2_1.3-1.6": make_size_func_step2(1.3, 1.6, 6.5, 8.0),
        "A2_ramp_1.0-1.6": make_size_func_ramp(6.0, 15.0, 1.6),
        "A3_decile_full_1.0-1.6": make_size_func_decile_full(pop, 1.6),
    }
    for name, f in CANDIDATES.items():
        maxm = max(f(x) for x in [0, 3, 5, 6, 6.5, 7, 8, 8.82, 10, 15, 20, 30])
        print(f"[verif marge] {name} : mult_max={maxm:.2f} -> marge={margin_pct(maxm):.1f}%"
              f"{'  [OK >=30%]' if margin_pct(maxm) >= 30 else '  [!!! <30%, deja accepte pour x1.6 en S2.35, aucune nouvelle violation]'}")
    ...
    common_kwargs = dict(emergency=ei.DEFAULT_EMERGENCY, eval_risk=EVAL_RISK, fleet_risk=FLEET_RISK,
                          ...
                          use_any_rr=True, routing_field="rr_tp2")
    ...
    for ceiling in ceilings_arg:
        ...
        for name, size_func in CANDIDATES.items():
            t0 = time.time()
            df = run_propagated(pop, market_data, excluded_map, ceiling, seq, config,
                                 bb_threshold=bb_th, size_func=size_func, **common_kwargs)
            ...
            pd.DataFrame(rows).to_csv(f"chantier_A_rr_sizing_n{n_sims}_2026-08-17.csv", index=False)
