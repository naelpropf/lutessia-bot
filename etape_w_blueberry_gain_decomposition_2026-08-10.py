"""
Etape W (08/10 nuit, suite 6) : decomposition du gain majeur (+16,4-16,5%)
mesure a l'Etape V suite a la correction du cap Blueberry
(FIRM_MAX_ACCOUNTS None, FIRM_CAPITAL_CAP 400k). Copie EXACTE de
etape_q_v2_plus_ftmo_gft_2026-08-10.py (config "REF_V2 + (a)+(b) combine"
uniquement -- V2 sizing + FTMO-10% + GFT Goat Guard, la reference
officielle), instrumentation ajoutee :

- extra_opened_<firm> pour Blueberry/FTMO/GFT (les seules firms avec
  mecanisme extra-compte, GROWTH_FIRMS_EXTRA) -- repond a la question (a).
- profit_by_firm_<firm> (net final = total_funded_pnl - total_fees_paid,
  agrege sur tous les comptes de la firm y compris starter+extras+resets)
  pour les 5 firms -- repond a la question (b) : le gain est-il concentre
  chez Blueberry seul ou reparti/detourne d'ailleurs ?

Meme seed (9999), meme mecanique -- pas de nouveau tirage aleatoire, juste
plus de visibilite sur les runs deja produits a l'Etape V.
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
    ...
    def net_by_firm(gname):
        return sum(a["total_funded_pnl"] - a["total_fees_paid"] for a in accounts_by_group[gname])
    ...
    result = {"final_net_split": combined_net(), "is_paid_cum": state["is_paid_cum"],
              ...
              "n_funding_events": len(state["funding_delays"])}
    for g in ("Blueberry", "FTMO", "GFT"):
        result[f"extra_opened_{g}"] = state["extra_accounts_opened"][g]
    for g in FIRMS:
        result[f"net_by_firm_{g}"] = net_by_firm(g)
    return result
...
if __name__ == "__main__":
    ...
    for ceiling in ceilings_arg:
        ...
        row = dict(tag=tag, ceiling=ceiling, n=len(df), profit=net.mean())
        for g in ("Blueberry", "FTMO", "GFT"):
            row[f"mean_extra_{g}"] = df[f"extra_opened_{g}"].mean()
        for g in FIRMS:
            row[f"mean_net_{g}"] = df[f"net_by_firm_{g}"].mean()
        rows.append(row)
        extra_str = " ".join(f"{g}={row[f'mean_extra_{g}']:.2f}" for g in ("Blueberry", "FTMO", "GFT"))
        net_str = " ".join(f"{g}={row[f'mean_net_{g}']:+,.0f}$" for g in FIRMS)
        print(f"[{tag:8s} plafond={ceiling:.0f}$] profit={row['profit']:+,.0f}$ extra_moy[{extra_str}] "
              f"net_par_firm[{net_str}] ({time.time()-t0:.0f}s)")
        pd.DataFrame(rows).to_csv(f"etape_w_decomposition_{tag}_n{n_sims}.csv", index=False)
