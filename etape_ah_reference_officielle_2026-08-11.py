"""
Etape AH (08/11) : RECONSTRUCTION DE LA REFERENCE OFFICIELLE en un seul
passage, integrant toutes les corrections validees cette session. Copie de
etape_ad_payout_cycle_2026-08-10.py (moteur payout/forfeiture inchange),
DEUX ajouts seulement :

1. state["hit_ceiling"] est remonte dans le dict de resultat (metrique de
   risque distincte, jamais rapportee avant ce jour -- cf. investigation
   08/11, ni "ruine" ni "hit_ceiling" ne sont un etat irreversible, la
   simulation continue dans les deux cas).
2. summarize() renomme "ruine" (net<0 an4) en "solde_negatif_annee4"
   (decision #15 -- renommage de REPORTING uniquement, le moteur de calcul
   ci-dessous est inchange, seule la couche de presentation change de nom)
   et ajoute "hit_ceiling_pct" cote a cote, jamais en remplacement.

Config de reference reconstruite (payout_cycle=True active -- l'utilisateur
confirme retirer manuellement des le deblocage du cycle 14j, le modele
automatique est donc un proxy valide de sa politique reelle) :
population 721 trades (defaut HIST_PATH depuis la correction 08/11),
eval=1,25%/flotte=1,90%, cap Blueberry corrige, V2 (sizing DD
pre-deblocage), combo FTMO-10%/GFT Goat Guard, payout_cycle=True.
...
if __name__ == "__main__":
    ...
    for ceiling in ceilings_arg:
        ...
        row = summarize(df, "REFERENCE_OFFICIELLE_2026-08-11", ceiling)
        rows.append(row)
        ...
        pd.DataFrame(rows).to_csv(f"etape_ah_reference_officielle_n{n_sims}.csv", index=False)
