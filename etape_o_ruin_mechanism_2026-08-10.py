"""
Etape O (08/10 nuit) : investigation du VRAI mecanisme de ruine BB+GFT a
plafond 1000$, suite a la decomposition Etape K qui a exclu la casse
rapprochee (3/64) et la double-reussite (0/600) sans identifier de
mecanisme positif de remplacement. Copie de etape_k_piste_a_decomposition_
2026-08-10.py, STARTERS generalise en parametre (solo_BB vs BB+GFT
comparables depuis le meme script), instrumentation ajoutee :

- cash_at_fleet_first_funded : real_cash_paid au moment du tout premier
  evenement de financement de la flotte (point a -- cout total avant
  premier financement).
- reserve_min_ever : minimum de state["reserve"] sur TOUTE la duree du
  run (pas juste les 6 premiers mois comme dans d'autres scripts).
- diverted_while_bb_waiting : $ effectivement preleves sur la reserve
  partagee pour ouvrir une AUTRE firm (FTMO/Fivers/FundedNext/GFT-propre)
  au moment precis ou Blueberry a une relance en attente non payee dans
  pending_reopen (point b -- trace directe, pas deduction).
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
    ...
    state = {"reserve": 0.0, "ever_funded": False, "real_cash_paid": active0_cost, "total_breaks": 0,
             ...
             "break_times": {g: [] for g in TRACKED}, "first_funded": {},
             "cash_at_fleet_first_funded": None, "reserve_min_ever": 0.0,
             "diverted_while_bb_waiting": 0.0}
    ...
    for slot_idx, trade_idx in enumerate(order):
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                if just_funded and gname in TRACKED and gname not in state["first_funded"]:
                    state["first_funded"][gname] = now
                ...
                if broke:
                    ...
                    if gname in TRACKED and acc is accounts_by_group[gname][0]:
                        state["break_times"][gname].append(now)
        ...
        for group_names, trig, thresh, is_final in pending_group_trigger:
            ...
            if state["group_funded_count"] >= n_req and state["reserve"] >= thresh:
                for gname in group_names:
                    ...
                    if "Blueberry" in STARTERS and gname != "Blueberry":
                        bb_acc0 = accounts_by_group["Blueberry"][0]
                        bb_pending = any(p["key"] == id(bb_acc0) for p in pending_reopen)
                        if bb_pending:
                            state["diverted_while_bb_waiting"] += min(cost0, state["reserve"])
    ...
    bb_breaks = state["break_times"]["Blueberry"]
    gft_breaks = state["break_times"]["GFT"]
    min_break_gap_days = None
    last_break_gap_days = None
    if bb_breaks and gft_breaks:
        min_break_gap_days = min(abs(b - g) for b in bb_breaks for g in gft_breaks) / 86400.0
        last_break_gap_days = abs(bb_breaks[-1] - gft_breaks[-1]) / 86400.0

    bb_fund = state["first_funded"].get("Blueberry")
    gft_fund = state["first_funded"].get("GFT")
    fund_gap_days = (abs(bb_fund - gft_fund) / 86400.0) if (bb_fund is not None and gft_fund is not None) else None
    ...
    return {"final_net_split": combined_net(), "is_paid_cum": state["is_paid_cum"],
            ...
            "min_break_gap_days": min_break_gap_days if min_break_gap_days is not None else float("nan"),
            "last_break_gap_days": last_break_gap_days if last_break_gap_days is not None else float("nan"),
            "n_bb_breaks": len(bb_breaks), "n_gft_breaks": len(gft_breaks),
            "bb_ever_funded": bb_fund is not None, "gft_ever_funded": gft_fund is not None,
            "fund_gap_days": fund_gap_days if fund_gap_days is not None else float("nan"),
            ...
            "diverted_while_bb_waiting": state["diverted_while_bb_waiting"]}
...
if __name__ == "__main__":
    ...
    configs = [("solo_BB", ("Blueberry",)), ("BB_GFT", ("Blueberry", "GFT"))]
    for label, starters in configs:
        t0 = time.time()
        ...
        df.to_csv(f"etape_o_ruin_mechanism_{label}_n{n_sims}_c{int(ceiling)}.csv", index=False)
        print(f"[{label}] termine en {time.time()-t0:.0f}s -> etape_o_ruin_mechanism_{label}_n{n_sims}_c{int(ceiling)}.csv")
