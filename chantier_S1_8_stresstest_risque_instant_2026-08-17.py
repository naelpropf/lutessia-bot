...
if __name__ == "__main__":
    ...
    # n=600, sur le correctif risque Instant (cap reel 1,5%/trade), meme
    # protocole que chantier_stresstest_pisteAB_2026-08-17.py. Objectif :
    # verifier que la dominance COMBINE-corrige vs REF ne s'inverse dans
    # AUCUNE sous-periode, aux 4 plafonds, avant de lancer le n=600 officiel.
    ...
    # 2 plafonds representatifs (regime bb_th=5000 pour 960/1000$, regime
    # bb_th=0 pour 3000/5000$ -- deja identiques au $ pres a n=300, cf.
    # tableau S1.8 officiel) suffisent a couvrir les 2 regimes distincts.
    ...
    for ceiling in CEILINGS_TESTED:
        ...
        for sp_name, sp_pop in subperiods.items():
            ...
            df_ref = run_propagated(sp_pop, market_data, excluded_map, ceiling, seq, config,
                                     bb_threshold=float("inf"), use_any_rr=False,
                                     apply_instant_risk_cap=False, **common_kwargs)
            df_cand = run_propagated(sp_pop, market_data, excluded_map, ceiling, seq, config,
                                      bb_threshold=bb_th, use_any_rr=True,
                                      apply_instant_risk_cap=True, **common_kwargs)
            row_ref = summarize(df_ref, "REF", ceiling, None, False)
            row_cand = summarize(df_cand, "COMBINE_corrige", ceiling, bb_th, True)
            ...
            direction = "profit MIEUX (dominance tient)" if d_profit_pct > 0 else "profit PIRE (INVERSION)"
            print(f"  [{sp_name}, n={n_sims}] REF profit={row_ref['profit_moyen']:+,.0f}$ hit_ceiling={row_ref['hit_ceiling_pct']:.2f}% | "
                  f"COMBINE_corrige profit={row_cand['profit_moyen']:+,.0f}$ ({d_profit_pct:+.2f}%, {direction}) "
                  f"hit_ceiling={row_cand['hit_ceiling_pct']:.2f}% (delta={d_hc:+.2f}pt) annee1<0 delta={d_a1:+.2f}pt "
                  f"({time.time()-t0:.0f}s)")
            rows.append(dict(ceiling=ceiling, subperiod=sp_name, n=n_sims,
            ...
            pd.DataFrame(rows).to_csv("chantier_S1_8_stresstest_risque_instant_2026-08-17.csv", index=False)
