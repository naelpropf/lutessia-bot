"""
Chantier 1, Section 2 (08/12) : le seuil de correlation inter-positions
(CORR_TH=0.6, point_liquidity_rules.py:34, utilise partout dans le moteur
flotte via monte_carlo_simulation.precompute_correlation_pairs) n'a jamais ete
balaye en tant que tel -- 0.6 est une valeur historique jamais optimisee.

Balaye {0.4, 0.5, 0.6 (reference), 0.7, 0.8} sur le moteur flotte officiel
(Option A/Prime, config Run C, copie de etape_ai_payout_cadence_calibration_
2026-08-11.py), n=300, deux plafonds, regle JPY-JPY INCHANGEE (active, comme
la reference -- seul le seuil de correlation numerique varie, question
orthogonale a la Section 1 de ce meme chantier).
...
if __name__ == "__main__":
    ...
    for thr in THRESHOLDS:
        ...
        label = f"corr_{thr:.1f}" + ("_ref" if thr == 0.6 else "")
        for ceiling in ceilings_arg:
            ...
            pd.DataFrame(rows).to_csv(f"chantier1_corr_threshold_sweep_n{n_sims}.csv", index=False)
