"""
Etape E, annee1<0, round 4 (08/09) -- premiere implementation d'un
mecanisme REEL de reset moins cher, trouve au point 1 de la session
precedente (etape_e_annee1_levers3_2026-08-09.md) et confirme par le
support Blueberry (verbatim, confiance elevee) :

  A la casse d'un compte Blueberry DEJA FINANCE, reset disponible a 2x le
  prix du challenge ORIGINAL (taille d'achat, pas taille actuelle -- sans
  objet ici, aucune croissance individuelle n'existe dans ce moteur, cf.
  notes de conception etape_e_fleet_integration.py), qui reprend
  DIRECTEMENT au niveau finance (saute P1/P2). Usage UNIQUE a vie par
  compte -- le compte issu du reset ne peut plus jamais en beneficier.
  Fenetre de 7 jours pour reclamer : sans effet ici, le moteur rouvre deja
  immediatement des que la tresorerie le permet (comme tous les autres
  rachats du projet) -- non modelisee.

  Exclusions verifiees NON pertinentes ici : "instant funding" (REF
  utilise Blueberry_Prime2Step, qui a des phases, pas instant) et "scaled"
  (aucune croissance individuelle de palier n'existe dans le moteur,
  aucun compte Blueberry ne peut donc jamais etre "scaled"). Le levier
  s'applique a TOUS les comptes Blueberry actifs (compte de depart +
  comptes supplementaires), aucune distinction necessaire.

  Une casse en phase EVALUATION (P1/P2) n'est PAS concernee -- reste un
  restart normal (repurchase complet), conformement a la reponse support
  qui parle specifiquement de "funded account breaches".

Copie de etape_e_fleet_integration.run_one, augmentee. N'importe pas
run_one directement, ne touche a aucun script existant.
"""
import random
import time
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
    ...
    for slot_idx, trade_idx in enumerate(order):
        ...
        if now <= SIX_MONTHS_SECONDS:
            reserve_min_6mo[0] = min(reserve_min_6mo[0], state["reserve"])
            if state["total_opens"] > 1:
                reserve_min_after_unlock[0] = min(reserve_min_after_unlock[0], state["reserve"])
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                if broke:
                    ...
                    use_bb_reset = (blueberry_reset_enabled and gname == "Blueberry" and was_funded
                                     and not acc["_reset_used"])
                    if use_bb_reset:
    ...
    return {"final_net_split": combined_net(), "is_paid_cum": state["is_paid_cum"],
            ...
            "reserve_min_6mo": reserve_min_6mo[0] if reserve_min_6mo[0] != float("inf") else 0.0,
            "reserve_min_after_unlock": (reserve_min_after_unlock[0]
                                          if reserve_min_after_unlock[0] != float("inf") else None),
...
def run_propagated(pop, market_data, excluded_map, ceiling, seq_grouped, format_by_firm, emergency,
    ...
    for _ in range(n_sims):
        ...
        res = run_one(raw_trades, raw_slots, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
                      ...
                      blueberry_reset_enabled=blueberry_reset_enabled)
        rows.append(res)
...
def summarize(df, label):
    ...
    return dict(config=label, n=len(df), profit=net.mean(), ruine=(net < 0).mean() * 100,
                ...
                reserve_min_after_unlock_worst=df["reserve_min_after_unlock"].dropna().min(),
...
if __name__ == "__main__":
    ...
    rows = []
    for risk_label, eval_r, fleet_r in [("eval=1.00/fleet=1.90", 1.00, 1.90), ("eval=1.25/fleet=1.90", 1.25, 1.90)]:
        for label, enabled in [("baseline (pas de reset Blueberry)", False), ("reset Blueberry active", True)]:
            t0 = time.time()
            df = run_propagated(pop, market_data, excluded_map, CEILING, seq, config, ei.DEFAULT_EMERGENCY,
                                 eval_r, fleet_r, 1.75, ei.FINAL_RESERVE_SHARE, ei.EXTRA_THRESHOLD_MULT,
                                 n_sims=n_sims, seed=8000, blueberry_reset_enabled=enabled)
            row = summarize(df, f"{risk_label} | {label}")
            rows.append(row)
            print(f"[{risk_label} | {label:35s}] profit={row['profit']:+,.0f}$ ruine={row['ruine']:.2f}% "
                  ...
                  f"bb_resets_used={row['mean_bb_resets']:.2f} casse<=30j={row['break_rate_30d_pct']:.2f}% "
                  f"casse<=60j={row['break_rate_60d_pct']:.2f}% reserve_min_6mo(pire)={row['reserve_min_6mo_worst']:,.0f}$ "
                  f"quasi_gele={row['quasi_frozen_pct']:.1f}% ({time.time()-t0:.0f}s)")
            pd.DataFrame(rows).to_csv(f"etape_e_blueberry_reset_n{n_sims}.csv", index=False)
