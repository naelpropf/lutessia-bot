"""
Partie 2, PISTE D (2026-08-17) : re-test du "compte contrarian RR
0,75-1,25" (ancien S2.47 `registre_parametres_projet.md`,
`structure_pistes_2026-08-11.py:_contrarian_population`/`run_one` section
C) sous la pile actuelle. Reconstruction PORTEE (pas copiee telle quelle --
le fichier source utilise une architecture differente, process_one_account
generique) mais mecanisme IDENTIQUE : compte dedie, meme palier/format que
Blueberry CLASSIQUE (jamais Instant -- inexistant a l'epoque), actif des
jour0 EN PLUS du starter normal, trade EXCLUSIVEMENT la bande
0,75<=rr_tp1<1,25 (jamais tradee par le reste de la flotte, filtre
standard >=1,35 desormais). Flux independant, bootstrappe separement
(seeds distincts, meme duree cible), fusionne par le temps avec le flux
principal (evenements tries chronologiquement, meme convention que le
script source). Pas de payout_cycle sur ce compte (gname distinct de
"Blueberry", jamais dans PAYOUT_CYCLE_FIRMS) ni de any-RR/rr_tp2 sizing
(n'existaient pas au moment de la piste originale).

Sous l'ANCIENNE pile (08/11, avant rebuild RR1,35/corr0,80 du 08/12, avant
S1.8) : marche aux DEUX plafonds (+1,80%/+2,09% profit), dominance sur
3 axes de risque a 3000$, un seul axe degrade a 1000$ (hit_ceiling +1pt).
Marque "candidat prioritaire n=600+cascade" a l'epoque, jamais confirme.
"""
import random
import sys
import time
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
            ...
            contrarian_trades=None, contrarian_slots=None):
    fmt_by_firm = {g: FORMATS[k] for g, k in format_by_firm.items()}
    has_contrarian = contrarian_trades is not None
    ...
    # <<< PISTE D (compte contrarian, 2026-08-17) : meme format/palier que
    # Blueberry CLASSIQUE (jamais Instant -- ce mecanisme n'existait pas
    # dans la piste originale de 08/11, reconstruction a l'identique), actif
    # des jour0 EN PLUS du starter normal.
    contrarian_acc = None
    if has_contrarian:
        c_fmt = FORMATS[BB_CLASSIC_KEY]
        c_palier = BASE_PALIER["Blueberry"]
        c_cost = price_for_bb(BB_CLASSIC_KEY, c_palier, ceiling)
        contrarian_acc = make_acc_mf(c_fmt, c_palier, cost=c_cost, active=True)
        contrarian_acc.update(_gname="Blueberry_contrarian", _fmt_key=BB_CLASSIC_KEY,
                               base_palier=c_palier, base_cost=c_cost,
    ...
    if has_contrarian:
        main_events = [(slot_arrivals[i], "main", trade_idx) for i, trade_idx in enumerate(order)]
        contra_events = [(contrarian_slots[i], "contrarian", i) for i in range(len(contrarian_trades))]
        merged_event_list = sorted(main_events + contra_events, key=lambda e: e[0])
    else:
        merged_event_list = [(slot_arrivals[i], "main", trade_idx) for i, trade_idx in enumerate(order)]

    # NB: la variable locale `events` est reutilisee plus bas (forfeit
    # bucket tracking, code herite inchange) -- nom distinct ici expres
    # pour eviter toute confusion, meme si le shadowing est inoffensif
    # (l'iterateur du for est capture une seule fois au debut de la boucle).
    for now, source, idx in merged_event_list:
        state["_now"] = now
        ...
        if source == "main":
          ...
          for gname, accs in list(accounts_by_group.items()):
            ...
            # <<< PISTE D : traitement du compte contrarian -- process_trade_mf
            # PUR (pas de any-RR/rr_tp2 sizing, la piste originale de 08/11
            # n'utilisait aucun des deux), pas de payout_cycle (gname distinct
            # de "Blueberry", jamais dans PAYOUT_CYCLE_FIRMS -- meme fidelite
            # que le script source `structure_pistes_2026-08-11.py`).
            trade = contrarian_trades[idx]
            acc = contrarian_acc
            gname = "Blueberry_contrarian"
            fmt = FORMATS[acc["_fmt_key"]]
            pdef = _current_phase(fmt, acc)
            base_r = fleet_risk if acc["phase"] == "funded" else eval_risk
            r = effective_risk(acc, pdef, base_r)
            was_funded = acc["active"] and acc["phase"] == "funded"
            phase_before, idx_before = acc["phase"], acc["phase_index"]
            just_funded = process_trade_mf(acc, trade, now, fmt, state, r, market_data, excluded_map,
            ...
            if broke:
                ...
                if t_since_open is not None:
                    ...
                    if t_since_open <= 60 * 86400:
                        state["breaks_within_60d"] += 1
                use_bb_reset = (was_funded and not acc["_reset_used"])
                if use_bb_reset:
                    cost = 2.0 * acc["base_cost"]
                    acc["active"] = False
                    acc["_reset_used"] = True
...
def run_propagated(pop, market_data, excluded_map, ceiling, seq_grouped, format_by_firm, emergency,
    ...
    for _ in range(n_sims):
        ...
        if contrarian_pop is not None:
            wr_draw_c = rng_wr_c.betavariate(ei.ALPHA_POST, ei.BETA_POST)
            trades_c, slots_c = eng.build_flexible_population(contrarian_pop, wr_draw_c, 1.0, False, random.Random(rng_boot_c.random()))
...
if __name__ == "__main__":
    ...
    contrarian_pop = contrarian_population()
    print(f"[verif] population contrarian (0,75<=rr_tp1<1,25) : {len(contrarian_pop)} trades")
    ...
    for ceiling in ceilings_arg:
        ...
        for name, c_pop in CONFIGS.items():
            ...
            pd.DataFrame(rows).to_csv(f"chantier_pisteD_n{n_sims}_2026-08-17.csv", index=False)
