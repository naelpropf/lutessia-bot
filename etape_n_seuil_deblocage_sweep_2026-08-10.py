"""
Etape J (08/10 nuit) : recalibration ciblee de piste F (Etape I) suite a la
decomposition du 08/10 -- B_entry20_red50 elimine totalement la ruine
(5->0/300 a plafond=1000$) mais le cout net (10 scenarios sains->annee1<0)
depasse legerement le benefice direct (3 conversions ruine->annee1<0 seul),
sans toutefois jamais creer de PIRE resultat (aucun scenario ne devient
ruine sous B -- matrice complete dans project_dd_distance_sizing_rejected_
2026-08-10).

Copie de etape_i_dd_distance_sizing_2026-08-10.py (meme dd_distance_pct,
meme fonction B palier+hysteresis, moteur inchange), deux volets
independants :

VOLET 1 -- grille plus fine de B (entry<20%, reduction<50%), objectif :
fermer l'ecart sur annee1<0 sans perdre le gain profit/ruine deja trouve.

VOLET 2 -- B_entry20_red50 restreint a la fenetre PRE-DEBLOCAGE uniquement
(sizing actif seulement tant que fleet_unlocked=False, risque plein des le
deblocage complet -- nouveau parametre pre_unlock_only). Objectif :
verifier si limiter le sizing a la periode ou le point de fragilite unique
(Blueberry seul avant le deblocage echelonne complet) existe reellement
reduit le cout d'opportunite observe, puisque la flotte est deja
diversifiee post-deblocage et n'a a priori pas besoin de cette protection.
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
    ...
    state = {"reserve": 0.0, "ever_funded": False, "real_cash_paid": active0_cost, "total_breaks": 0,
             ...
             "dd_reduced_obs": 0, "dd_total_obs": 0, "funding_delays": [], "reserve_after_unlock": {}}
    ...
    def open_group(gname, is_final):
        for a in accounts_by_group[gname]:
            ...
            a["_dd_reduced"] = False
            state["total_opens"] += 1
        state["reserve_after_unlock"][gname] = state["reserve"]
    ...
    return {"final_net_split": combined_net(), "is_paid_cum": state["is_paid_cum"],
            ...
            "reserve_after_ftmo_unlock": state["reserve_after_unlock"].get("FTMO", float("nan")),
...
def summarize(df, label, ceiling):
    ...
    return dict(config=label, ceiling=ceiling, n=len(df), profit=net.mean(), ruine=(net < 0).mean() * 100,
                ...
                mean_reserve_after_ftmo_unlock=df["reserve_after_ftmo_unlock"].mean(),
                p10_reserve_after_ftmo_unlock=df["reserve_after_ftmo_unlock"].quantile(0.10))
...
if __name__ == "__main__":
    ...
    FTMO_THRESHOLDS = [0.0, 500.0, 1000.0, 2000.0, 5000.0]

    rows = []
    for t_ftmo in FTMO_THRESHOLDS:
        seq = ei.seq_grouped_multi(t_ftmo, 15000, 25000, 25000)
        label = f"FTMO_seuil_{int(t_ftmo)}"
        for ceiling in ceilings_arg:
            ...
            rows.append(row)
            print(f"[{label:20s} plafond={ceiling:.0f}$] profit={row['profit']:+,.0f}$ ruine={row['ruine']:.2f}% "
                  f"annee1<0={row['annee1_neg']:.2f}% (pre={row['annee1_neg_pre']:.2f}%) "
                  f"coussin_moy_ftmo={row['mean_reserve_after_ftmo_unlock']:,.0f}$ "
                  f"coussin_p10_ftmo={row['p10_reserve_after_ftmo_unlock']:,.0f}$ ({time.time()-t0:.0f}s)")
            pd.DataFrame(rows).to_csv(f"etape_n_seuil_deblocage_sweep_n{n_sims}.csv", index=False)
