"""
Etape M (08/10 nuit) : piste H -- routage flotte-wide, exclusion de la paire
AUD/JPY-USD/CHF pour un SOUS-ENSEMBLE FIXE (50/50) des comptes plutot que
toute la flotte. Objectif : reduire le nombre de firms simultanement
touchees par un run annee1<0 post-deblocage (confirme 92-97% des cas
touchent chaque firm, etape_e_annee1_levers_2026-08-09.md Volet 2).

⚠️ VERIFICATION PREALABLE (avant ce script) : la paire AUD/JPY-USD/CHF
citee ne tient PAS a l'examen -- correlation reelle -0,09 (quasi nulle,
tres en dessous du seuil CORR_TH=0,6), seulement 2 jours de chevauchement
dans daily_dd_pair_ranking_288.csv, jamais >=4-5% de DD combine (le seuil
que ce meme fichier utilise pour qualifier une combinaison "dangereuse").
De plus toute paire JPY-JPY est DEJA exclue automatiquement pour TOUTE la
flotte (monte_carlo_simulation.py:83, is_jpy(a) and is_jpy(b)), donc si
"JPY-JPY" etait l'intention reelle, piste H n'aurait rien a faire de plus.
Teste quand meme ici sur la paire effectivement trouvee (AUD/JPY-USD/CHF)
pour clore la question empiriquement -- effet nul ou tres faible attendu
etant donne la faiblesse du signal source.

Copie de etape_l_recovery_diagnostic_2026-08-10.py (moteur REF+V2 inchange,
memes 2 configs baseline/V2 disponibles), routage AJOUTE : chaque compte
tague a la creation avec un groupe fixe (0/1, alternance par compteur
global) ; le groupe 1 utilise un excluded_map STRICT (normal + la paire
ciblee explicitement exclue), le groupe 0 garde le excluded_map normal.
Instrumentation ajoutee : nombre de firms distinctes touchees par au moins
une casse POST-deblocage, par run.
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
            ...
            strict_excluded_map=None):
    fmt_by_firm = {g: FORMATS[k] for g, k in format_by_firm.items()}
    use_routing = strict_excluded_map is not None
    route_counter = [0]

    def next_route_group():
        g = route_counter[0] % 2
        route_counter[0] += 1
        return g
    ...
    for gname in FIRMS:
        ...
        for a in accs:
            ...
            a["_route_group"] = next_route_group()
            a["_dd_reduced"] = False
    ...
    state = {"reserve": 0.0, "ever_funded": False, "real_cash_paid": active0_cost, "total_breaks": 0,
             ...
             "dd_reduced_obs": 0, "dd_total_obs": 0, "funding_delays": [], "firms_broken_post_unlock": set()}
    ...
    def process_extra_account(now):
        ...
        for gname in ei.GROWTH_FIRMS_EXTRA:
            ...
            if state["reserve"] >= extra_threshold_mult * extra_cost:
                ...
                new_acc["_route_group"] = next_route_group()
                accs.append(new_acc)
    ...
    for slot_idx, trade_idx in enumerate(order):
        ...
        if now >= YEAR_SECONDS and recovery_time is None and combined_net() >= 0:
            recovery_time = now
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                map_for_acc = strict_excluded_map if (use_routing and acc["_route_group"] == 1) else excluded_map
                just_funded = process_trade_mf(acc, trade, now, fmt, state, r, market_data, map_for_acc,
                ...
                if broke:
                    ...
                    if fleet_unlocked:
                        state["firms_broken_post_unlock"].add(gname)
    ...
    recovery_month = (recovery_time / MONTH_SECONDS) if recovery_time is not None else float("nan")
    return {"final_net_split": combined_net(), "is_paid_cum": state["is_paid_cum"],
            ...
            "n_firms_broken_post_unlock": len(state["firms_broken_post_unlock"])}
...
def run_propagated(pop, market_data, excluded_map, ceiling, seq_grouped, format_by_firm, emergency,
                    ...
                    b_entry_frac=None, b_reduction=None, pre_unlock_only=False, strict_excluded_map=None):
    ...
    for _ in range(n_sims):
        ...
        res = run_one(raw_trades, raw_slots, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
                      ...
                      strict_excluded_map=strict_excluded_map)
        rows.append(res)
...
def summarize(df, label, ceiling):
    ...
    post_neg_mask = year1_neg & ~pre_mask
    pct_all5_touched = ((df["n_firms_broken_post_unlock"] >= 5) & post_neg_mask).sum() / max(1, post_neg_mask.sum()) * 100
    return dict(config=label, ceiling=ceiling, n=len(df), profit=net.mean(), ruine=(net < 0).mean() * 100,
                ...
                pct_post_neg_touching_all5=pct_all5_touched,
                mean_firms_broken_post_unlock=df.loc[post_neg_mask, "n_firms_broken_post_unlock"].mean())


def build_strict_map(normal_map):
    strict = {k: set(v) for k, v in normal_map.items()}
    strict["AUD/JPY"].add("USD/CHF")
    strict["USD/CHF"].add("AUD/JPY")
    return strict
...
if __name__ == "__main__":
    ...
    strict_map = build_strict_map(excluded_map)
    seq = ei.seq_grouped_multi(1000, 15000, 25000, 25000)
    config = ei.CONFIG_REF
    EVAL_RISK, FLEET_RISK, GFT_EVAL_RISK = 1.25, 1.90, 1.75

    configs = [("REF_V2 baseline (pas de routage)", None), ("REF_V2 + routage piste H", strict_map)]
    ...
    for label, smap in configs:
        for ceiling in ceilings_arg:
            ...
            df = run_propagated(pop, market_data, excluded_map, ceiling, seq, config, ei.DEFAULT_EMERGENCY,
                                 ...
                                 strict_excluded_map=smap)
            row = summarize(df, label, ceiling)
            rows.append(row)
            print(f"[{label:32s} plafond={ceiling:.0f}$] profit={row['profit']:+,.0f}$ ruine={row['ruine']:.2f}% "
                  f"annee1<0={row['annee1_neg']:.2f}% (pre={row['annee1_neg_pre']:.2f}% post={row['annee1_neg_post']:.2f}%) "
                  f"touch_5firms_post_neg={row['pct_post_neg_touching_all5']:.1f}% "
                  f"mean_firms_touched={row['mean_firms_broken_post_unlock']:.2f} ({time.time()-t0:.0f}s)")
            pd.DataFrame(rows).to_csv(f"etape_m_piste_h_routing_n{n_sims}.csv", index=False)
