"""
Chantier 1 (08/09, confirmation finale) : confirmation n=600 + cascade check
du meilleur point de la "zone basse" (aucun point de la "zone haute"
eval>=1,75%/flotte>=2,25% n'a domine ni egale, cf. balayage
etape_e_risk_sweep_bbreset_2026-08-09.py -- arbitrage reel, pas une
domination, ruine nettement plus elevee en zone haute au plafond 1000$).

Point confirme ici : eval=1,25%/flotte=1,90%, GFT=1,75%, reset Blueberry
actif (mecanisme confirme separement, etape_e_blueberry_reset_2026-08-09.py),
aux deux plafonds (1000$/3000$), n=600, avec instrumentation cascade
complete (casse<=30j/60j, reserve minimale, quasi-gele).

Copie de etape_e_risk_sweep_bbreset_2026-08-09.run_one, augmentee de
l'instrumentation cascade (meme methodologie que etape_e_cascade_check.py
et etape_e_blueberry_reset_2026-08-09.py). N'importe pas ces scripts
directement.
"""
import random
import sys
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
    ...
    """Blueberry reset toujours actif (mecanisme confirme n=600)."""
...
if __name__ == "__main__":
    ...
    for ceiling in (1000.0, 3000.0):
        ...
        row = dict(ceiling=ceiling, profit=net.mean(), ruine=(net < 0).mean() * 100,
        ...
        pd.DataFrame(rows).to_csv(f"etape_e_final_lock_bbreset_n{n_sims}.csv", index=False)
