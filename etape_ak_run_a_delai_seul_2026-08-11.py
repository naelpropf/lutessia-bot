"""
Etape AK (08/11) -- RUN A de la decomposition delai/forfeiture : DELAI
SEUL, sans forfeiture reelle. Copie de etape_ai_payout_cadence_
calibration_2026-08-11.py (cadence par firm/occurrence deja corrigee,
INCHANGEE ici) -- UN seul changement : sur une casse, pending_payout
n'est PLUS abandonne, il est integralement RECUPERE dans total_funded_pnl
et state["reserve"] au moment de la casse (comme un flush anticipe
declenche par la casse elle-meme). Le retard d'acces au cash entre deux
flush existe toujours (isole cet effet), mais plus aucune perte reelle de
profit n'est possible.

Objectif : avec Run B (forfeiture seule, etape_al) et Run C (deja fait,
les deux combines = ~32,67% annee1<0), decomposer les ~8pt d'ecart entre
Run C (32,67%) et Run E (24,67%, aucune contrainte cash) en part
attribuable au SEUL retard d'acces au cash (Run A) vs a la SEULE
forfeiture reelle (Run B).
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                if use_goat_guard:
                    ...
                    # Run A (etape AK) : DELAI SEUL -- pending_payout n'est
                    # PAS perdu sur une casse, il est integralement
                    # recupere (comme un flush anticipe declenche par la
                    # casse). Aucune forfeiture reelle possible dans ce
                    # run -- forfeited_pre/post restent a 0 par construction.
...
if __name__ == "__main__":
    ...
    for ceiling in ceilings_arg:
        ...
        row = summarize(df, "RUN_A_delai_seul_2026-08-11", ceiling)
        rows.append(row)
        ...
        pd.DataFrame(rows).to_csv(f"etape_ak_run_a_delai_seul_n{n_sims}.csv", index=False)
