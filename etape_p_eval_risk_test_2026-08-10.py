    ...
    for eval_risk in eval_risks:
        for label, entry, reduction, pre_only in configs:
            for ceiling in ceilings_arg:
                ...
                row = summarize(df, f"eval{eval_risk}_{label}", ceiling)
                row["eval_risk"] = eval_risk
                rows.append(row)
                print(f"[eval={eval_risk} {label:10s} plafond={ceiling:.0f}$] profit={row['profit']:+,.0f}$ "
                ...
                pd.DataFrame(rows).to_csv(f"etape_p_eval_risk_test_n{n_sims}.csv", index=False)
