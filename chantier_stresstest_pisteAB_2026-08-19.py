"""
Partie 2 (2026-08-17) : re-test de 2 anciennes pistes sous la pile actuelle
(RR>=1,35/corr0,80, S1.8, S2.35), reconstruites a l'identique (SANS scaling
de risque, contrairement au double-starter FTMO 50/50 de Section C) :

PISTE A -- BBx2 (2x Blueberry parallele, ancien S2.15
`registre_parametres_projet.md`) : CONFIRME n=600 a 3000$ (+4,31%),
REJETE a 1000$ (domine par solo_BB) sous l'ANCIENNE pile (avant rebuild
RR1,35/corr0,80 du 08/12, avant S1.8). 2e compte Blueberry actif des jour0,
meme palier/format standard (bb_choose_fmt_key() appele independamment
pour chaque compte), cout ~2x le starter normal.

PISTE B -- Bootstrap parallele BB+GFT jour0 (ancien S2.6/S2.41) : REJETE
a 1000$ (cout combine 453$ epuise la tresorerie, hit_ceiling x10),
arbitrage a 3000$ (PAS une dominance stricte -- profit/annee1<0 meilleurs,
ruine/hit_ceiling legerement pires) sous l'ANCIENNE pile. GFT ouvre son
1er compte des jour0 a son palier/cout standard (288$, format classique
GFT_2Step_GOAT), risque PLEIN (pas de fraction, contrairement a Section C).

Correctif de mecanisme necessaire (meme raison que Section C) :
`open_group()` saute maintenant les comptes deja actifs pour eviter un
double-facturation/reset du compte GFT jour0 au trigger sequentiel normal.
"""
import random
import sys
import time
...
# <<< CORRECTIF RISQUE INSTANT (regen 2026-08-19) : cap reel 1,5%/trade sur
# Blueberry Instant Elite/Lite, applique APRES tout autre multiplicateur.
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
    ...
    for gname in FIRMS:
        ...
        gft_day0_slot = (gname == "GFT" and extra_day0_starter == "GFT")
    ...
    for slot_idx, trade_idx in enumerate(order):
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                # <<< CORRECTIF RISQUE INSTANT : cap 1,5%/trade reel, en dernier.
...
def run_propagated(pop, market_data, excluded_map, ceiling, seq_grouped, format_by_firm, emergency,
    ...
    for _ in range(n_sims):
        ...
        res = run_one(raw_trades, raw_slots, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
                      ...
                      extra_day0_starter=extra_day0_starter)
        rows.append(res)
...
def build_market_data_with_indices():
    """<<< CORRECTIF 2026-08-19 : identique a chantier_strategie_b_isolation_
    indices_2026-08-18.py:802-819 -- feasible_risk_pct() plante sur les 5
    labels index de la population elargie (742 trades), aucune spec broker
    reelle disponible. Contrainte de capacite desactivee pour ces labels."""
...
if __name__ == "__main__":
    # <<< STRESS-TEST H1/H2 + 4 blocs k-fold, regen 2026-08-19 sous cap
    # Instant 1,5% + population 742 trades. n reduit a 100 pour rester dans
    # un temps de calcul raisonnable -- controle de robustesse preliminaire,
    # pas la confirmation finale.
    n_sims = int(sys.argv[1]) if len(sys.argv) > 1 else 100

    t_start = time.time()
    pop_full, market_data, excluded_map = load_common()
    pop_full = pop_full.sort_values("date_creation").reset_index(drop=True)
    print(f"[verif] population complete (RR>={MIN_RR_NEW}) : {len(pop_full)} trades")
    ...
    mid = len(pop_full) // 2
    subperiods = {
        "H1": pop_full.iloc[:mid].reset_index(drop=True),
        "H2": pop_full.iloc[mid:].reset_index(drop=True),
    }
    blocks4 = np.array_split(pop_full, 4)
    for i, b in enumerate(blocks4):
        subperiods[f"bloc{i}"] = b.reset_index(drop=True)

    TESTS = [
        ("BBx2@5000", "Blueberry", 5000.0),
        ("BBGFT@5000", "GFT", 5000.0),
        ("BBGFT@3000", "GFT", 3000.0),
    ]

    rows = []
    for label, extra_starter, ceiling in TESTS:
        bb_th = BB_THRESHOLD_BY_CEILING[ceiling]
        print(f"\n{'='*70}\n{label} (extra_day0_starter={extra_starter}, ceiling={ceiling:.0f}$)\n{'='*70}")
        for sp_name, sp_pop in subperiods.items():
            t0 = time.time()
            df_ref = run_propagated(sp_pop, market_data, excluded_map, ceiling, seq, config,
                                     bb_threshold=bb_th, extra_day0_starter=None, **common_kwargs)
            df_cand = run_propagated(sp_pop, market_data, excluded_map, ceiling, seq, config,
                                      bb_threshold=bb_th, extra_day0_starter=extra_starter, **common_kwargs)
            row_ref = summarize(df_ref, "REF", ceiling)
            row_cand = summarize(df_cand, label, ceiling)
            d_profit_pct = (row_cand["profit_moyen"] - row_ref["profit_moyen"]) / row_ref["profit_moyen"] * 100
            d_hc = row_cand["hit_ceiling_pct"] - row_ref["hit_ceiling_pct"]
            d_a1 = row_cand["annee1_neg"] - row_ref["annee1_neg"]
            direction = "profit MIEUX" if d_profit_pct > 0 else "profit PIRE"
            print(f"  [{sp_name}, n={n_sims}] REF profit={row_ref['profit_moyen']:+,.0f}$ hit_ceiling={row_ref['hit_ceiling_pct']:.2f}% | "
                  f"{label} profit={row_cand['profit_moyen']:+,.0f}$ ({d_profit_pct:+.2f}%, {direction}) "
                  f"hit_ceiling={row_cand['hit_ceiling_pct']:.2f}% (delta={d_hc:+.2f}pt) annee1<0 delta={d_a1:+.2f}pt "
                  f"({time.time()-t0:.0f}s)")
            rows.append(dict(test=label, subperiod=sp_name, n=n_sims,
                              ref_profit=row_ref["profit_moyen"], cand_profit=row_cand["profit_moyen"],
                              d_profit_pct=d_profit_pct, ref_hc=row_ref["hit_ceiling_pct"], cand_hc=row_cand["hit_ceiling_pct"],
                              d_hc=d_hc, ref_a1=row_ref["annee1_neg"], cand_a1=row_cand["annee1_neg"], d_a1=d_a1))
            pd.DataFrame(rows).to_csv("chantier_stresstest_pisteAB_2026-08-19.csv", index=False)
