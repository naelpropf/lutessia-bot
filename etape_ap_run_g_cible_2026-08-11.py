"""
Etape AP (08/11) -- RUN G : variante CIBLEE de l'add-on "7 Day Payout"
Blueberry. Copie de etape_ao_run_f_cout_reel_2026-08-11.py (cout reel
+20% confirme). Au lieu de payer le surcout des l'achat initial (Run F),
chaque lignee de compte (meme slot, reouvertures successives) commence en
cadence normale 14j SANS surcout ; des la 1ere casse de cette lignee, elle
bascule DEFINITIVEMENT en cadence rapide 7j AVEC surcout (le rachat qui
suit cette 1ere casse est deja surcharge). Objectif : economiser le
surcout sur les comptes qui ne re-cassent jamais, ne payer l'acces rapide
au cash que pour les lignees ayant deja prouve un risque de recasser.
...
PAYOUT_CYCLE_DAYS_FIRST = {"Blueberry": 7, "GFT": 3, "Fivers": 14}
PAYOUT_CYCLE_DAYS_SUBSEQUENT = {"Blueberry": 7, "GFT": 1.5, "Fivers": 14}
# Etape AP (08/11) -- RUN G, variante CIBLEE de l'add-on "7 Day Payout"
# Blueberry (+20% confirme officiellement, etape_ao) : au lieu de payer le
# surcout des l'achat initial, chaque "lignee" de compte (meme slot,
# reouvertures successives) commence en cadence NORMALE 14j SANS surcout.
# Des la 1ere casse de cette lignee, elle bascule DEFINITIVEMENT en
# cadence rapide 7j AVEC surcout (le rachat qui suit cette 1ere casse est
# deja surcharge -- logique : on ne paie l'acces rapide au cash qu'une
# fois le compte "prouve" a risque de recasser). acc["_has_broken_before"]
# (nouveau champ, distinct de acc["_reset_used"] qui ne suit que
# l'eligibilite au mecanisme de reset a prix double) porte cet etat,
# initialise a la creation (jour 0 / compte extra), JAMAIS remis a False
# lors d'une reouverture (persiste pour toute la lignee).
BB_PAYOUT_ADDON_MULT = 1.20


def price_for_bb(gname, fmt_key, palier, surcharge=False):
    price = ei.price_for(fmt_key, palier)
    return price * BB_PAYOUT_ADDON_MULT if (gname == "Blueberry" and surcharge) else price


def bb_cycle_days_targeted(has_broken_before):
    return 7 if has_broken_before else 14


def payout_cycle_days(gname, first_payout_done, has_broken_before=False):
    if gname == "Blueberry":
        return bb_cycle_days_targeted(has_broken_before)
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
    ...
    def base_palier_cost(gname):
        ...
        return palier, price_for_bb(gname, format_by_firm[gname], palier)
    ...
    def process_extra_account(now):
        ...
        for gname in ei.GROWTH_FIRMS_EXTRA:
            ...
            if current_capital + unit_palier > ei.FIRM_CAPITAL_CAP[gname]:
                continue
            extra_cost = price_for_bb(gname, format_by_firm[gname], unit_palier)
            if state["reserve"] >= extra_threshold_mult * extra_cost:
                ...
                new_acc["_has_broken_before"] = False
        ...
        for gname, accs in list(accounts_by_group.items()):
            ...
            for acc in list(accs):
                ...
                if use_payout_cycle and acc["active"] and acc["last_payout_time"] is not None \
                        and now - acc["last_payout_time"] >= payout_cycle_days(
                            gname, acc["_first_payout_done"], acc.get("_has_broken_before", False)) * 86400:
                ...
                if use_goat_guard:
                    ...
                    if gname == "Blueberry":
                        # Run G : bascule DEFINITIVE en cadence 7j + surcout
                        # des la 1ere casse de cette lignee -- affecte deja
                        # le cout de reouverture calcule plus bas.
                        acc["_has_broken_before"] = True
                    ...
                    if use_bb_reset:
                        cost = 2.0 * acc["base_cost"] * BB_PAYOUT_ADDON_MULT
                        acc["active"] = False
                        ...
                        if downgrade_active() and gname == ei.STARTER:
                            cost = acc["base_cost"] * (BB_PAYOUT_ADDON_MULT if gname == "Blueberry" else 1.0)
                        else:
                            cost = price_for_bb(gname, format_by_firm[gname], acc["palier"],
                                                 surcharge=(gname == "Blueberry"))
...
if __name__ == "__main__":
    ...
    for ceiling in ceilings_arg:
        ...
        row = summarize(df, "RUN_G_cible_2026-08-11", ceiling)
        rows.append(row)
        ...
        pd.DataFrame(rows).to_csv(f"etape_ap_run_g_cible_n{n_sims}.csv", index=False)
