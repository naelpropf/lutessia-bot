"""
Suite de chantier_pisteAB_bbx2_bbgft_2026-08-17.py (08/17, meme jour) --
QUESTION : le mecanisme BB+GFT jour0 (Piste B) est-il structurellement
limite a T=0, ou peut-il etre simule comme une ouverture GFT differee a un
instant T>0 (mois 2/4) sous la PILE ACTUELLE (RR>=1,35/corr0,80, S1.8
any-RR, S2.35 rr_tp2 sizing) ?

Precedent trouve par lecture de code : `etape_r_piste_a_delayed_start_
2026-08-10.py` implementait deja un trigger differe (jours de survie ou
seuil de reserve) sous l'ANCIENNE pile (pre-08/12, MIN_RR=1,25, sans
S1.8/S2.35) -- preuve que le mecanisme n'est PAS structurellement lie a
T=0 : `active` est un simple booleen active a n'importe quel instant via
`handle_cost_hybrid`, pas un flag fige a l'initialisation. Jamais porte
sous la pile actuelle, jamais teste avec un delai en MOIS (seulement
jours/seuil de reserve).

Copie de chantier_pisteAB_bbx2_bbgft_2026-08-17.py, meme moteur complet
(S1.8 any-RR/S2.35 rr_tp2 sizing/DD-distance sizing V2/payout cycle/GOAT
Guard/etc., TOUS conserves a l'identique pour rester comparable aux
chiffres n=600 deja confirmes), piste A (BBx2) retiree (hors sujet ici),
piste B generalisee : `extra_day0_starter="GFT"` + `gft_delay_months=None`
= comportement jour0 original inchange ; `gft_delay_months=X>0` = GFT
reste un compte normal (inactif) jusqu'a T=X mois puis s'ouvre via
`handle_cost_hybrid` (respecte le plafond, comme `open_group()`, PAS un
cout jour0 inconditionnel). Si le trigger normal de croissance
(reserve>=25000$ apres 1er financement) a deja ouvert GFT avant T, le
trigger differe devient un no-op (meme garde que `open_group()`,
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
            ...
            extra_day0_starter=None, gft_delay_months=None, gft_delay_days=None):
    fmt_by_firm = {g: FORMATS[k] for g, k in format_by_firm.items()}

    # gft_delay_months/gft_delay_days : None -> comportement jour0 original
    # inchange (si extra_day0_starter=="GFT") ; sinon GFT reste inactif au
    # demarrage et s'ouvre via handle_cost_hybrid a T=delai (voir
    # try_gft_delayed_trigger plus bas), au lieu d'un cout jour0 inconditionnel.
    # gft_delay_days a priorite si fourni (precision exacte en jours, pour
    # les delais courts type semaines -- gft_delay_months reste la voie
    # mois pour les delais longs).
    _delay_seconds = None
    if gft_delay_days is not None and gft_delay_days > 0:
        _delay_seconds = gft_delay_days * DAY_SECONDS
    elif gft_delay_months is not None and gft_delay_months > 0:
        _delay_seconds = gft_delay_months * MONTH_SECONDS
    gft_delay_slot = (extra_day0_starter == "GFT" and _delay_seconds is not None)
    gft_delay_fire_time = _delay_seconds
    gft_delayed_triggered = not gft_delay_slot
    gft_delayed_acc = None
    ...
    for gname in FIRMS:
        ...
        # <<< PISTE A/B (2026-08-17) : starter jour0 supplementaire, SANS
        # scaling de risque (reconstruction a l'identique de l'ancienne
        # piste, contrairement au double-starter FTMO 50/50 de Section C
        # qui testait un risque partiel).
        bbx2_slot = (gname == "Blueberry" and extra_day0_starter == "Blueberry")
        gft_day0_slot = (gname == "GFT" and extra_day0_starter == "GFT" and not gft_delay_slot)
        n_day0_accounts = 2 if bbx2_slot else (1 if gft_day0_slot else 0)
        if gname == "Blueberry":
        ...
        accs = []
        for slot_i in range(ei.N_ACCOUNTS_DAY0[gname]):
            this_is_day0 = is_day0 or (gft_day0_slot and slot_i == 0)
            a = make_acc_mf(fmt, palier, cost=cost, active=this_is_day0)
            a["_gname"] = gname
            a["_fmt_key"] = fmt_key
            a["base_palier"] = palier
            a["base_cost"] = cost
            a["_reset_used"] = False
            a["last_open_time"] = 0.0 if this_is_day0 else None
            a["_dd_reduced"] = False
            a["_dd_oscillations"] = 0
            a["_gg_triggered_count"] = 0
            a["_gg_split_until"] = None
            a["pending_payout"] = 0.0
            a["last_payout_time"] = 0.0 if this_is_day0 else None
            a["_first_payout_done"] = False
            accs.append(a)
            if gname == "GFT" and gft_delay_slot and slot_i == 0:
                gft_delayed_acc = a
        if bbx2_slot:
            # 2e compte Blueberry actif des jour0, choix de format INDEPENDANT
            fmt_key2 = bb_choose_fmt_key()
            fmt2 = FORMATS[fmt_key2]
            cost2 = price_for_bb(fmt_key2, palier, ceiling)
            a2 = make_acc_mf(fmt2, palier, cost=cost2, active=True)
            a2.update(_gname=gname, _fmt_key=fmt_key2, base_palier=palier, base_cost=cost2,
        ...
        active0_cost += sum(a["cost"] for a in accs if a["active"])
    ...
    state.update({"ever_funded": False, "real_cash_paid": active0_cost, "total_breaks": 0,
             ...
             "corr_swap_evictions": 0, "corr_swap_admits": 0, "gft_delayed_open_month": None})
    ...
    def try_gft_delayed_trigger(now):
        nonlocal gft_delayed_triggered
        if gft_delayed_triggered:
            return
        if now < gft_delay_fire_time:
            return
        gft_delayed_triggered = True
        acc = gft_delayed_acc
        if acc["active"]:
            # deja ouvert par le trigger normal de croissance (reserve>=25000$) avant T -- no-op
            return

        def _activate():
            acc["active"] = True
            acc["total_fees_paid"] = acc["cost"]
            acc["last_open_time"] = state["_now"]
            acc["_dd_reduced"] = False
            acc["_gg_triggered_count"] = 0
            acc["_gg_split_until"] = None
            ...
            acc["_first_payout_done"] = False
            state["total_opens"] += 1
            state["gft_delayed_open_month"] = now / MONTH_SECONDS

        handle_cost_hybrid(acc["cost"], pending_delayed_gft, "gft_delayed", _activate)
    ...
    result = {"final_net_split": combined_net(), "is_paid_cum": state["is_paid_cum"],
              ...
              "gft_delayed_open_day": (state["gft_delayed_open_month"] * MONTH_SECONDS / DAY_SECONDS
                                        if state["gft_delayed_open_month"] is not None else None)}
...
def run_propagated(pop, market_data, excluded_map, ceiling, seq_grouped, format_by_firm, emergency,
    ...
    for _ in range(n_sims):
        ...
        res = run_one(raw_trades, raw_slots, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
                      ...
                      extra_day0_starter=extra_day0_starter, gft_delay_months=gft_delay_months,
                      gft_delay_days=gft_delay_days)
        rows.append(res)
...
def summarize(df, label, ceiling):
    ...
    mean_delayed_open = df["gft_delayed_open_day"].dropna().mean()
    return dict(config=label, ceiling=ceiling, n=len(df),
                ...
                mean_gft_open_day=mean_delayed_open)
...
if __name__ == "__main__":
    ...
    ref_size_func = make_size_func_tail(1.6, threshold=8.0)  # sizing rr_tp2 deja adopte S2.35
    ...
    BB_THRESHOLD_BY_CEILING = {3000.0: 0.0, 5000.0: 0.0}

    # (label, extra_day0_starter, gft_delay_days) -- granularite semaine,
    # fenetre de decision reelle utilisateur (negociation financeur 1-3
    # semaines), suite du run 2mo/4mo (deja consigne registre_strategie_
    # trading.md, cf. session)
    CONFIGS = [
        ("REF_travail (S1.8+S2.35)", None, None),
        ("PisteB_day0", "GFT", 0),
        ("PisteB_delay_1sem", "GFT", 7),
        ("PisteB_delay_2sem", "GFT", 14),
        ("PisteB_delay_3sem", "GFT", 21),
        ("PisteB_delay_4sem", "GFT", 28),
    ]

    rows = []
    for ceiling in ceilings_arg:
        bb_th = BB_THRESHOLD_BY_CEILING[ceiling]
        for name, extra_starter, delay_days in CONFIGS:
            t0 = time.time()
            df = run_propagated(pop, market_data, excluded_map, ceiling, seq, config,
                                 bb_threshold=bb_th, extra_day0_starter=extra_starter,
                                 gft_delay_days=delay_days, **common_kwargs)
            row = summarize(df, name, ceiling)
            rows.append(row)
            dj = row["mean_gft_open_day"]
            dj_str = f"{dj:.1f}j" if dj == dj else "n/a"
            print(f"[{name} plafond={ceiling:.0f}$] profit_moy={row['profit_moyen']:+,.0f}$ "
                  ...
                  f"annee1<0={row['annee1_neg']:.2f}% gft_ouvert@{dj_str} ({time.time()-t0:.0f}s)")
            pd.DataFrame(rows).to_csv(f"chantier_pisteB_delayed_start_semaines_n{n_sims}_2026-08-17.csv", index=False)
