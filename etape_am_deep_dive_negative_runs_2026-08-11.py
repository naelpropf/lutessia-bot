"""
Etape AM (08/11) : extraction du journal chronologique COMPLET de chaque
run qui finit en solde_negatif_annee4 (net<0 a l'annee 4) sous le moteur
REFERENCE ACTUEL -- cadence de payout corrigee par firm/occurrence
(etape_ai_payout_cadence_calibration_2026-08-11.py, PAS l'ancien
etape_ah non corrige) : population 721 trades, eval=1,25%/flotte=1,90%,
cap Blueberry corrige, V2, combo FTMO-10%/GFT Goat Guard, payout/
forfeiture actif avec cadence correcte (Blueberry/Fivers 14j, GFT
3j->1,5j). Choix explicite documente : la demande dit "reference
officielle actuelle" sans preciser, le moteur cadence-corrigee est la
version la plus a jour de ce chantier (etape_ai/Run C), utilisee ici.

Deux changements par rapport a etape_ai :
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
            ...
            ftmo_discount=False, gft_goat_guard=False, payout_cycle=False, log_events=False):
...
if __name__ == "__main__":
    ...
    for ceiling in ceilings_arg:
        t0 = time.time()
        df = run_and_extract(pop, market_data, excluded_map, ceiling, seq, config, ei.DEFAULT_EMERGENCY,
        ...
        print(f"[plafond={ceiling:.0f}$] profit_moyen={net.mean():+,.0f}$ "
              f"solde_negatif_annee4={((net < 0).mean() * 100):.2f}% ({time.time()-t0:.0f}s)")
