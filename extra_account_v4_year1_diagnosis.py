"""
Session 08/08 (suite) -- Etape 1 de la demande : reverifier le diagnostic
categorie A/B (P(annee1<0)) sous la config finale v4 VERROUILLEE
(extra_account_v4_multi.py : mecanisme "compte supplementaire" deplafonne
sous les vrais caps prop firm, flotte=2.75%, eval=2.25%/GFT=1.75%),
au lieu de l'ancienne base (flotte=2.5%, 1 seul extra/firm, ancien
process_growth_upgrade) utilisee par year1_negative_diagnosis.py.

Meme logique de categorisation que year1_negative_diagnosis.py, verifiee
par citation de code ci-dessous (transposee du mecanisme "Scaling Plan"
process_growth_upgrade vers le mecanisme "compte supplementaire" de v4) :
  - Categorie A : year1_fleet_unlocked=True a 12 mois (le groupe
    FTMO/Fivers/GFT/FundedNext a ete debloque avant le mois 12) ->
    deficit decompose en (i) frais pre-financement Blueberry avant son
    1er financement, (ii) cout de l'ouverture groupee du reste de la
    flotte, (iii) casses post-financement dont le cout net (cout de
    rachat - profit deja engrange par CE compte) est positif -- helas le
    mecanisme "compte supplementaire" ajoute une 4e source de deficit
    potentielle (iv) cout d'ouverture des comptes supplementaires
    eux-memes, trackee separement ici (absente de l'ancien diagnostic
    car le mecanisme n'existait pas).
  - Categorie B : year1_fleet_unlocked=False a 12 mois -> Blueberry seul,
    reserve pas encore a 30k$.

process_trade() ci-dessous reprend EXACTEMENT process_trade() de
extra_account_v4_multi.py (meme calcul de risque, meme split, meme
reserve_share, meme downgrade-on-reopen Blueberry pre-deblocage), avec
l'instrumentation supplementaire de year1_negative_diagnosis.py ajoutee
(ever_funded_self, starter_prefund_fees, postfund_break_net_cost) plus le
nouveau extra_account_open_cost pour le mecanisme v4.
"""
import random
import time
...
if __name__ == "__main__":
    ...
    for ceiling in (1000.0, 3000.0):
        print("\n" + "=" * 100 +
              f"\nDIAGNOSTIC ANNEE1<0 v4 -- eval={FINAL_EVAL_RISK}%(GFT={FINAL_GFT_EVAL_RISK}%)/flotte={FINAL_FLEET_RISK}% "
        ...
        df.to_csv(f"extra_account_v4_year1_diagnosis_ceiling{int(ceiling)}.csv", index=False)

        net = df["final_net_split"] - df["is_paid_cum"]
        print(f"\nProfit final (n={n_sims}) = {net.mean():+,.0f}$ | ruine = {(net < 0).sum()/len(df)*100:.2f}%")
        ...
        cat_b = neg[~neg["year1_fleet_unlocked"]]
        print(f"\nCategorie A (flotte debloquee a 12 mois) : {len(cat_a)}/{n_neg} ({len(cat_a)/n_neg*100:.1f}%)")
        print(f"Categorie B (flotte PAS debloquee a 12 mois) : {len(cat_b)}/{n_neg} ({len(cat_b)/n_neg*100:.1f}%)")

        print(f"\nn comptes actifs a 12 mois (parmi les annee1<0) : moy={neg['year1_n_active'].mean():.2f} "
              f"| median={neg['year1_n_active'].median():.1f} | min={neg['year1_n_active'].min()} | max={neg['year1_n_active'].max()}")

        if len(cat_a) > 0:
            print(f"\n--- Categorie A : decomposition du deficit (moyennes) ---")
            print(f"  Frais pre-financement Blueberry (avant son 1er financement) = {cat_a['year1_starter_prefund_fees'].mean():,.0f}$")
            print(f"  Cout ouverture groupee reste de la flotte                   = {cat_a['year1_fleet_unlock_cost'].mean():,.0f}$")
            print(f"  Cout ouverture comptes supplementaires (nouveau vs 08/08)   = {cat_a['year1_extra_account_open_cost'].mean():,.0f}$")
            print(f"  Casses post-financement (cout net > profit deja engrange)   = {cat_a['year1_postfund_break_net_cost'].mean():,.0f}$")
            print(f"  Deficit moyen annee1 = {cat_a['year1_net_split'].mean():,.0f}$")
            catchup_a = cat_a["catchup_month"].dropna()
            print(f"  Delai median de rattrapage (repasse >=0 apres mois 12) = "
                  f"{catchup_a.median():.2f} mois (n={len(catchup_a)}/{len(cat_a)} rattrapes avant la fin de l'horizon)")

        if len(cat_b) > 0:
            print(f"\n--- Categorie B : Blueberry seul, reserve pas encore a 30k$ ---")
            print(f"  Reserve moyenne a 12 mois = {cat_b['year1_reserve'].mean():,.0f}$ (seuil 30 000$)")
            print(f"  Frais pre-financement Blueberry cumules = {cat_b['year1_starter_prefund_fees'].mean():,.0f}$")
            print(f"  Deficit moyen annee1 = {cat_b['year1_net_split'].mean():,.0f}$")
            catchup_b = cat_b["catchup_month"].dropna()
            print(f"  Delai median de rattrapage (repasse >=0 apres mois 12) = "
                  f"{catchup_b.median():.2f} mois (n={len(catchup_b)}/{len(cat_b)} rattrapes avant la fin de l'horizon)")
