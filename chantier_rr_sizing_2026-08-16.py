...
# ============================================================
# <<< CHANTIER SIZING (Section D) : fonctions de scaling candidates
# RR observe dans la population actuelle : min=1,351 (filtre MIN_RR),
# P25=1,587, median=1,889, P75=2,318, max=3,000 (plafond de construction
# du payoff trailing, verifie S2.28). Multiplicateur max=1,30 pour les 3
# variantes (borne prudente commune, cf. docstring module).
# ============================================================
RR_MIN, RR_MAX = 1.35, 3.0
SIZE_MULT_MAX = 1.30
RR_QUARTILES = (1.5868005738880648, 1.8888888888887791, 2.3178571428572114)  # P25/P50/P75 mesures sur pop 631


def size_func_linear(rr):
    """(a) Lineaire : x1,00 a RR=1,35 -> x1,30 a RR=3,0 (max de construction),
    clippe par securite si jamais un RR hors bornes apparaissait."""
    frac = (rr - RR_MIN) / (RR_MAX - RR_MIN)
    frac = min(1.0, max(0.0, frac))
    return 1.0 + frac * (SIZE_MULT_MAX - 1.0)


def size_func_palier(rr):
    """(b) Palier (3 tranches, seuils choisis a la main)."""
    if rr < 1.75:
        return 1.00
    if rr < 2.50:
        return 1.15
...
def size_func_quantile(rr):
    """(c) Quantile (quartiles de la population actuelle, evite un seuil
    RR arbitraire choisi a la main -- 4 tranches d'effectif egal par
    construction)."""
    q1, q2, q3 = RR_QUARTILES
    if rr < q1:
        return 1.00
    if rr < q2:
        return 1.10
...
SIZE_FUNCS = {"linear": size_func_linear, "palier": size_func_palier, "quantile": size_func_quantile}
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
            ...
            bb_threshold=float("inf"), use_any_rr=False, size_func=None):
...
def run_propagated(pop, market_data, excluded_map, ceiling, seq_grouped, format_by_firm, emergency,
    ...
    for _ in range(n_sims):
        ...
        res = run_one(raw_trades, raw_slots, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
                      ...
                      bb_threshold=bb_threshold, use_any_rr=use_any_rr, size_func=size_func)
        rows.append(res)
...
if __name__ == "__main__":
    ...
    variants_arg = sys.argv[3].split(",") if len(sys.argv) > 3 else list(SIZE_FUNCS.keys())
    ...
    # <<< CHANTIER SIZING : seuil de bascule Blueberry = mapping actuellement
    # propose en S1.8 (960/1000->5000, 3000/5000->0). Le baseline SANS sizing
    # (COMBINE) n'est PAS recalcule ici -- reutilise le chiffre n=600 deja
    # confirme (plus precis qu'un nouveau n=300), cite dans l'analyse.
    ...
    for ceiling in ceilings_arg:
        ...
        for variant in variants_arg:
            size_func = SIZE_FUNCS[variant]
            t0 = time.time()
            df = run_propagated(pop, market_data, excluded_map, ceiling, seq, config,
                                 bb_threshold=bb_th, use_any_rr=True, size_func=size_func, **common_kwargs)
            row = summarize(df, f"sizing_{variant} (bb_th={bb_th:.0f}+any-RR)", ceiling, bb_th, True)
            rows.append(row)
            print(f"[sizing_{variant:9s} bb={bb_th:.0f} plafond={ceiling:.0f}$] profit_moy={row['profit_moyen']:+,.0f}$ "
            ...
            pd.DataFrame(rows).to_csv(f"chantier_rr_sizing_n{n_sims}_2026-08-16.csv", index=False)
