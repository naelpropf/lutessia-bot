"""
Section A - Etape 1-2 (2026-08-17) : candidats de sizing continu/paliers sur
rr_tp2, justifies par le diagnostic chantier_A_rr_sizing_diagnostic_2026-08-17.py :
- Deciles sur rr_tp2 NON monotones sur l'ensemble de la plage (Spearman
  global = +0.024, quasi nul) -- le signal fiable est concentre dans les 3
  derniers deciles (rr_tp2 >~ 6.46 : EV=+1.198R/+1.350R/+1.973R, CROISSANT),
  les deciles 1-7 (rr_tp2 < 6.46) oscillent sans tendance (+0.538 a +0.945R,
  aucun ordre).
- Deciles sur rr_tp1 confirment §2.34 (Pearson=+0.006, Spearman=-0.053,
  aucune tendance nulle part).

Copie figee de chantier_rrtp2_sizing_2026-08-16.py (lui-meme copie figee de
chantier_cascade_combined_bb_switch_any_rr_2026-08-16.py), seule la section
<<< CHANTIER A ci-dessous change (3 candidats + reference recalculee dans le
MEME script/seed pour comparabilite stricte).

3 candidats, tous plafonnes a x1.6 (meme marge que le seuil deja adopte
S2.35, 24,0% < 30% -- deja accepte explicitement par l'utilisateur pour
cette valeur, aucune NOUVELLE violation introduite ici) :
  A1_step2   : palier 2 niveaux, x1.3 sur [6.5,8), x1.6 sur [8,+inf) --
               extension ciblee du seuil adopte vers le "pre-tail" (deciles
               8-9) identifie par le diagnostic.
  A2_ramp    : rampe lineaire continue de x1.0 (rr_tp2<=6) a x1.6 (rr_tp2>=15)
               -- version lissee de la meme zone, evite la discontinuite du
               palier.
  A3_decile  : palier sur les 10 deciles COMPLETS de rr_tp2 (x1.0 a x1.6
               lineairement par rang de decile) -- test litteral de la
               demande initiale ("fonction sur l'ensemble de la
               distribution"), attendu MOINS bon d'apres le diagnostic
               puisqu'il majore aussi des deciles sans signal fiable
               (1-7), mais teste empiriquement plutot que rejete par
               deduction seule.
  REF        : seuil simple deja adopte S2.35 (x1.6 sur rr_tp2>=8),
               recalculee ICI (meme script/seed) pour comparaison stricte.
"""
import random
import sys
import time
...
"""
BALAYAGE FIN 2026-08-17 : le sweep grossier (1000/1500/2000/2500/3000,
seuils 0 et 5000 uniquement) a trouve qu'a 2000$ AUCUN des 2 regimes deja
etablis ne domine (bb_th=0 gagne profit+annee1<0, bb_th=5000 gagne solde_
neg+hit_ceiling) -- un vrai arbitrage, pas un choix evident si le plafond
reel est fixe a 2000$. Ce script teste une grille FINE de bb_threshold
(pas seulement les 2 valeurs extremes deja mesurees) A CEILING=2000$ SEUL,
pour verifier si une valeur intermediaire domine les deux regimes connus
(hypothese : le seuil optimal a 2000$ n'est ni 0 ni 5000 mais quelque
part entre les deux -- teste, pas suppose).
"""
if __name__ == "__main__":
    n_sims = int(sys.argv[1]) if len(sys.argv) > 1 else 300
    ceiling = float(sys.argv[2]) if len(sys.argv) > 2 else 2000.0
    thresholds_arg = [float(x) for x in sys.argv[3].split(",")] if len(sys.argv) > 3 else \
        [0.0, 500.0, 1000.0, 1500.0, 2000.0, 2500.0, 3000.0, 3500.0, 4000.0, 4500.0, 5000.0]

    t_start = time.time()
    pop, market_data, excluded_map = load_common()
    print(f"[verif] population construite (RR>={MIN_RR_NEW}) : {len(pop)} trades, plafond={ceiling:.0f}$")
    seq = ei.seq_grouped_multi(1000, 15000, 25000, 25000)
    config = ei.CONFIG_REF

    size_func = make_size_func_tail(1.6, threshold=8.0)  # sizing rr_tp2 deja adopte S2.35
    ...
    common_kwargs = dict(emergency=ei.DEFAULT_EMERGENCY, eval_risk=EVAL_RISK, fleet_risk=FLEET_RISK,
                          ...
                          use_any_rr=True, routing_field="rr_tp2", size_func=size_func)

    rows = []
    for bb_th in thresholds_arg:
        t0 = time.time()
        df = run_propagated(pop, market_data, excluded_map, ceiling, seq, config,
                             bb_threshold=bb_th, **common_kwargs)
        row = summarize(df, f"bb_th={bb_th:.0f}", ceiling)
        rows.append(row)
        print(f"[plafond={ceiling:.0f}$ bb_th={bb_th:.0f}] profit_moy={row['profit_moyen']:+,.0f}$ "
              f"solde_neg={row['solde_negatif_annee4']:.2f}% hit_ceiling={row['hit_ceiling_pct']:.2f}% "
              f"annee1<0={row['annee1_neg']:.2f}% ({time.time()-t0:.0f}s)")
        pd.DataFrame(rows).to_csv(f"chantier_bb_threshold_finegrid_{ceiling:.0f}_n{n_sims}_2026-08-17.csv", index=False)
