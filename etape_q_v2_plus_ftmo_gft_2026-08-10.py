"""
Etape Q (08/10 nuit) : V2 (sizing DD pre-deblocage, piste F) COMBINE avec
FTMO -10% et GFT Goat Guard (confirmes n=600 separement le 08/09, jamais
testes ensemble avec V2). Hypothese : complementaires plutot que redondants
puisque V2 cible le PRE-deblocage (fragilite du starter unique) et les
mecanismes FTMO/GFT ciblent le POST-deblocage (restart complet sur compte
deja finance) -- fenetres temporelles differentes.

Copie de etape_j_dd_distance_recalibration_2026-08-10.py (mecanisme V2
inchange) + mecanismes FTMO-10%/GFT Goat Guard portes depuis
etape_e_ftmo_gft_mechanisms_2026-08-09.py (memes constantes, meme logique).
...
if __name__ == "__main__":
    ...
    configs = [
        ("REF_V2 seul", False, False),
        ("REF_V2 + FTMO-10%", True, False),
        ("REF_V2 + GFT Goat Guard", False, True),
        ("REF_V2 + (a)+(b) combine", True, True),
    ]
    ...
    for label, ftmo_disc, gg in configs:
        for ceiling in ceilings_arg:
            ...
            rows.append(row)
            print(f"[{label:26s} plafond={ceiling:.0f}$] profit={row['profit']:+,.0f}$ ruine={row['ruine']:.2f}% "
                  f"annee1<0={row['annee1_neg']:.2f}% (pre={row['annee1_neg_pre']:.2f}%) "
                  f"casse<=30j={row['break_rate_30d_pct']:.2f}% ({time.time()-t0:.0f}s)")
            pd.DataFrame(rows).to_csv(f"etape_q_v2_plus_ftmo_gft_n{n_sims}.csv", index=False)
