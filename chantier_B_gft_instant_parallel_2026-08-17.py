"""
Section B (2026-08-17) : cumul de comptes GFT Instant en PARALLELE, en
add-on isole par-dessus la reference de travail complete (cascade groupee
BB Instant+any-RR S1.8 + sizing rr_tp2 Variante B x1.6 S2.35).

Contexte/mecanisme teste : GFT Instant SOLO (compte isole, tresorerie
infinie) a ete rejete sans ambiguite en S6.2/S6.3
(`registre_parametres_projet.md`) -- le DD reel 6% trailing de
GFT_InstantGOAT est structurellement trop serre pour ce style de trading
(jusqu'a 15,54-59,20 casses/4ans pour UN SEUL compte selon la variante,
cash requis ~9x plus eleve que le format classique). Cette section teste
une idee DIFFERENTE et structurelle : au lieu d'un seul compte Instant,
faire du format Instant le format des comptes EXTRA (croissance) de GFT
-- deja pris en charge nativement par `process_extra_account`/
`GROWTH_FIRMS_EXTRA` (mecanisme identique a la bascule Blueberry
Classic/Instant deja adoptee en S1.8) -- au lieu du classique
GFT_2Step_GOAT actuellement utilise pour ces extras.

Hypothese a verifier EMPIRIQUEMENT (pas juste deduite) : les comptes GFT
copytradent le MEME signal entre eux (confirme support GFT,
`engine_multiformat.py` note Instant GOAT : "Copy trading is allowed only
between your own funded accounts, including instant accounts") -- donc
plusieurs comptes Instant paralleles subiraient des casses CORRELEES
(meme mecanisme deja identifie en S2.37/Piste H), pas une diversification
independante, ce qui pourrait annuler tout gain de "parallelisation" a
tresorerie fleet reelle (contrairement au test S6.2/S6.3 en tresorerie
infinie isolee).
...
any_rr_2026-08-16.py). Le compte GFT jour0 (classique, N_ACCOUNTS_DAY0=1)
et son seuil de croissance restent INCHANGES -- seul le FORMAT des comptes
EXTRA (`process_extra_account`, deja utilise pour GFT via
`GROWTH_FIRMS_EXTRA`) bascule sur GFT_InstantGOAT quand `gft_extra_instant=
True`.
"""
import random
import sys
...
def run_propagated(pop, market_data, excluded_map, ceiling, seq_grouped, format_by_firm, emergency,
    ...
    for _ in range(n_sims):
        ...
        res = run_one(raw_trades, raw_slots, market_data, excluded_map, order, ceiling, seq_grouped, format_by_firm,
                      ...
                      gft_extra_instant=gft_extra_instant)
        rows.append(res)
...
if __name__ == "__main__":
    ...
    CONFIGS = {
        "REF_travail (S1.8+S2.35)": False,
        "B_gft_instant_parallel": True,
    }

    rows = []
    for ceiling in ceilings_arg:
        bb_th = BB_THRESHOLD_BY_CEILING[ceiling]
        for name, gft_extra_instant in CONFIGS.items():
            t0 = time.time()
            df = run_propagated(pop, market_data, excluded_map, ceiling, seq, config,
                                 bb_threshold=bb_th, gft_extra_instant=gft_extra_instant, **common_kwargs)
            ...
            print(f"[{name} plafond={ceiling:.0f}$] profit_moy={row['profit_moyen']:+,.0f}$ "
                  ...
                  f"({time.time()-t0:.0f}s)")
            pd.DataFrame(rows).to_csv(f"chantier_B_gft_instant_parallel_n{n_sims}_2026-08-17.csv", index=False)
