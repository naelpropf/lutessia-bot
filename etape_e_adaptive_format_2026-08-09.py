"""
Chantier 2 (08/09) : format adaptatif par seuil de reserve -- implementation
corrigee suite au scoping (etape_e_chantier1_risk_resweep_bbreset_2026-08-09.md
+ echange de suivi) qui a identifie 2 problemes structurels a integrer, pas
a contourner :

  1. FIVERS EXCLU du mecanisme adaptatif -- son format rapide (Hyper
     Growth) plafonne a 40k$/compte vs 100k$ REF, sans aucun mecanisme de
     compte supplementaire pour compenser (Fivers n'est pas dans
     GROWTH_FIRMS_EXTRA) -- un vrai manque a gagner structurel, pas un
     arbitrage a tester. Fivers reste TOUJOURS au format REF
     (Fivers_HighStakes, 100k$).

  2. SEUIL DE BASCULE INDEPENDANT des seuils de deblocage actuels (1k-25k$,
     bien trop bas -- y basculer y aurait fige presque toute la flotte au
     format rapide a vie, reproduisant WINNER deja rejete). Balaye
     explicitement : 10k/20k/30k/50k/75k/100k$ de reserve fleet-wide.

  3. BLUEBERRY : arbitrage rapide (Instant Elite, perd le reset a 2x prix
     confirme gagnant, retombe a plein tarif 800$/reopen) vs lent
     (Prime2Step, garde le reset a 330$) NON tranche a l'avance -- teste
     les deux (blueberry_adaptive=True/False) pour chaque seuil.

Format FIGE A LA CREATION (option (a) du scoping) : decide au moment ou le
compte est REELLEMENT ouvert (activation du groupe, ouverture d'un compte
supplementaire), PAS a la pre-allocation initiale (qui a lieu a reserve=0
pour tous les groupes non-STARTER, avant toute activation reelle). Les
reouvertures apres casse REUTILISENT le format fige du compte, jamais
re-evalue.

FTMO/GFT/FundedNext basculent en rapide (formats CONFIG_WINNER) avant le
seuil, REF apres -- Blueberry idem SI blueberry_adaptive=True, sinon
toujours REF (comme Fivers).

Copie de etape_e_final_lock_bbreset_2026-08-09.run_one, augmentee du
choix de format par compte. N'importe pas ces scripts directement.
"""
import random
import sys
import time
...
SLOW_FMT = ei.CONFIG_REF  # {"FTMO":"FTMO_2Step_Swing", "Fivers":"Fivers_HighStakes", "Blueberry":"Blueberry_Prime2Step", "GFT":"GFT_2Step_GOAT", "FundedNext":"FundedNext_StellarLite"}
...
def run_one(trades, slot_arrivals, market_data, excluded_map, order, ceiling, seq_grouped,
            ...
            blueberry_adaptive, switch_threshold):
    accounts_by_group = {g: [] for g in FIRMS}
    active0_cost = 0.0

    # Blueberry (STARTER) : seul groupe reellement actif des t=0 (reserve=0
    # a cet instant) -- si adaptatif, decide donc TOUJOURS "rapide" pour ce
    # compte precis (0 < tout seuil positif teste), independamment du
    # seuil -- resultat attendu, pas un bug.
    ...
    # Autres groupes : places inactifs, format decide plus tard a
    # l'activation reelle (open_group), pas ici (reserve pas encore
    # significative a la pre-allocation).
    ...
    if not FORMATS[bb_fmt_key]["phases"]:
        # Blueberry demarre en format instant (0 phase) -- deja "funded" a
        # la creation, ne passera jamais par une transition challenge->
        # funded detectable (meme correction que _init_own_funded dans
        # etape_e_fleet_integration.py, necessaire ici aussi car le format
        # de depart de Blueberry peut desormais varier selon bb_adaptive).
    ...
    quasi_frozen_flag = state["reserve"] < 100
...
if __name__ == "__main__":
    ...
    if mode == "screen":
        ...
        for thresh in THRESHOLDS:
            for bb_adapt in (True, False):
                label = f"seuil={thresh/1000:.0f}k bb_adapt={bb_adapt}"
                for ceiling in (1000.0, 3000.0):
                    ...
                    df = run_propagated(pop, market_data, excluded_map, ceiling, seq, ei.DEFAULT_EMERGENCY,
                                         ...
                                         ei.EXTRA_THRESHOLD_MULT, n_sims=n_sims, seed=11000,
                                         blueberry_adaptive=bb_adapt, switch_threshold=thresh)
                    row = summarize(df, label)
                    row["ceiling"] = ceiling
                    row["switch_threshold"] = thresh
                    row["blueberry_adaptive"] = bb_adapt
                    rows.append(row)
                    print(f"[{label} plafond={ceiling:.0f}$] profit={row['profit']:+,.0f}$ ruine={row['ruine']:.2f}% "
                          ...
                          f"({time.time()-t0:.0f}s)")
                    pd.DataFrame(rows).to_csv("etape_e_adaptive_format_screen.csv", index=False)

    elif mode == "confirm":
        thresh = float(sys.argv[3]) * 1000
        bb_adapt = sys.argv[4].lower() == "true"
        rows = []
        for ceiling in (1000.0, 3000.0):
            t0 = time.time()
            df = run_propagated(pop, market_data, excluded_map, ceiling, seq, ei.DEFAULT_EMERGENCY,
                                 EVAL_RISK, FLEET_RISK, GFT_EVAL_RISK, ei.FINAL_RESERVE_SHARE,
                                 ei.EXTRA_THRESHOLD_MULT, n_sims=n_sims, seed=11999,
                                 blueberry_adaptive=bb_adapt, switch_threshold=thresh)
            row = summarize(df, f"CONFIRM seuil={thresh/1000:.0f}k bb_adapt={bb_adapt}")
            row["ceiling"] = ceiling
            rows.append(row)
            print(f"[plafond={ceiling:.0f}$] profit={row['profit']:+,.0f}$ ruine={row['ruine']:.2f}% "
                  ...
                  f"bb_resets={row['mean_bb_resets']:.2f} casse<=30j={row['break_rate_30d_pct']:.2f}% "
                  f"casse<=60j={row['break_rate_60d_pct']:.2f}% quasi_gele={row['quasi_frozen_pct']:.1f}% "
                  f"({time.time()-t0:.0f}s)")
            pd.DataFrame(rows).to_csv(f"etape_e_adaptive_format_confirm_n{n_sims}.csv", index=False)
